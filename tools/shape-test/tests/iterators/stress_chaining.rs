...
fn test_iter_map_then_any() {
    ShapeTest::new(r#"[1, 2, 3].iter().map(|x| x * 10).any(|x| x > 25)"#).expect_bool(true);
}

/// Iter map then all.
#[test]
fn test_iter_map_then_all() {
    ShapeTest::new(r#"[1, 2, 3].iter().map(|x| x * 10).all(|x| x > 5)"#).expect_bool(true);
}
...
fn test_find_first_greater_than() {
    ShapeTest::new(r#"[5, 10, 15, 20, 25].iter().find(|x| x > 12)"#).expect_number(15.0);
}

/// All positive.
#[test]
fn test_all_positive() {
    ShapeTest::new(r#"[1, 2, 3, 4, 5].iter().all(|x| x > 0)"#).expect_bool(true);
}

/// Any negative.
#[test]
fn test_any_negative() {
    ShapeTest::new(r#"[1, 2, 3, 4, 5].iter().any(|x| x < 0)"#).expect_bool(false);
}
...
fn test_array_includes() {
    ShapeTest::new(r#"[1, 2, 3, 4, 5].includes(3)"#).expect_bool(true);
}

/// Array includes missing.
#[test]
fn test_array_includes_missing() {
    ShapeTest::new(r#"[1, 2, 3, 4, 5].includes(99)"#).expect_bool(false);
}

/// Array indexOf.
#[test]
fn test_array_indexOf() {
    ShapeTest::new(r#"[10, 20, 30, 40].indexOf(30)"#).expect_number(2.0);
}

/// Array concat.
#[test]
fn test_array_concat() {
    ShapeTest::new(r#"[1, 2, 3].concat([4, 5, 6]).length"#).expect_number(6.0);
}

/// Array unique.
#[test]
fn test_array_unique() {
    ShapeTest::new(r#"[1, 2, 2, 3, 3, 3].unique().length"#).expect_number(3.0);
}

/// Array flatten.
#[test]
fn test_array_flatten() {
    ShapeTest::new(r#"[[1, 2], [3, 4], [5, 6]].flatten().length"#).expect_number(6.0);
}

/// Array slice.
#[test]
fn test_array_slice() {
    ShapeTest::new(r#"[10, 20, 30, 40, 50].slice(1, 4)[0]"#).expect_number(20.0);
}

/// Array join string.
#[test]
fn test_array_join_string() {
    ShapeTest::new(r#"["a", "b", "c"].join(", ")"#).expect_string("a, b, c");
}
...
fn test_array_findindex() {
    ShapeTest::new(r#"[10, 20, 30, 40, 50].findIndex(|x| x > 25)"#).expect_number(2.0);
}

/// Array findIndex not found.
#[test]
fn test_array_findindex_not_found() {
    ShapeTest::new(r#"[10, 20, 30].findIndex(|x| x > 100)"#).expect_number(-1.0);
}
...
fn test_iter_take_one() {
    ShapeTest::new(r#"[10, 20, 30].iter().take(1).collect()[0]"#).expect_number(10.0);
}

/// Iter skip one.
#[test]
fn test_iter_skip_one() {
    ShapeTest::new(r#"[10, 20, 30].iter().skip(1).collect()[0]"#).expect_number(20.0);
}
