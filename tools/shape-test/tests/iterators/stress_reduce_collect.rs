...
fn test_iter_reduce_sum() {
    ShapeTest::new(r#"[1, 2, 3, 4, 5].iter().reduce(|acc, x| acc + x, 0)"#)
        .expect_number(15.0);
}
...
fn test_iter_reduce_product() {
    ShapeTest::new(r#"[1, 2, 3, 4].iter().reduce(|acc, x| acc * x, 1)"#)
        .expect_number(24.0);
}
...
fn test_iter_find_found() {
    ShapeTest::new(r#"[10, 20, 30, 40].iter().find(|x| x > 25)"#)
        .expect_number(30.0);
}
...
fn test_iter_find_not_found() {
    ShapeTest::new(r#"[10, 20, 30].iter().find(|x| x > 100)"#)
        .expect_none();
}
...
fn test_iter_find_first_element() {
    ShapeTest::new(r#"[1, 2, 3].iter().find(|x| x > 0)"#)
        .expect_number(1.0);
}
...
fn test_iter_any_true() {
    ShapeTest::new(r#"[1, 2, 3, 4, 5].iter().any(|x| x > 4)"#)
        .expect_bool(true);
}
...
fn test_iter_any_false() {
    ShapeTest::new(r#"[1, 2, 3].iter().any(|x| x > 10)"#)
        .expect_bool(false);
}
...
fn test_iter_all_true() {
    ShapeTest::new(r#"[2, 4, 6].iter().all(|x| x > 0)"#)
        .expect_bool(true);
}
...
fn test_iter_all_false() {
    ShapeTest::new(r#"[2, 4, 6].iter().all(|x| x > 3)"#)
        .expect_bool(false);
}
...
fn test_iter_enumerate_count() {
    ShapeTest::new(r#"[10, 20, 30, 40].iter().enumerate().count()"#)
        .expect_number(4.0);
}
...
fn test_single_element_iter_any_check() {
    ShapeTest::new(r#"[42].iter().any(|x| x == 42)"#)
        .expect_bool(true);
}
...
#[test]
fn test_array_skip() {
    ShapeTest::new(r#"[1, 2, 3, 4, 5].skip(2)[0]"#)
        .expect_number(3.0);
}
...
#[test]
fn test_array_first() {
    ShapeTest::new(r#"[10, 20, 30].first()"#)
        .expect_number(10.0);
}
...
fn test_string_iter_take() {
    ShapeTest::new(r#""abcde".iter().take(3).collect()[0]"#)
        .expect_string("a");
}
...
fn test_string_iter_skip() {
    ShapeTest::new(r#""abcde".iter().skip(3).collect()[0]"#)
        .expect_string("d");
}
