...
#[test]
fn ok_wrap_int() {
    ShapeTest::new("match Ok(42) { Ok(v) => v, Err(e) => -1 }")
        .expect_number(42.0);
}
...
#[test]
fn ok_wrap_zero() {
    ShapeTest::new("match Ok(0) { Ok(v) => v, Err(e) => -1 }")
        .expect_number(0.0);
}
...
#[test]
fn ok_wrap_negative() {
    ShapeTest::new("match Ok(-1) { Ok(v) => v, Err(e) => 999 }")
        .expect_number(-1.0);
}
...
fn ok_wrap_bool_true() {
    ShapeTest::new("match Ok(true) { Ok(v) => v, Err(e) => false }")
        .expect_bool(true);
}
...
fn ok_wrap_bool_false() {
    ShapeTest::new("match Ok(false) { Ok(v) => v, Err(e) => true }")
        .expect_bool(false);
}
...
#[test]
fn ok_wrap_string() {
    ShapeTest::new(r#"match Ok("hello") { Ok(v) => v, Err(e) => "err" }"#)
        .expect_string("hello");
}
...
#[test]
fn ok_wrap_float() {
    ShapeTest::new("match Ok(3.14) { Ok(v) => v, Err(e) => 0.0 }")
        .expect_number(3.14);
}
...
fn ok_wrap_large_int() {
    ShapeTest::new("match Ok(999999) { Ok(v) => v, Err(e) => -1 }")
...
fn err_wrap_short_string() {
    ShapeTest::new(r#"match Err("fail") { Ok(v) => 1, Err(e) => -1 }"#)
        .expect_number(-1.0);
}
...
fn err_wrap_empty_string() {
    ShapeTest::new(r#"match Err("") { Ok(v) => 1, Err(e) => -1 }"#)
        .expect_number(-1.0);
}
...
#[test]
fn err_wrap_int() {
    ShapeTest::new("match Err(404) { Ok(v) => 1, Err(e) => -1 }")
        .expect_number(-1.0);
}
...
#[test]
fn err_is_not_ok() {
    ShapeTest::new(r#"match Err("bad") { Ok(v) => 1, Err(e) => -1 }"#)
        .expect_number(-1.0);
}
...
#[test]
fn ok_is_not_err() {
    ShapeTest::new("match Ok(1) { Ok(v) => v, Err(e) => -1 }")
        .expect_number(1.0);
}
...
fn match_err_extracts_error() {
    ShapeTest::new(r#"let x = Err("fail")
...
fn match_err_with_string_message() {
    ShapeTest::new(r#"let x = Err("boom")
...
fn match_err_returns_fallback_int() {
    ShapeTest::new(r#"let x = Err("nope")
...
#[test]
fn ok_wrap_array() {
    ShapeTest::new("match Ok([1, 2, 3]) { Ok(v) => v.length, Err(e) => -1 }")
        .expect_number(3.0);
}
...
fn err_wrap_with_number_payload() {
    ShapeTest::new("match Err(42) { Ok(v) => 1, Err(e) => -1 }")
        .expect_number(-1.0);
}
...
fn nested_result_ok_ok() {
    ShapeTest::new("match Ok(Ok(42)) { Ok(inner) => match inner { Ok(v) => v, Err(e) => -1 }, Err(e) => -2 }")
        .expect_number(42.0);
}
...
fn nested_result_err_wrapper() {
    ShapeTest::new(r#"match Err("outer fail") { Ok(v) => 1, Err(e) => -1 }"#)
        .expect_number(-1.0);
}
...
fn err_stored_in_variable() {
    ShapeTest::new(r#"let r = Err("fail")
...
fn err_reassigned_to_ok() {
    ShapeTest::new(r#"var r = Err("fail")
r = Ok(42)
match r { Ok(v) => v, Err(e) => -1 }"#)
        .expect_number(42.0);
}
...
fn match_err_string_payload_extraction() {
    ShapeTest::new(r#"let x = Err("error message")
...
fn ok_with_computed_expression() {
    ShapeTest::new("match Ok(2 + 3 * 4) { Ok(v) => v, Err(e) => -1 }")
        .expect_number(14.0);
}
...
fn ok_containing_negative_zero() {
    ShapeTest::new("match Ok(-0.0) { Ok(v) => v, Err(e) => 999.0 }")
        .expect_number(0.0);
}
...
fn match_result_computed_ok_value() {
    ShapeTest::new("fn test() -> int { let x = Ok(3 * 7)\nmatch x { Ok(v) => v, Err(e) => 0 } }\ntest()")
        .expect_number(21.0);
}
...
fn match_on_direct_ok_literal() {
    ShapeTest::new("match Ok(99) { Ok(v) => v, Err(e) => -1 }")
        .expect_number(99.0);
}
...
fn match_on_direct_err_literal() {
    ShapeTest::new(r#"match Err("boom") { Ok(v) => 1, Err(e) => -1 }"#)
        .expect_number(-1.0);
}
...
fn ok_wrapping_large_negative() {
    ShapeTest::new("match Ok(-999999) { Ok(v) => v, Err(e) => 0 }")
        .expect_number(-999999.0);
}
...
fn err_wrap_bool_payload() {
    ShapeTest::new("match Err(false) { Ok(v) => 1, Err(e) => -1 }")
        .expect_number(-1.0);
}
...
fn ok_wrapping_empty_string() {
    ShapeTest::new(r#"match Ok("") { Ok(v) => v, Err(e) => "err" }"#)
        .expect_string("");
}
