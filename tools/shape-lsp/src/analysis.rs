...
use crate::diagnostics::{
    ...
    validate_comptime_side_effects, validate_foreign_function_async,
    validate_foreign_function_marshal_types, validate_foreign_function_types,
...
pub fn analyze_program_semantics_for_document(
    ...
    // ADR-019 §5 (#201) — the editor must refuse `async fn python` for the same
    // reason `shape run` does. TRANSITIONAL: deleted when #202 lands.
    diagnostics.extend(validate_foreign_function_async(program, text));
    // ADR-019 §1 (#196) — the editor must refuse a declared type that cannot
    // cross the foreign boundary, for the same reason `shape run` does.
    diagnostics.extend(validate_foreign_function_marshal_types(program, text));
...
"#;
    ...
    // ADR-009 E2 #18 5b Part B (companion A): `generated_capture_diagnostics_
    // survive_an_unrelated_recovered_error` RETIRED. Its subject was a REPARSED
    // capturing closure emitting the [C0910] source-unavailable diagnostic (the
    // deleted U03 route); post-deletion no capture is reparse-source-unavailable,
    // so the fixture has no live [C0910] to assert. (The retain-mode rollback
    // behavior it also touched is a distinct concern covered by the install-
    // transaction rollback tests in functions_annotations/c2_slice0_preflight.)
