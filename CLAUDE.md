...
3. **VM Interpreter** (shape-vm): Stack-based execution with typed 8-byte slots driven by per-slot `NativeKind` metadata (parallel `Vec<u64>` data + `Vec<NativeKind>` kinds per ADR-006 §2.7.7), feedback vectors for type profiling
...
- **Typed slots** (`NativeKind` / `KindedSlot` / per-FieldType typed pointers per ADR-006 §2.3 + §2.7): 8-byte raw native values (`Float64`, `Int64`, `Int32`, `Int8`, `Bool`, `Ptr(HeapKind)`) carry the type inline; opcodes encode the kind. No runtime tag decoding, no dynamic fallback. The legacy `ValueWord` tagged-word path is deleted (`value_word.rs` removed from `crates/shape-value/src/`); §Forbidden Patterns refuses re-introduction under any rename. Snapshot/wire serialization uses parallel `Vec<u64>` data + `Vec<NativeKind>` kinds per ADR-006 §2.7.7 / Q9.
- **`NativeKind`** (`crates/shape-value/src/native_kind.rs`): per-slot type metadata, 27 variants — the full signed/unsigned width family (`Int8/16/32/64/Size`, `UInt8/16/32/64/Size`, plus `Nullable*` forms), `Float64` / `NullableFloat64` / `Float32` / `Char` / `Bool` / `Null` (the merged unit/null absence sentinel — there is NO separate `Unit` variant) / `Ptr(HeapKind)` / `String` / `StringV2` / `DecimalV2`. Stamped at compile time per §2.7.5; never fabricated from raw bits. **Bit encodings are normative in `crates/shape-value/src/encoding.rs` (ADR-020 §2.1) — no private encodings.**
...
- **VM stack: parallel `Vec<NativeKind>` track** alongside `Vec<u64>` data slots (§2.7.7 / Q9) drives `clone_with_kind` / `drop_with_kind` — no tag decode, no `is_heap()` probe. Forbidden: `Vec<KindedSlot>` for the stack; 16-byte slots (NOTE per ADR-020 §5: a value spanning N *consecutive 8-byte slots* with a statically-known per-type count is NOT a 16-byte slot — the slot ELEMENT stays 8 bytes; one `SlotId` per value through MIR/solver/planner, expansion only at frame layout/local declaration); packed tag bits; `Option<NativeKind>` / `Unknown` placeholders; transitional shims with deleted ValueWord-shape names (`push_raw_u64`, `pop_raw_u64`, `push_native_i64`, `stack_read_owned`, `stack_peek_raw`) backed by Bool-default kinded primitives.
...
- `just verify-merge` / `bash scripts/verify-merge.sh` — Phase 2d merge gate (21 checks as of 2026-07-28, exit-code-based, NOT grep -c). Required pre-merge for every Phase 2d sub-cluster branch. Catches the 4 take-both regex misses + HeapKind ordinal collisions + 4-table HeapKind lockstep + receiver-recovery suspicious patterns (3ac2f11 soundness rule heuristic) + the ADR-011–016 legacy-authority ratchets (CHECK 15/16) and the ADR-016 public feature manifest contract (CHECK 17) Book coverage manifest contract (CHECK 18), public-feature candidate denominator (CHECK 19), Book fence universe (CHECK 20), the ADR-018 §2 whole-program JIT bail ratchet (CHECK 21), and the JIT FFI reachability ratchet (CHECK 22, #216 — registration is not reachability).
- **`verify-merge.sh` reports PASS on a CONFLICTED tree.** Observed three times on 2026-08-01. It does not detect an in-progress merge, so a green gate is **not** evidence that a merge completed — only the absence of `UU`/`AA` entries in `git status --porcelain` is. Check for unmerged paths before reading the gate, and note that `git push` answering "Everything up-to-date" right after a "successful" merge is the tell.
- **Registration is not reachability** (CHECK 22, `scripts/check-jit-ffi-reachability.mjs`). An FFI symbol becomes callable from emitted code only via a Cranelift `FuncRef`, which only `ffi_builder.rs`'s `r!("...")` can mint for it. A `builder.symbol` / `declare_function` registration with no `r!()` key is unreachable no matter how it looks — the class that produced `jit_v2_string_eq` (correct, tested, never registered) and `jit_v2_release` (registered, declared, size-blind `Layout(8,8)` dealloc, filed as live UB, in fact uncallable). New instances fail the gate; the existing debt is a shrink-only baseline at `docs/program/adr011-012/baselines/jit-ffi-reachability-inventory.json`. **A green CHECK 22 means *callable*, not *live*** — it covers tiers 1→2→3 (registered → declared → given a referenced `FuncRef`); tier 4, "actually reached by a real program", is not statically decidable and needs a corpus, which is currently vacuous (see §Known Constraints on the vm/jit differential).
...
- **v2-raw-heap-audit — UPDATED 2026-05-17 (W5 close)** per `docs/cluster-audits/v0.3-w5-v2-raw-residuals.md`: the two residual SIGABRT anchors at cluster-1.5 close (2026-05-17) are now RESOLVED: (i) `length_typed_object_empty` was a test-fixture allocator-pair mismatch (legacy `Arc::new(TypedObjectStorage::new(...))` + `Arc::into_raw` flowing through the v2-raw `drop_with_kind(Ptr(HeapKind::TypedObject))` dispatch that expects `_new`-allocated memory) — fix migrates the test + 5 sibling tests (`set_prop_typed_object_*` × 2, `set_field_typed_schema_match_*`, `clone_then_double_drop_balances_refcount`, `test_typed_object_kinded_slot_discriminates_via_kind_label`, `drop_decrements_arc_typed_object_for_heap_pointer_slot`) to the v2-raw `_new` carrier matching production code; (ii) `w17_comptime_*` (4 tests: build_config / implements / warning / error `_dispatches_end_to_end`) was a share-accounting double-claim at the VM-state snapshot-clone boundary in `crates/shape-vm/src/executor/vm_state_snapshot.rs:295::clone_slot_kinded` — the pre-fix `KindedSlot::new(...)` + `clone()` shape claimed ownership without bumping the underlying refcount, then the cancelling clone+drop pair left the returned `KindedSlot` owning a ghost share that drove a use-after-free at snapshot drop time. Fix mirrors the cluster-1.5 share-accounting pattern (explicit `clone_with_kind` retain before `KindedSlot::new` claim — same shape as `executor/mod.rs:792::module_binding_read_owned_kinded`). Cluster-2 §D Class 1 v2-raw anchors fully retired; the broader v2-raw aliasing class no longer has any live SIGABRT anchors at workspace HEAD. Residual surfaces (the 4 `bin/shape-cli/tests/stdlib/simulation.rs` ignored tests, JIT `#[ignore]` set) remain blocked on V3-S5 ckpt-5/ckpt-6 SURFACE territory + JIT-heavy parallel-execution caching — distinct root cause families, tracked per their own follow-up cadence.
...
- **The vm/jit corpus differential is mostly VACUOUS right now — a `MATCH` usually means the JIT never ran** (#260, measured 2026-08-01 by two lanes independently). A program whose `--mode jit` leg falls back executes the interpreter on *both* sides, so the differential compares the interpreter against itself and **cannot observe a JIT defect even in principle**. Measured native-executing share: **123/483 (25.5%)** pre-#257 — independently reproduced as 121/481 by a second lane — and **measured at 11/488 (2.3%) on main after #257** (`17e8591b`) made `write_place` refuse unproven destinations — **475 of 486 MATCHes are vacuous**, and all 11 native-executing programs have exactly **one** dispatch each, so the corpus is one dispatch away from measuring nothing at all under JIT. So a claim of the form *"we ran the corpus and observed zero hits across 481 programs"* has an effective denominator near **11**, not 481. **`run-diff.mjs` now reports this per run** (`92540057`) — read the native-executing line before quoting any corpus result. **Quote the native-executing denominator with any corpus-derived claim, and assert `sum(native_dispatches) > 0` via `--native-witness` rather than tier agreement** — note `program_fallback == null` is the WRONG metric, since a per-function bail leaves it null while nothing runs natively (the two were measured differing by an order of magnitude on one sample). Recovery is deliberate work — the #239 conversion plus #240/#262 kind-stamping — not a rebound. Corollary for CHECK 22: it mechanizes reachability tiers 1→2→3 (registered → declared → given a `FuncRef` that is referenced), but **tier 4, "actually reached by a real program", is not statically decidable and needs a corpus** — so a green CHECK 22 means *callable*, not *live*, and there is currently no working instrument for *live*.
