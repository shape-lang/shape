...
- **~25 pre-existing deep-test failures** in shape-vm (down from 41 pre-architectural-cleanup). The path-b architectural cleanup (per-Runtime `TypeSchemaRegistry`, per-InferenceContext `TypeVarGen`, per-VM `ShapeTransitionTable`, per-Runtime `core_stdlib_cache` and `extension_module_schemas`, plus BUG1–BUG5 fixes and the refcount-retain audit) retired 16 of the original failures; the residual are mostly in `module_qualified_type_tests::*`, `extension_integration_tests::*`, and `typed_object_regression_tests::*`. Documented categories in `/home/dev/.claude/plans/cleanup-41-residual-failures.md` (I1/I2/I3/I5).
- **shape-jit heavy-execution tests gated behind `deep-tests`**: The five test modules that call `JITExecutor::execute_program` (`mir_compiler::integration_tests`, `mir_compiler::v2_array_tests`, `compiler::a1d2_tests`, `compiler::a1e_tests`) each JIT-compile ~118 stdlib functions via MirToIR per test. Collectively they made the shape-jit test binary slow enough to miss the summary line under any reasonable timeout, and racy enough at default n-cpu parallelism to SIGILL in the JIT code cache (observed on `jit-v2-phase1@2d52e17` and again on `path-c-integration`). They now live behind `#[cfg(all(test, feature = "deep-tests"))]` and run via `just test` / `just test-all` / `just test-deep`. `cargo test -p shape-jit --lib` (Tier 1) completes in ~50s with 394 passing tests. Gating is behavioral — the gated tests themselves still pass individually, and the root-cause perf/codegen work is tracked as a follow-up (stdlib JIT-compilation caching).
- **B1.7 deferred** (follow-up task): `STDLIB_SCHEMA_REGISTRY` + `NEXT_SCHEMA_ID` statics in `type_schema/mod.rs` remain as fallbacks behind the per-Runtime `TypeSchemaRegistry`. Removing them requires aligning the per-instance counter with the legacy global counter's builtin-ID domain (candidate fix: `Runtime::new` seeds its registry's `next_id` from a process-wide allocator, or registers builtins with fixed well-known IDs before free-function paths can see them). Safe to live with — the statics are shimmed behind `current_registry()` so behavior is correct; the only remaining liability is that a process can still see cross-test ID drift through the legacy paths until the shim is removed.
