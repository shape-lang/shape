...
- **shape-jit SIGILL in `cargo test -p shape-jit --lib`**: pre-existing, reproduces on `jit-v2-phase1@2d52e17`. The individual JIT subtests pass in isolation; the aggregate binary crashes with SIGILL before summary. Does not block cargo test on other crates.
