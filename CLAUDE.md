...
- `just verify-merge` / `bash scripts/verify-merge.sh` — Phase 2d merge gate (18 checks as of 2026-07-28, exit-code-based, NOT grep -c). Required pre-merge for every Phase 2d sub-cluster branch. Catches the 4 take-both regex misses + HeapKind ordinal collisions + 4-table HeapKind lockstep + receiver-recovery suspicious patterns (3ac2f11 soundness rule heuristic) + the ADR-011–016 legacy-authority ratchets (CHECK 15/16) and the ADR-016 public feature manifest contract (CHECK 17) and Book coverage manifest contract (CHECK 18).
