...
- **`NativeKind`** (`crates/shape-value/src/native_kind.rs`): per-slot type metadata. Variants: `Float64` / `NullableFloat64` / `Float32` / `Char` / `Int64` / `Int32` / `Int8` / `Bool` / `Null` (the merged unit/null absence sentinel — there is NO separate `Unit` variant) / `Ptr(HeapKind)` / `String` / `StringV2` / `DecimalV2`. Stamped at compile time per §2.7.5; never fabricated from raw bits.
