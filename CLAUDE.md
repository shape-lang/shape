...
- **v2-raw-heap aliasing class — 4 simulation tests `#[ignore]`'d** in `bin/shape-cli/tests/stdlib/simulation.rs` (`test_harmonic_oscillator_rk4_system`, `test_rk45_system_harmonic_oscillator`, `test_find_collisions_brute`, `test_find_collisions_sweep`): `typed_array_push_*` realloc invalidates aliased raw pointers held across iterations → VM Drop double-free. Same bug class as FR/B6/WB2 refcount-audit waves. Repro shapes: hot-loop closure `|t,y| [y[1], -y[0]]` in RK4, or `for i in range(0,n) { let x = arr[i]; fn(arr) }` over `Array<TypedObject>`. Precedent fix = `c-stdlib-msgpack` `vw_clone`/`vw_drop` (commit `afb1651`). Tracked as `v2-raw-heap-audit`.
