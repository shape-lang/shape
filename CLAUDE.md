...
- **`NativeKind`** (`crates/shape-value/src/native_kind.rs`): per-slot type metadata. Variants: `Float64` / `Int64` / `Int32` / `Int8` / `Bool` / `Unit` / `Null` / `Ptr(HeapKind)` / `String` / `StringV2` / `DecimalV2`. Stamped at compile time per §2.7.5; never fabricated from raw bits.
