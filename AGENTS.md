...
| `bulldozer-strictly-typed-phase-1b-vm` | `../shape-phase-1b-vm` | Phase 1.B-vm consumer migration | — | territory: `crates/shape-value/src/kinded_slot.rs` (bounded additions only) + `crates/shape-vm/src/executor/vm_impl/builtins.rs` + `crates/shape-vm/src/compiler/comptime_builtins.rs:227-254` + `crates/shape-vm/src/executor/builtins/kind_coerce.rs` (NEW free helper) | idle (last: Wave 5a close `66e7b90` — KindedSlot scalar accessor/ctor bound per Q8/§2.7.6; dispatch shape flip in `vm_impl/builtins.rs`; `build_config → OpaqueTypedObject`; shape-vm 2000→1874 Δ -126; ~95 `todo!` stubs left for 5b-5e) | 2026-05-08 |
