...
| `bulldozer-strictly-typed-phase-1b-vm` | `../shape-phase-1b-vm` | Phase 1.B-vm consumer migration | — | territory: `crates/shape-vm/src/executor/{builtins/{math,array_ops,minimize,object_ops,runtime_delegated,intrinsics/{math,signal,statistical}},vm_impl/builtins}.rs` | idle (last: Wave 5b close `fa2bafc` — math/array/object_rest body migration to `&[KindedSlot] -> Result<KindedSlot, VMError>` + dispatch arms wired (24 math + 8 array + 1 object_rest + Snapshot/Exit inlined; Print/Format deferred to 5e); Δ shape-vm 1874→1718, -156; cumulative -282; `pop_builtin_args` landed with Wave 6 architectural surface (no per-stack-slot kind metadata; transitional `NativeKind::Bool` tagging surfaces gap as runtime type-error rather than silent corruption)) | 2026-05-08 |
