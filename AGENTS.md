...
| `bulldozer-strictly-typed-phase-1b-vm` | `../shape-phase-1b-vm` | Phase 1.B-vm consumer migration | Wave 3 (test registration hygiene — `register_test_function` → variadic `register_typed_function`; `TypedReturn::Bool` → `Concrete(ConcreteReturn::Bool)`; `ValueWordDisplay` → `format!`; add `KindedSlot::as_str()`) | territory: `crates/shape-vm/src/{executor/tests/mod,lib_tests_parts/extension_*tests,compiler/{compiler_tests,comptime,comptime_builtins},executor/objects/content_methods}.rs` | Wave 1 `95d52bc`, Wave 2 `9d01005` (audit follow-up: `NativeKind::Dynamic` also deleted; corrected pattern in audit doc); Wave 3 dispatched 2026-05-08 | 2026-05-08 |
