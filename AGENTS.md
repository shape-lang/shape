...
| `bulldozer-strictly-typed-phase-1b-vm` | `../shape-phase-1b-vm` | Phase 1.B-vm consumer migration | Wave 6.5 (delete transitional shims from Wave 6.0 + migrate the +700 cascade to kinded API: arithmetic/comparison/loops/call_convention/raw_helpers/control_flow ValueWord callers; grep-fail gate) | territory: `crates/shape-vm/src/executor/{vm_impl/stack,objects/raw_helpers,arithmetic/mod,comparison/mod,loops/mod,call_convention,control_flow/mod}.rs` + every caller of the deleted shims | Wave 6.0 `d782401` landed parallel kind track + helpers but introduced shim layer (rejected by supervisor as W-series defection-attractor; §2.7.7 explicit-forbid landed); Wave 6.5 dispatched 2026-05-09 to delete shims + migrate cascade | 2026-05-09 |
