...
| `bulldozer-strictly-typed-phase-1b-vm-cluster-A` | `../shape-phase-1b-vm-cluster-A` | Phase 1.B-vm Wave 6.5 substep-2 cluster A migrator | — | `crates/shape-vm/src/executor/{arithmetic,comparison,logical}/mod.rs` + `executor/builtins/kind_coerce.rs` | idle (last: cluster-A close `<TBD>` — kind_coerce shared helpers + arithmetic/comparison/logical migrated to kinded API; shape-vm --lib 2591→2338 errors, Δ -253; both shim grep gates zero; check-no-dynamic exits 0 with -2/-1 baseline lowering) | 2026-05-09 |
