...
| `bulldozer-strictly-typed-phase-1b-vm` | `../shape-phase-1b-vm` | Phase 1.B-vm consumer migration | Wave 5a (foundation — bounded `KindedSlot` accessor/constructor additions per Q8/§2.7.6; central `vm_impl/builtins.rs` dispatch slice; `build_config` → `OpaqueTypedObject`; pre-req for 5b-5e) | territory: `crates/shape-value/src/kinded_slot.rs` (bounded additions only) + `crates/shape-vm/src/executor/vm_impl/builtins.rs` + `crates/shape-vm/src/compiler/comptime_builtins.rs:227-254` | Wave 1-4 cumulative Δ -69; Wave 5 stopped pre-edits surfacing body-migration shape (Q8 ruling §2.7.6 landed); Wave 5a dispatched 2026-05-08 | 2026-05-08 |
