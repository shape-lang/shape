...
| `bulldozer-strictly-typed-phase-1b-vm` | `../shape-phase-1b-vm` | Phase 1.B-vm consumer migration | — | territory: `crates/shape-vm/src/executor/{vm_impl/stack,objects/raw_helpers,stack_ops/mod,arithmetic/mod,comparison/mod,logical/mod,loops/mod,call_convention,control_flow/mod}.rs`, `crates/shape-vm/src/bytecode/opcode_defs.rs` | idle (last: Wave 6 close — parallel `Vec<NativeKind>` track + `clone_with_kind`/`drop_with_kind` helpers landed in `vm_impl/stack.rs`; `pop_builtin_args` reads real per-arg kind from parallel track; `stack_ops/mod.rs` and `logical/mod.rs` migrated to kinded API; cumulative Δ shape-vm 1718→1684, -34) | 2026-05-09 |
