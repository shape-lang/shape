...
## 2026-07-13 — ADR009-B6 S1: FrozenCallable payload + composite-memo widening (rejected compromises)

Slice: reflect(TypeRef<callable>) → a fully-populated `FrozenType::Callable(FrozenCallable)` (catalog ordinal 6, appended to the enabled-payload catalog; no ordinal renumber). Ordered `ParamDescriptor`s (value-type identity + optional + sealed `PassingMode`) + return identity. Enforces Dec 52 (R3): an unresolved signature is rejected by the single freeze-boundary predicate BEFORE any hook.

**1. A parallel identity-keyed structural side-table beside the composite memo.**
The one-way SHA-256 `FrozenTypeIdentity` drops param NAMES (identity-insignificant by design) and PASSING MODES (never in the descriptor string) — so `payload_of` cannot recover a `FrozenCallable` from the identity alone. The tempting shape was a second `HashMap<FrozenTypeIdentity, CallableStructure>` threaded alongside the existing `composites` memo (and a sibling at the base index beside `frozen_primitive_payloads`). REJECTED per CLAUDE.md Forbidden Patterns (no parallel producer/consumer carriers that can drift): a second table keyed by the same identity is exactly the drift attractor. Chosen: WIDEN the memo value in place (`FrozenTypeCategory` → `CompositeMemoEntry { category, callable: Option<CallableDescriptor> }`) so category and structure are one write, one read, one lock — the load-bearing S1 change. The base index stores the callable descriptor symmetrically with the existing `frozen_primitive_payloads` (same construction point, populated in the same `rebuild_frozen_type_index` pass), not a new subsystem.

**2. Modeling a signature as one homogeneous `Array<Any>` of params.**
Carrying params as a single top-typed `Array<Any>` (or `FieldType::Any` on the value-carrier `params` field) was refused: a signature's parameters are HETEROGENEOUS signature-indexed descriptors, and `FieldType::Any` outside the named-exception classes is the schema-side analogue of the deleted dynamic-slot-kind variants (E0900). Chosen: the value carrier's `params` element is the TYPED `ParamDescriptor` object (`FieldType::Object(COMPTIME_FROZEN_PARAM_DESCRIPTOR_SCHEMA)`) — no `FieldType::Any`, no post-inference whitelist entry (distinct from `FrozenErased.bounds`, whose element is a provably-empty/uninhabited set that is legitimately whitelisted). The R2 rejection (a user modeling params as `Array<Any>`) is the pre-existing Any-erasure family.

**3. Passing mode baked into the canonical identity / a `param("name")` string selector.**
Encoding the mode as a descriptor token (e.g. splitting `reference:&mut h` out of the callable child embedding) was refused — it would re-hash the landed A2 `callable:(h,h?,…)->h` grammar (a B4/B7 ABI break). Chosen: the descriptor still embeds each param's FULL annotation identity (reference wrapper included, so `(&int)->r` stays identity-distinct from `(int)->r`); mode is a PARALLEL derivation off the same canonicalizer walk (`TypeAnnotation::Borrow{mutable}` → `SharedBorrow`/`ExclusiveBorrow`, else `Move`), stored in the widened structure, reconstructed at `payload_of` — never from inverting the hash. A string-keyed `param("name")` selector (S2 surface) is a named R1 rejection by design: names are identity-insignificant hygiene aids, preserved in the freeze structure for `param(#name)` positional resolution, never a runtime string lookup key.

**Cost saved:** one memo value carries category + structure end to end (no second table to drift), the callable payload is complete at both the overlay memo and base-index layers with a single derivation each, and mode/name stay off the identity substrate so the A2 canonical grammar is untouched.

## 2026-07-13 — ADR009-B6 S2: public FrozenCallable accessor surface (rejected compromises)

Slice: the ADR-named accessor surface on top of the S1 carrier — `callable.param(I)` (signature-indexed positional), `callable.parameters` (ordered descriptor collection), R1 (string-keyed selection) + R2 (`Array<Any>` params) named rejections, VM+JIT annotation entry proof, LSP completion + signature help.

**1. A `param(I)` / `parameters` accessor as a NEW comptime intrinsic that re-reads the freeze by identity.**
The S1 `FrozenCallable` value already carries the ordered `params` array (each a typed `ParamDescriptor` with value-type identity + optional + `PassingMode`). A tempting shape was a fresh freeze-consuming intrinsic (`__callable_param(type_ref, i)`) that re-resolves the callable through `frozen_type_from_ref` and rebuilds the descriptor — a SECOND producer of the same descriptor beside the reflect payload builder. REJECTED per CLAUDE.md Forbidden Patterns (no parallel producer/consumer carriers): two builders of one descriptor drift. Chosen: the accessor DESUGARS at comptime-prep to the working carrier — `callable.param(I)` → `callable.params[I]` (index access), `callable.parameters` → `callable.params` (the one field). One producer (the reflect payload builder), one carrier, no refcount re-plumbing, no second freeze query.

**2. `param(#name)` faked with a runtime string-key lookup, or `param<I>()` faked with a synthetic const-generic method AST.**
`param(#name)` (hygienic token → position) needs a `#ident` token the grammar does not carry, and `param<I>()` (turbofish const on a method) folds to no `MethodCall` const-arg slot. The defection would be to accept `param("name")` and resolve it as a runtime string key (a name→position map materialized into the value carrier), or to invent a hidden const-generic method call shape. REFUSED: names are identity-INSIGNIFICANT hygiene facts that stay in the freeze structure (never a runtime string key — that is the exact R1 rejection this slice adds), and inventing a parallel method-call const-arg carrier is a surface the AST does not model. Chosen: `param(I)` positional accessor (a plain int index, computed indices accepted) is CURRENT; `param(#name)` is surfaced-and-stopped as grammar-pending (TARGET), with the freeze structure already carrying the names it will resolve against. No fake string selector, no synthetic AST node.

**3. R2 `Array<Any>` params handled by silently coalescing `Any` to `any` (the Erased leaf).**
Capital `Any` (the compiler-internal top type) as a callable param could have been quietly mapped to the enabled lowercase-`any` Erased leaf, "accepting" the signature. REFUSED — that erases the per-position type and is the same defection the B3 `WITNESS_ERASED_TO_ANY_DIAGNOSTIC` refuses. Chosen: a callable param that structurally mentions capital `Any` is the named `CALLABLE_PARAM_ERASED_TO_ANY_DIAGNOSTIC` rejection (reusing the single `annotation_erases_witness_to_any` walk), fired at the freeze boundary before any FrozenCallable forms; lowercase `any` still reaches the Erased leaf untouched.

**Cost saved:** the accessor surface is pure sugar over the one landed carrier (zero new producers, zero new intrinsics, zero refcount surface), the rejection matrix reuses the single Any-erasure predicate, and the grammar-pending hygienic-token spelling is honestly deferred rather than faked with a string key that would contradict the R1 invariant.

**Adjacent fix (not a compromise):** the comptime symbol-rewrite (`rewrite_comptime_type_symbol_args_expr`) did not recurse into the control-flow EXPRESSION forms (`Expr::For`/`While`/`If`/`Loop`), so a reflection/accessor call inside `for p in c.parameters { … }` (or `if implements(...) { … }`) in a comptime block was silently missed. Added the four recursion arms — a general correctness fix for any reflection call nested in a comptime control-flow expression, not B6-specific.

**Review round 1 (2026-07-13) — receiver-blind accessor rewrite (fixed) + reaffirmed grammar deferral.** Review confirmed two findings. (a) The `.param(I)` / `.parameters` desugarings were receiver-BLIND — keyed only on the method/field name, they fired on ANY comptime receiver, so a user struct with a `.parameters` field (silently renamed to `.params`) or a `.param(a, b)` method (mis-reported with the `PARAM_ARITY` diagnostic) was captured. FIXED, not compromised: the rewrite is now gated on a `FrozenCallable` receiver via a lexical `CallableScope` that tracks exactly the identifiers bound by a `FrozenType::Callable(x)` match arm (the sole route a `FrozenCallable` value enters scope in the S1 model). The considered-but-rejected alternative — leaving the rewrite blind "because comptime blocks rarely collide" — is refused per the precise-surface discipline (a name-keyed rewrite that captures unrelated receivers is the same shape as the deleted name-keyed dispatch class). Scope-tracking, not a type-inference pass, because the rewrite runs at comptime-prep before types exist; the guard is sound (no false rewrites) and admits every enabled `c.param(i)` / `c.parameters` form. Evidence: `compiler::comptime::tests::{frozen_callable_accessors_rewrite_inside_a_callable_match_arm, user_receiver_param_and_parameters_are_not_captured, non_frozentype_callable_variant_does_not_open_a_callable_scope}`. (b) The `param<I>()` (turbofish-typed) / `param(#name)` (hygienic-token) spellings and the type-INDEXED `ParamDescriptor<Sig, I, T, Mode>` remain undelivered — reaffirmed as the item-2 deferral above. These are blocked on grammar surface the parser does not carry (`#ident` token; method-call const-arg slot) AND on generic descriptor payloads that are B7 (`Parameter` category) territory; closing them is a dedicated grammar + B7 workstream, not a B6 fix-round change, and faking either (string-key selector / synthetic const-generic method node) is refused per item 2. The deferral is a genuine surface-and-stop (both spellings are hard parse errors, never a silent partial), documented CURRENT (`param(i)`) vs TARGET in the design status row.

## 2026-07-13 — ADR009-B6 S3: LSP signature help + completion driven by the shared reflection catalog (rejected compromises)

Slice: FrozenCallable/ParamDescriptor/PassingMode LSP completion + `reflect(` signature help come from the shared reflection catalog (`reflection_enum_variant_names`, auto-extended by the S1 enabled-payload macro), never a hand-written LSP table; reconcile the LSP `ParamReferenceMode { Shared, Exclusive }` vocabulary with the ADR mode axis `PassingMode { Move, SharedBorrow, ExclusiveBorrow }` established in S1.

**1. A hand-written LSP variant list for the new payload types (`FrozenCallable`, `ParamDescriptor`, `PassingMode`).**
The tempting shape was to add explicit LSP completion arms enumerating the callable payload's variants. REFUSED per the A1 close-out (a second hand-written variant list is the exact defect that close-out deleted). Chosen: the LSP asks `shape_runtime::comptime_reflection::reflection_enum_variant_names`, which is generated from the same catalog constants the compiler consumes. `PassingMode` (an enum) completes its full sealed axis from `PassingMode::ALL`; `FrozenCallable`/`ParamDescriptor` are payload STRUCTS (like `FrozenNever`/`FrozenErased`), so the catalog returns `None` and enum-variant completion falls through cleanly — no fabricated variants, no Unknown arm. Verified by `completion/mod.rs::{test_passing_mode_variant_completion_is_catalog_sourced, test_frozen_callable_and_param_descriptor_have_no_enum_variant_completions}`.

**2. A parallel LSP mode enum that duplicates the ADR axis, or bolting a `Move` variant onto `ParamReferenceMode`.**
`ParamReferenceMode` decorates rendered parameter types with a borrow prefix (`&` / `&mut `). It has exactly two members because a by-value (`Move`) parameter carries NO prefix — Move is the ABSENCE of a reference mode, already modeled as `Option<ParamReferenceMode>::None` throughout the signature-help renderer (`compute_param_ref_modes -> Vec<Option<ParamReferenceMode>>`). Two defections were available: (a) grow a third `ParamReferenceMode::Move` variant that renders to the empty string (a parallel three-mode vocabulary drifting from the shared catalog), or (b) leave the two axes silently unrelated. REFUSED both. Chosen: the LSP axis is reconciled as a provable PROJECTION of the single shared axis — `ParamReferenceMode::from_passing_mode(PassingMode) -> Option<Self>` (`Move -> None`, `SharedBorrow -> Shared`, `ExclusiveBorrow -> Exclusive`) + the total inverse `to_passing_mode`. The reconciliation is guarded against drift by an exhaustive test iterating `PassingMode::ALL` (a new ADR mode forces the projection to update), the same completeness-test discipline every catalog in this codebase uses. No second mode enum is introduced; the ADR axis stays the single source.

**Cost saved:** zero hand-written LSP variant tables (all reflection-enum completion flows through one shared lookup), and the LSP's borrow-only mode axis is a machine-checked projection of the single ADR mode vocabulary rather than an independent enum that could disagree with the freeze descriptor.

## 2026-07-13 — ADR009-B6 S4: docs + design-index status + defections consolidation (rejected compromises)

Slice: documentation-only close (DoD 7 + 8) — flip the `FrozenCallable<Sig>` / `ParamDescriptor<Sig,I,T,Mode>` design-index rows from `CURRENT-partial` to consolidated `CURRENT / VM+JIT`, add the B6 prose section + `Book status:` line, and record the DoD-8 considered-but-rejected compromises. The three ticket-named compromises (composite-memo widening vs a parallel identity-keyed side-table; the string `param()` lookup temptation; `Array<Any>` homogeneous param modeling) were already logged in place at their originating slices — S1 entry items 1/2/3 (composite-memo widening, `Array<Any>` param carrier, mode-in-identity + `param()` string selector), S2 entry items 2/3 (faked `param(#name)`/`param<I>()` spellings, `Any`-coalescing). This S4 entry is the consolidation pointer, not a re-derivation, plus the two S4-specific documentation choices.

**1. Marking the rows fully `CURRENT` while hiding the grammar-blocked spellings, OR downgrading the whole feature to `CURRENT-partial` because two SPELLINGS are pending.**
The hygienic `param(#name)` token and the `param<I>()` turbofish depend on parser grammar the AST does not yet carry (`#ident` token / method-call const-arg slot). Two opposite temptations: (a) flip the row to a clean `CURRENT` and quietly drop the pending-spelling note, overstating the surface; or (b) keep the row `CURRENT-partial` even though the CAPABILITY (positional parameter selection, iteration, R1/R2/R3, VM+JIT proofs, LSP) is complete and only alternate spellings of already-CURRENT `param(I)` selection are missing. REFUSED both. Chosen: `CURRENT / VM+JIT via B6 (S1–S3)` with an explicit inline TARGET tail naming the grammar-pending spellings — the same honest posture the `TraitRef` row uses (`turbofish pending A2`, still `CURRENT / VM+JIT`). The capability is current; a sugar spelling is TARGET, surfaced not hidden.

**2. A parallel B6 book-status tracking doc, or a bespoke "B6 done" marker outside the established pattern.**
Every prior ADR-009 slice records its book readiness with a `Book status:` line inside its prose block in `typed-comptime.md`. The tempting shape was a separate B6 status artifact (or a new field in the index table). REFUSED per the no-parallel-surface discipline — a second status ledger drifts from the per-slice pattern the same way a second descriptor carrier drifts from the memo. Chosen: extend the existing `Book status:` prose convention with one B6 block, pointing at the gate-runnable ShapeTest + LSP evidence and deferring the book-chapter examples to stage F1 like B1/B2/B4/D1/D2. No new tracking surface.

**Cost saved:** the two payload rows carry an honest consolidated status (capability CURRENT, pending spellings explicitly TARGET), the DoD-8 compromises stay logged once at their originating slice with a single consolidation pointer here (no duplicated re-derivation to drift), and book readiness rides the one established `Book status:` convention.

## 2026-07-14 — ADR009-B5 S1: NominalShape sealed algebra + FrozenNominal.shape() discrimination (rejected compromises)

Slice: `reflect(TypeRef<nominal>)` → a fully-populated `FrozenType::Nominal(FrozenNominal)` (catalog ordinal 3, appended to the enabled-payload catalog; no ordinal renumber). `FrozenNominal.shape()` projects the sealed `NominalShape` sum (Struct/Enum/Newtype/Opaque), discriminated by an exhaustive typed `match`. Reconstructed from the struct/enum freeze inputs 1+3, populated in the SAME `rebuild_frozen_type_index` pass that interns the identity (symmetric with `frozen_primitive_payloads` / `frozen_callable_descriptors` — one derivation, no second subsystem).

**1. A parallel identity-keyed nominal side-table that could drift from the base index.** Same drift attractor the B6 close named. Chosen: `frozen_nominal_descriptors: HashMap<FrozenTypeIdentity, NominalDescriptor>` is populated in the one existing rebuild pass beside the other payload maps, and the enriched enum freeze input (`enum_defs: Vec<FrozenEnumVariantDef>` carrying name + payload arity) EXTENDS the single named freeze input 3 rather than adding a second table (module-header discipline). The site-interned APPLIED-nominal case reuses the ONE `composites` memo path (a distinct named rejection arm, not a new carrier).

**2. A `.kind` string / `nominal.is_builtin` / `inspect_native_layout` discriminator (Dec 55 R4/R5).** Refused: nominal shape selection is a typed exhaustive `NominalShape` match, and runtime representation classes (`NativeKind` / heap layout) are not reflection categories. The descriptor schemas carry NO `name`/`kind`/`source` string field (asserted by `builtin_schemas` round-trip); `.shape()` desugars to a typed enum-field read guarded on a provably-`FrozenNominal` receiver (`is_frozen_nominal_receiver`, mirroring the B6 `CallableScope` precision guard — no receiver-blind rewrite that could capture a user `.shape()` method).

**3. A `record.field("name")` string selector / a source-name field on `FieldDescriptor` (Dec 57 R1/R2/R3).** Refused: every field/variant carries an owner-bound HYGIENIC member identity minted from `member:{owner}:{name}` (a distinct identity kind from the field's value type — asserted `!= int_id` in the unit test), NEVER the source-name string. No name-string selection surface exists.

**4. Issuing a StructDescriptor off an un-applied generic head or an unsubstituted applied form (R10/R11).** Refused: a bare generic constructor head (`Array`, arity ≥1, `TypeConstructorRef` / B4 territory) is a named rejection ("un-applied generic type constructor is not a resolved nominal shape"); an APPLIED generic (`Option<int>`) whose substituted member types are not yet threaded is the distinct "requires generic substitution" pending rejection — never a descriptor off the un-substituted form. Full generic substitution (`Page<User>` → `Array<User>` field types via B4 applied identities) is the next B5 slice.

**5. A partial / filtered / misleading-Opaque descriptor (Dec 56 R7/R8, §3.1).** Refused: `NominalDescriptor` has no `Default`/partial constructor; a struct whose field type fails to canonicalize is SKIPPED (no half-populated shape), not issued with an INVALID sentinel. `Opaque` means a genuinely non-decomposable nominal, never a hidden-field filter of a decomposable one. `reflect_repr` authority gating (Dec 56) is greenfield — the S3 slice.

**6. Extending the legacy `__ComptimeFieldDescriptor {name,type,annotations,optional}` name-keyed row.** Refused (E5-deletes territory): the B5 path is the FROZEN typed descriptor family (`\u{1}comptime:` unspellable schemas + spellable mini-VM models), each with its own `runtime_lift_rejection` arm registered in the same commit. The legacy name-keyed row is neither routed through nor grown.

### CURRENT-vs-TARGET partials logged (S1 honest scope, not compromises on the model)

- **Newtype/Opaque classification is field-count-based.** Shape has no dedicated newtype/opaque declaration syntax (grep-confirmed: only `type X { … }` structs, `enum`, and `builtin type X;`). Absent that syntax the compiler must still decide a shape, so S1 classifies a struct by field count: 0 → `Opaque` (no decomposable representation), 1 → `Newtype` (single wrapper), ≥2 → `Struct`. TARGET: dedicated newtype/opaque syntax (or a nominal-kind freeze fact) supersedes the heuristic. The sealed algebra itself is final.
- **`FieldInitialization` is always `Required` today.** The struct freeze input (`struct_types` field order + `struct_generic_info.runtime_field_types`) carries no per-field default flag, so the `Defaulted` variant is modeled + schema-registered but not yet produced (Dec 59: a default is construction policy only; every runtime field always exists). TARGET: thread the field default flag as a freeze-input projection.
- **`VariantDescriptor` carries name-derived member identity + payload arity, not per-variant payload field TYPES.** The enriched `enum_defs` input carries the variant arity (from `EnumVariantInfo.payload_fields`); full per-variant payload field type identities need the env `EnumDef.members` AST source + canonicalization. TARGET: a later B5 slice (paired with generic substitution).
- **`#name` hygienic explicit member selection (`record.field(#name)`) is grammar-blocked (B7).** No `#ident` token exists in `shape.pest` (same block as B6's `param(#name)`). The primary field-selection vehicle is B3 `comptime for some<F,T>` iteration; member identities are already minted and ready. TARGET: B7 grammar + the explicit-selection surface. No name-string path was faked to close it (R1/R3).
- **`reflect_repr` + `RepresentationAccess<T>` (Dec 56 authority) is greenfield.** Ordinary `reflect()` exposes identity + public shape; complete/private representation gating is the S3 slice. Not started in S1.

**Cost saved:** the sealed shape algebra + payload query + dual-engine discrimination land as one vertical over the ONE freeze rebuild and the ONE composite memo, with every deferred surface an honest CURRENT-vs-TARGET partial rather than a partial descriptor or a name-string shim.

## 2026-07-14 — ADR009-B5 S2: member reflection — generic substitution + field/variant iteration (rejected compromises)

Slice: reflecting an APPLIED user struct (`Page<User>`) now substitutes the head's declared type parameters with the applied argument identities BEFORE issuing descriptors, so a field spelled `Array<T>` freezes as `Array<User>` (R10). Derive-style iteration over `record.fields` / `enum.variants` reads the typed `FieldDescriptor` / `VariantDescriptor` rows. Both engines proven (`tools/shape-test/tests/comptime/nominal.rs`).

**1. A second parallel identity-keyed side-table mapping applied identities → head+args (to invert the one-way descriptor hash).** REFUSED per the B6 memo-widening precedent. Chosen: the `applied_nominal: Option<RefinedApplication>` field is a WIDENING of the existing `CompositeMemoEntry` (sibling of the B6 `callable` widening), populated in the ONE `canonicalize_type` interning path from the canonical descriptor via `canonical_refine` — no parallel table, no hash inversion. `payload_of` reads head+args off the widened memo and threads them to `FrozenTypeIndex::substituted_applied_nominal`.

**2. A second canonicalizer / substitution pass that re-derives field type identities off the un-substituted base descriptor.** REFUSED (spec §4.1 no-two-derivations). Chosen: substitution reuses the ONE `canonicalize_with` + `LeafScope` machinery the base `rebuild_frozen_type_index` uses; the only difference is a resolve closure that maps a declared parameter NAME to its applied argument identity (with the argument's frozen category), every other leaf resolving through the base freeze unchanged. So `identity(Page<User>.items)` equals `identity(type_ref(Array<User>))` by construction (asserted in the unit test), not by a parallel derivation that could drift.

**3. The NAME projection of struct type params as a second freeze fact.** `struct_generic_param_names` is added beside `struct_generic_param_kinds` — but both are projections of the SAME freeze input (`struct_generic_info.type_params`), populated in the SAME loop from the one source, not a second independent fact. Names and kinds are two views of one declaration; the substitution path needs names, the arity/kind path needs kinds.

**4. Substituting a builtin/enum applied form (`Option<int>`) off arity alone.** REFUSED — the enum freeze projection carries variant name + payload arity but NOT per-variant payload field TYPES, so there is nothing to substitute into; `substituted_applied_nominal` returns `None` for any head with no `struct_defs` entry and the form stays the named applied-substitution-pending rejection (`reflect_on_applied_enum_generic_is_the_named_pending_rejection`). Never a descriptor issued off an un-substituted or fabricated field set. Full applied-ENUM substitution (per-variant payload field types) is a later slice.

**5. A partial descriptor when a substituted field type fails to canonicalize.** REFUSED (§3.1 R8). Chosen: if ANY substituted field annotation fails to canonicalize, `substituted_applied_nominal` returns `None` (→ the named pending rejection), never a half-populated shape. The field-count classification (0 → Opaque, 1 → Newtype, ≥2 → Struct) is the SAME rule the base rebuild uses, so an applied form and a hypothetical monomorphized declaration agree.

### CURRENT-vs-TARGET partials logged (S2 honest scope)

- **`comptime for some<F,T> field in record.fields` (the existential field-selection vehicle) is NOT wired for descriptor collections.** The B3 `some`-site gate (`require_existential_element`) requires the iterable's element type to canonicalize to the `Existential` category (an `exists<W...> Descriptor<W...>` package annotation). The S1 model types `StructDescriptor.fields` as a plain `Array<FieldDescriptor>`, so the `some` route rejects it as non-existential. Making the collection genuinely existential-typed (a generic `FieldDescriptor<T>` under an `exists<T>` package, with matching schema/inference) is a larger model change than this slice carries. CURRENT vehicle: an ordinary `comptime for field in record.fields` over the typed descriptor array — functionally identical to the `some` unroll (per the B3 design, the `some` executable unroll IS the plain runtime for-loop; the existential adds only the type-level witness opening + escape check). TARGET: existential-typed descriptor collections so `comptime for some<F,T>` opens each field's hidden value-type witness. No name-string field-selection path was faked (R1/R3).
- **`associated_constants` + `constant(#c).value` is un-declarable, hence deferred.** Shape has no associated-const declaration syntax on types/impls (grep-confirmed: `const` appears only as a `let`/statement keyword and as a const-GENERIC parameter `const N: int`, never as `impl T { const X = … }`). Absent a way to DECLARE an associated const there is nothing to iterate or read, and the `constant(#c)` selector rides the same B7-blocked `#ident` token as `#name`. The `AssociatedConstDescriptor` schema + model stay registered + lift-walled (B5 S1) but unpopulated. No test asserts an un-enabled structure (DoD-6). TARGET: associated-const declaration syntax + the `#c` selection surface.
- **The descriptor OWNER of an applied form is the base head identity, not the applied identity.** `substituted_applied_nominal` sets `owner = head` (the base `Page` identity), matching the name-scoped member identities (`member:Page:first`) which are shared across all applications of `Page` (the declared member is the same; only its substituted TYPE differs). TARGET (if a consumer needs it): carry the applied identity as a distinct `applied_owner` alongside the declaration owner.

**Cost saved:** generic substitution + member iteration land over the ONE composite memo (widened, not duplicated) and the ONE canonicalizer, with the deferred `some`-existential vehicle and associated-const surface logged as honest CURRENT-vs-TARGET partials rather than a faked existential typing or a name-string selector.

## 2026-07-14 — ADR009-B5 S3: RepresentationAccess<T> authority + reflect_repr + hard rejection matrix R1-R11 (rejected compromises)

Slice: `reflect_repr(type_ref, access) -> FrozenType<T>` exposes the complete nominal representation ONLY under a compiler-issued `RepresentationAccess<T>` authority, delivered to a declaration-attached annotation type-target expand hook as the third positional `access` parameter (Dec 56 author consent). Ordinary `reflect()` exposes identity + public interface (R7). The hard member-selection rejection matrix (R1 string-field, R2 ordinal, R4 `.kind` string, R5 runtime-repr-class, R6 no-authority, R9 comptime-field, R10 substitution, R11 freeze-boundary) each fires a named diagnostic. Proven VM+JIT (`tools/shape-test/tests/comptime/nominal.rs`) + unit (`type_reflection/tests.rs::carriers::representation_access_*`).

**1. Minting the `RepresentationAccess` carrier in the outer compiler registry and passing the KindedSlot into the handler mini-VM as a module binding.** REFUSED after it FAILED under test (the schema-id-collision family in MEMORY): a carrier minted with `typed_object_for_named_schema` in the outer `current_registry()` stores that registry's `RepresentationAccess` schema id; the mini-VM's schema-name-checked decoder resolves that id against the mini-VM's registry, which had allocated the same counter id to a different schema (`TypeRef`) — so the decode saw `TypeRef` and fired R6 non-deterministically. Chosen: an unspellable `MINT_REPRESENTATION_ACCESS_INTRINSIC` (SOH-prefixed, never registered as a spellable forwarder) whose call the compiler injects DIRECTLY as the handler's third argument (`__comptime__::<mint>(high, low)`), so the carrier is built INSIDE the mini-VM with that registry's native schema id — the decoder reads the same registry. Identity halves are identity-literal transport (the `type_constructor` precedent); the mint re-validates the identity through the shared freeze (`category_of`) so a fabricated identity mints nothing. User code can never call the mint (unspellable) nor obtain a capability any other way.

**2. A backdoor to construct/obtain a `RepresentationAccess` from ordinary comptime code (so the positive test wouldn't need the annotation path).** REFUSED — that would make R6 meaningless (authority would be ambient). The ONLY route into scope is the compiler-injected third handler parameter of a declaration-attached type-target hook; the positive VM+JIT proof runs through a real `@annotation type User {}` whose handler generates a function the user body calls (the `frozen_reflection.rs` precedent). R6 is proven by ordinary comptime calling `reflect_repr(type_ref(User), 0)` — an ordinary value in the authority position — failing at the type-check with the named diagnostic.

**3. Filtering private fields out of ordinary `reflect()` to make the public-vs-complete distinction observable.** NOT DONE — Shape has no field-visibility (`public`/`private`) syntax (grep-confirmed: `struct_field` grammar carries no visibility modifier). Absent visibility, ordinary `reflect()` and `reflect_repr` expose the same field set today; S3 delivers the AUTHORITY GATE (the load-bearing Dec-56 mechanism), not a filtered view. Faking a filter would violate R7 ("ordinary reflection cannot substitute a filtered/partial representation"). Logged as CURRENT-vs-TARGET below.

**4. Reordering the annotation handler parameters to the design's `(target, access, ctx)`.** REFUSED — the landed handler param binding is positional `(target=0, ctx=1)` and reordering would break every existing handler. CURRENT vehicle: `access` is the THIRD positional parameter `(target, ctx, access)`, delivered only when the type identity is present (type-target hooks). The typed-param form `access: RepresentationAccess<T>` and the design ordering are grammar-pending (annotation handler params are untyped positional — `AnnotationHandlerParam` carries only name + is_variadic, same B7 family as `#name`). No parallel handler-param subsystem was added; the third positional slot is typed by the compiler at binding time.

**5. A partial/Default `RepresentationAccess` or a partial descriptor when the authority mismatches.** REFUSED (§3.1 R8) — the carrier has no Default; a mismatched authority (`RepresentationAccess<User>` reflecting `Other`) is the named "bound to a different type identity" rejection, never a partial or cross-type representation. An unknown identity mints nothing.

**6. Adding a `.field(#name)` / `expr.field(receiver, member)` member-SELECTION surface to close R1/R2/R3 with a real selection path.** REFUSED — the `#ident` hygienic-token grammar does not exist (B7 block, shared with B5/B6 `param(#name)`). R1/R2/R3 are closed as NAMED REJECTIONS fired against a tracked `NominalShape::<Variant>(<ident>)`-bound descriptor receiver (`s.field("id")` R1, `s.field(0)` R2, and any descriptor-derived name R3) — never a generic "unknown method" decay, and never a faked name-string selection path. The CURRENT member vehicle stays ordered `fields` / `variants` iteration.

### CURRENT-vs-TARGET partials logged (S3 honest scope)

- **The public-interface-vs-complete-representation field-set distinction collapses until field-visibility syntax lands.** Dec 56 gates the COMPLETE representation behind `reflect_repr(access)` while ordinary `reflect()` exposes the public interface. With no `public`/`private` field syntax (grammar-pending), all fields are public today, so both surfaces expose the same fields; S3 delivers the authority GATE (R6) + the `reflect_repr` surface + the non-filtered-ordinary-reflect invariant (R7, proven by `ordinary_reflect_is_not_a_filtered_representation`). TARGET: field-visibility syntax → `reflect_repr` additionally surfaces private members.
- **`#name` explicit member selection remains grammar-blocked (B7).** Same `#ident` token gap as B5 S2 / B6. The forbidden SELECTION spellings (R1/R2/R3) are named-rejected; the enabling `#name` surface is TARGET. No test asserts the un-enabled `#name` structure (DoD-6).
- **R8 (no partial descriptor) is structural, not a spellable rejection.** `NominalDescriptor` has no Default/partial constructor (B5 S1); every value is fully populated at its single construction point. Asserted structurally via the carrier unit tests (a mismatch/forgery yields a NAMED error, never a half-value), not a user-spellable program.

**Cost saved:** the authority gate lands as an unspellable mint intrinsic injected into the mini-VM (dodging the schema-id-collision family by building the carrier in the reading registry) + a typed third handler parameter, with the visibility-dependent field-set distinction and the `#name` selection surface logged as honest CURRENT-vs-TARGET partials rather than a faked filter or a name-string selector.

## 2026-07-14 — ADR009-B5 S4: LSP nominal-descriptor hover/completion + docs status (rejected compromises)

Slice: the LSP surface for the nominal descriptor algebra. Hover over `reflect_repr` renders the descriptor TYPES it exposes (`FieldDescriptor<Owner, #f, T>` with owner + hygienic member token `#f`, `VariantDescriptor`, `AssociatedConstDescriptor`) and the `FrozenNominal<T>.shape()` entry into the sealed `NominalShape`. `reflect_repr` completes as a comptime-only builtin; the `NominalShape` result axis + `FieldInitialization` disposition complete through the ONE shared `reflection_enum_variant_names` lookup; the descriptor payload STRUCTS fall through it. All from the shared `comptime_reflection` catalog (`REFLECT_REPR_BUILTIN_ROW` spliced verbatim into `builtin_metadata::CORE_BUILTINS`). Design-index rows moved to CURRENT with S4 LSP evidence; Book-status line added. Proven: `tools/shape-test/tests/lsp/typed_comptime.rs::{reflect_repr_hover_renders_descriptor_types_and_owner, comptime_completion_offers_the_reflect_repr_reflection_entry, nominal_shape_result_axis_completes_through_the_shared_reflection_lookup}` + unit `completion/mod.rs::test_nominal_member_descriptor_structs_have_no_enum_variant_completions`.

**1. A hand-written LSP variant/hover table for the B5 descriptor vocabularies.** REFUSED (the exact defect the A1 close-out deleted, re-affirmed by B6). The LSP does NOT own a second list: `reflect_repr` hover/completion flow from the single `REFLECT_REPR_BUILTIN_ROW` in the shared catalog; `NominalShape` / `FieldInitialization` variant completion flows through the ONE `reflection_enum_variant_names(enum_name)` lookup (already B5 S1/S2), and the `FieldDescriptor` / `VariantDescriptor` / `AssociatedConstDescriptor` / `FrozenNominal` payload STRUCTS return `None` from it and fall through cleanly (mirrors B6's `FrozenCallable` / `ParamDescriptor` fall-through). A red LSP test is fixed by enriching the shared row, never by adding an LSP-local table. The hover enrichment was a one-line change to the catalog row's `description`, surfaced by the existing `get_comptime_builtin_hover` renderer — no new hover code path.

**2. Offering `shape` as a bare-name comptime completion (to literally satisfy "completion offers shape()").** REFUSED — `shape()` is a METHOD on a `FrozenNominal` receiver (`nominal.shape()`), not a free builtin; offering it as a bare-name completion would be a fake surface (it is not callable bare, and there is no `shape` in `CORE_BUILTINS`). This is the same shape as B6, which did NOT complete `FrozenCallable`'s `.param()` as a bare name. CURRENT vehicle for the "shape()/reflect_repr surface": `reflect_repr` completes as a real comptime builtin, the `NominalShape` sealed axis (the `.shape()` RESULT) completes through the shared catalog, and the `.shape()` entry METHOD is documented in the `reflect_repr` hover. TARGET: descriptor-receiver method completion (`frozen_nominal.` → `shape`), which needs comptime-binding type inference the LSP does not yet carry — the same method-receiver-completion gap as B6's `.param()`.

**3. Faking a `#name` member-selection completion/hover surface.** REFUSED (R1/R3, same B7 grammar block as B5 S1-S3 / B6 `param(#name)`). No `#ident` token exists in `shape.pest`, so `record.field(#name)` is not spellable. The hover renders the hygienic member position as the `#f` token INSIDE the `FieldDescriptor<Owner, #f, T>` TYPE (documenting the descriptor's owner-bound member identity), never a `record.field("name")` string-selection surface and never a fabricated `#name` completion. CURRENT-vs-TARGET logged below.

**4. Rendering the descriptor's owner/member as a source-NAME string in hover (so the hover looks "friendlier").** REFUSED (Dec 57 R1/R3). The member identity is an owner-bound hygienic token, never a source spelling; the hover renders the TYPE `FieldDescriptor<Owner, #f, T>` (owner nominal + hygienic `#f`), consistent with the freeze's `member:{owner}:{field}` hash carrier — it does not print a raw field name as if it were the member identity.

**5. Extending the legacy `__ComptimeFieldDescriptor {name,type,annotations,optional}` name-keyed row for the LSP field surface.** REFUSED (E5-deletes territory, as in S1-S3): the LSP field-descriptor surface is documented off the FROZEN typed `FieldDescriptor<Owner, #f, T>` type in the catalog row, never routed through or grown from the legacy name-keyed comptime-field row.

### CURRENT-vs-TARGET partials logged (S4 honest scope)

- **`shape()` and member selectors are NOT bare-name completions; they are a receiver-method / documented-entry surface.** `reflect_repr` (free builtin) + the `NominalShape` result axis complete through the catalog; `.shape()` is documented in the `reflect_repr` hover. TARGET: descriptor-receiver method completion once comptime-binding inference reaches the LSP (shared gap with B6 `.param()`).
- **`#name` explicit member selection stays grammar-blocked (B7).** The `#f` token in the rendered `FieldDescriptor<Owner, #f, T>` TYPE is the descriptor's hygienic member position, NOT a spellable `record.field(#name)` call form. Same `#ident` gap as B5 S1-S3 / B6. No test asserts the un-enabled `#name` structure (DoD-6).
- **The public-vs-complete field-set distinction is not LSP-observable until field-visibility syntax lands.** As in S3: with no `public`/`private` syntax, ordinary `reflect()` and `reflect_repr` expose the same fields, so the hover documents the AUTHORITY distinction (Dec 56) rather than a filtered member view.

**Cost saved:** the entire S4 LSP surface is a one-line catalog-row enrichment + a struct-fall-through regression lock, with `shape()`/`#name`/visibility deferrals logged as honest CURRENT-vs-TARGET partials rather than a hand-written LSP table, a faked bare-name `shape` completion, or a name-string member selector.

---

## ADR-009 B7 — the four composite FrozenType payloads (Tuple / Record / Reference / Union), 2026-07-14

Slice: enable the four composite `FrozenType` payloads end-to-end through the ONE query surface (`FreezeOverlay::payload_of` / `FrozenTypeIndex::payload_for_identity`), mirroring the committed B6 `FrozenCallable` and B5 `FrozenNominal` precedents exactly. This slice covers Tuple/Record/Reference/Union only; the `Parameter` payload (A3 base-fn-scoped identity path) is a separate slice.

**1. Adding a second query path (a composite-specific reflection surface) instead of extending the ONE `payload_of` surface.** REFUSED (spec §4.1, R11). Every composite payload flows through the SAME `payload_of` / `payload_for_identity` dispatch as Primitive/Never/Erased/Callable/Nominal; the master switch (`frozen_type_enabled_payload_catalog!` at `comptime_reflection.rs`) auto-derives the injected enum variants, the `Frozen<Category>` payload type names, the register_enum ordinal pins, and the LSP completion list from ONE extended list. No re-implemented classification outside `FrozenTypeCategory`.

**2. Reconstructing composite payloads by inverting the one-way SHA-256 identity hash (or re-parsing the descriptor string as the primary mechanism).** REFUSED (§3.1, no partial descriptors). The structural child identities are THREADED on `CanonicalType` (new `tuple`/`record`/`reference`/`union` fields, exactly as B6 threads `callable`), carried through the freeze's WIDENED `CompositeMemoEntry` (site-interned composites) and per-category BASE-index maps populated in the alias fixpoint (composite aliases) — same widening discipline as `callable`/`applied_nominal`, never a parallel identity-keyed side-table, never a hash inversion.

**3. A `ReferenceMutability { Shared, Exclusive }` aux carrier enum for reference mutability.** REFUSED as unnecessary parallelism. Shape has no declared variance/ownership syntax on references, so the ONLY representable axis is mutability — a typed `bool` `mutable` field (mirroring `RecordFieldDescriptor::optional` and `ParamDescriptor::optional`), never a rendered `.kind` string. Introducing an enum with a single real axis would be a second vocabulary with no additional discrimination; the `bool` is the honest structural form. (Contrast B6 `PassingMode`, which carries THREE genuinely distinct modes off a borrow annotation.)

**4. Deferring `Parameter` into this slice's catch-all as "pending" while claiming the ten-category catalog complete.** NOT DONE — `Parameter` and `Existential` remain the honest named per-category R1 rejections after B7. `Parameter` needs the A3 base-fn-scoped-identity overlay-threading + public-e2e path (a distinct slice); `Existential`'s witness payload is B3-S3. The catch-alls in both dispatch sites shrank to exactly {Parameter, Existential}, never a silent partial.

**5. A hygienic member identity for structural records keyed on a source-name string.** REFUSED (Dec 57). The `RecordField` member identity is minted from the record's OWN frozen identity + the field name (`member:record:{record_hex}:{name}`) — a stable opaque 128-bit token, never the source-name string; two records with the same normalized shape mint the same member identities (the payload is a pure function of the frozen identity).

**Cost saved:** the entire slice is (a) one extended entry in the master switch, (b) descriptor schemas + descriptor structs + heap builders mirroring `frozen_callable_descriptor_slot` + `object_array_slot` (v2-raw `TypedArray` + `KindedSlot` `mem::forget` ownership-transfer — no ValueWord/tag-decode, check-no-dynamic clean), (c) four threaded `CanonicalType` fields + memo/base-map population, and (d) the two dispatch arms — no second query surface, no hash inversion, no aux mutability enum, no source-name member strings.

---

## ADR-009 B7 Slice 2 — the `Parameter` FrozenType payload (A3 base-fn-scoped identity), 2026-07-14

Slice: enable the tenth `FrozenType` payload (`Parameter`), completing the Dec 50/94 ten-category catalog. A generic body reflecting its own declared type parameter now answers `FrozenType::Parameter(FrozenParameter)` off the stable base-fn-scoped `parameter:{owner}:{name}` overlay identity (the public A3 path), through the ONE query surface (`FreezeOverlay::payload_of`). `Existential` (B3-S3 witness iteration) is now the SOLE remaining named per-category R1 rejection.

**1. Adding a second query path (a parameter-specific reflection surface) instead of extending the ONE `payload_of` surface.** REFUSED (spec §4.1, R11). The Parameter payload flows through the SAME `FreezeOverlay::payload_of` dispatch as every other category — the existing scoped-parameter arm (which returned the R1 rejection) now returns a complete `TypeParamDescriptor`. The master switch (`frozen_type_enabled_payload_catalog!`) auto-derives the injected `FrozenType::Parameter` variant, the `FrozenParameter` payload type name, the `register_enum` ordinal pin (2, ABI-stable, no renumber), and the LSP completion list from ONE extended list.

**2. Emitting a `Parameter` payload for an unresolved analyzer TypeVar / inference hole.** REFUSED (Dec 52, §3.1). The payload is issued ONLY off a scoped `parameter:{owner}:{name}` overlay identity — the queried identity itself, declaration-stable across instantiations (owner = BASE fn name, never the mono key). An undeclared name inside a generic body stays the named `unknown semantic type identity` freeze-boundary rejection (`reflect_on_undeclared_name_in_generic_body_is_the_named_rejection`); the analyzer-tyvar inference-hole family stays pinned at the unit level (no source spelling smuggles a tyvar into `type_ref`'s checked type position). Never a fabricated/partial `TypeParamDescriptor`.

**3. Inventing a parallel bound representation to make `bounds` non-empty today.** REFUSED (§3.1, no partial descriptors; would collide with B2). Trait-reference bound descriptors are ticket B2 territory — the SAME element that inhabits `FrozenErased.bounds` (`FrozenErasedBound` is uninhabited until B2). So `FrozenParameterBound` is deliberately uninhabited and the `bounds` array is PROVABLY empty (the exhaustive `match *bound {}` in `frozen_parameter_descriptor_slot` is the structural proof, mirroring `frozen_erased_descriptor_slot`). This is the honest "bounds where representable" form — Shape also has no declared variance/ownership/default syntax on type parameters, so the parameter's stable identity is the COMPLETE representable payload today. Inventing a bound carrier now would either duplicate B2's future representation or lie about a bound set we cannot yet represent.

   **Judgment call (contestable — surfaced for supervisor review):** a type parameter's declared bounds are NOT part of its frozen identity (`parameter:{owner}:{name}` is bound-independent), so a bounded parameter (`fn f<T: Display>`) and an unbounded one both answer a `TypeParamDescriptor` with an empty `bounds` array today. Two dispositions were considered:
   - **(chosen)** emit `{identity, bounds: []}` for ALL scoped params — the plan's literal "provably-empty bounds array" form; the empty array represents "the currently-representable bound set (none)", and B2 inhabits the element. Simplest; needs no bounds threading through the specialization overlay.
   - **(rejected)** mirror `FrozenErased`'s bounded→reject split exactly (unbounded param answers; bounded param rejects with a B2-pending diagnostic). Rejected because (a) it would require threading declared-bound presence through `specialization_type_param_overlay` (which carries only param NAMES on the public per-instantiation A3 path — a larger, cross-file change into `monomorphization/cache.rs` + `function_calls.rs`), and (b) the plan explicitly specifies the empty-bounds form and lists no bounded-rejection test. Logged here so B2 can revisit: when the bound element becomes inhabited, B2 threads the declared `trait_bounds` (`function_defs` type_params + the specialization overlay) into `FreezeOverlay::new` and populates the array.

**4. Threading declared bounds into the overlay ctor as dead scaffolding today.** NOT DONE. The plan noted "the overlay ctor still gets bounds threaded so the machinery is in place for B2." Because the bound ELEMENT is uninhabited, there is no storage target for threaded bounds today, and storing them unused would be dead code. The machinery-in-place is instead the STRUCT field (`TypeParamDescriptor.bounds`) + the SCHEMA field (`FrozenParameter.bounds` array) + the injected-model field — all present and provably-empty. B2 threads the actual bounds when it retypes the element (see disposition 3). Documented deviation from the plan's ctor-threading suggestion, chosen to avoid dead code.

**5. A `TypeParamVariance` / ownership-constraint aux carrier (the ticket DoD lists "defaults, variance, ownership constraints").** REFUSED as unrepresentable-today parallelism (same shape as B7 Slice 1's rejected `ReferenceMutability` enum). Shape source has NO declared variance, ownership-constraint, or default syntax on type parameters, so none of these axes has a source-observable value to reflect. The honest payload is identity + (B2-pending) bounds only; adding empty variance/ownership/default carriers would be fabricated structure with no discriminating content.

**Cost saved:** the entire slice is (a) one extended entry in the master switch, (b) one schema + one descriptor struct + one heap builder mirroring `frozen_erased_descriptor_slot` (v2-raw empty `TypedArray` + `KindedSlot`, no ValueWord/tag-decode, check-no-dynamic clean), (c) one flipped `payload_of` arm (rejection → complete descriptor), and (d) the injected-model struct + ten-way exhaustive-match growth — no second query surface, no inference-hole fabrication, no parallel bound/variance representation.

## ADR-009 B7 Slice 3 — LSP surface for the ten-category payload catalog (rejected compromises), 2026-07-14
Slice: surface the five new B7 payload descriptor type names (`FrozenTuple` / `FrozenRecord` / `FrozenReference` / `FrozenUnion` / `FrozenParameter`) + their element structs (`TupleElement` / `RecordField` / `UnionMember` / `TypeParamDescriptor`) in the LSP, and close the DoD paperwork (design-index TARGET→CURRENT, book status). The LSP surface for the enabled payload catalog is auto-derived: extending the master switch (`frozen_type_enabled_payload_catalog!`) in Slices 1–2 already grew `FrozenType::` completion, the `reflect` hover enumeration, and the `reflection_enum_variant_names` closure — the LSP consumes the ONE query surface (spec §4.1). This slice adds the catalog-completeness LOCKS + docs.

**1. Adding a hand-written LSP completion/hover row for each new payload descriptor type name.** REFUSED (spec §4.1, R11 — the exact defect the A1 close-out deleted). `FrozenType::` completion is a single catalog-keyed lookup (`reflection_enum_variant_names` over `FROZEN_TYPE_ENABLED_PAYLOAD_CATEGORIES`); the reflect hover embeds `FROZEN_TYPE_ENABLED_PAYLOADS_DOC`. Both auto-derive the ten enabled payloads with `Existential` excluded — no per-name LSP edit, no second variant list that could disagree with the freeze. The new tests ASSERT this closure by iterating the shared catalog, so a future ticket enabling another category auto-extends every lock.

**2. Registering a dedicated type-hover / go-to-definition entry for the payload descriptor STRUCT type names (`FrozenTuple`, `TupleElement`, …).** REFUSED as fabricated parallelism. The descriptor structs are payload carriers, not user-declared types with variants; like B5's `FieldDescriptor`/`VariantDescriptor` family and B6's `FrozenCallable`/`ParamDescriptor`, they have NO enum-variant completion — `reflection_enum_variant_names` returns `None`, so completion falls through cleanly (no fabricated `::` variants, no `.field(0)` ordinal member list, no Unknown arm). The LSP treats them exactly as the committed B5/B6 struct payloads; inventing a bespoke hover row would be a second surface with no additional discrimination.

**3. Asserting the LSP closure with a hard-coded ten-name list.** REFUSED — a hard-coded list is a second catalog that drifts. Every Slice-3 assertion derives its expected set from the shared constants (`FROZEN_TYPE_ENABLED_PAYLOAD_CATEGORIES`, `frozen_type_enabled_payload_type_name`), so the lock is a projection of the ONE catalog, not a parallel copy. The element-struct list (`TupleElement`/`RecordField`/`UnionMember`/`TypeParamDescriptor`) is the sole explicit enumeration — these are threaded INSIDE their parent payload's array and have no catalog-category of their own, so they are named directly with a `None`-arm assertion (mirroring the B5 `FieldDescriptor` family lock).

**Carry-forward (unchanged from Slices 1–2, not re-litigated here):** Reference mutability stays a typed `bool` (no `ReferenceMutability` aux enum); the `Parameter` bound set stays provably empty (`FrozenParameterBound` uninhabited) until B2 lands the trait-reference bound descriptors — never an inference hole, never a parallel bound representation that would collide with B2; no variance/ownership/default carriers (Shape has no such declared syntax). See the two B7 entries above.

**Cost saved:** the entire slice is (a) catalog-driven completeness LOCK tests at three layers (LSP completion unit, LSP hover unit, runtime `reflection_enum_variant_names` unit) + one integration reflect-hover twin, all projections of the ONE catalog, and (b) docs paperwork (design-index TARGET→CURRENT, book status) — zero new LSP source rows, zero hand-written variant tables, zero bespoke descriptor-struct hover surface.

## 2026-07-14 — ADR009-B6 residual: `param(#name)` / `param<I>()` hygienic spellings — premise refutation + named tracer (rejected compromises)

Residual close for the B6 DoD's two alternate parameter-selection spellings, deferred at the B6/S2 close as "blocked on B7 grammar work carried later on this same branch." The deferral's stated unblock condition **did not materialize**: B7 (#11) landed the ten-category FrozenType payload catalog (the `FrozenParameter` type-parameter category included) — a payload/schema slice, NOT grammar; B5 (#10) landed the NominalShape sealed algebra — also not grammar. At `adr009/descriptors` HEAD (8f2940d2) `shape.pest` still carries **no general `#ident` selection token** and the `Expr::MethodCall` AST still carries **no const-arg slot**. So both spellings remain genuinely unimplementable within a residual-close scope: each is a real grammar + AST language feature (a dedicated workstream), not a B6 fix-round change. The task's contingency (land if unblocked; else NAMED compile-time rejection via the sanctioned tracer + document) therefore applies — the spellings are named-rejected, not landed.

**1. Faking `param(#name)` resolution now that "B7 has landed".** The tempting read of the residual was that the grammar unblock happened and the token could resolve to a position. REFUSED on evidence: no `#ident` token exists; resolving a faked `#name` would require either a runtime string-key lookup (the exact R1 rejection this feature forbids — names are identity-insignificant hygiene facts) or a hand-minted AST node the parser does not model. Chosen: the general `#name` selection token is parsed by a NAMED tracer (`hash_member_selector` in `shape.pest` → the `Rule::hash_member_selector` arm in `parser/expressions/primary.rs`, mirroring `type_ref_const_generic_reject`) that emits "hygienic member/parameter selection tokens (#name) are not yet a spellable surface (grammar-pending)" and points at the CURRENT positional `param(i)` selection. The tracer produces no AST node (no exhaustive-match ripple) and covers the whole family in one wall: B6 `param(#name)`, B5 `record.field(#name)`, and `constant(#c)`. This upgrades a cryptic raw pest parse error into a precise actionable rejection — the task's "never a silent gap."

**2. Adding a `MethodCall` const-arg slot / bare method turbofish to land `param<I>()`.** REFUSED — two independent blockers, each a language feature. (a) The bare `.param<I>()` angle-bracket turbofish is the universal comparison ambiguity (`a < b > c`): Shape, like Rust, only accepts the disambiguated `::<I>` turbofish, so a bare-form tracer cannot exist without regressing comparison parsing. (b) The disambiguated `.param::<I>()` DOES parse, but `Expr::MethodCall` has no const-arg carrier, so it already lands on the named parse rejection "const generic arguments are only supported on function calls" (postfix builder, `expr` is a `PropertyAccess`, not an `Identifier`) — a named, non-silent wall. Inventing a `MethodCall::const_args` field to carry the index, then const-generic method monomorphization producing `ParamDescriptor<Sig,I,T,Mode>`, is a multi-file AST + type-system feature outside a residual close. Chosen: leave the existing named parse rejection; document the two blockers; both spellings stay TARGET.

**Cost saved:** one grammar production + one parser arm (no AST variant, no exhaustive-match ripple, no new intrinsic, no refcount surface) converts the entire `#name` family from raw parse errors into a single named grammar-pending rejection; the `param<I>()` turbofish rides the already-present named parse wall; and the DoD's alternate spellings are honestly named-rejected-and-deferred rather than faked with a string-key selector (contradicting R1) or a synthetic const-generic method node the AST does not model. The CURRENT vehicles — positional `param(i)` selection (B6) and ordered `fields` / `variants` iteration (B5) — are unchanged and fully proven VM+JIT.

## 2026-07-14 — ADR009-E3 S1 (#19): U10 hygienic generated LOCALS (rejected compromises)

Slice: replace the compiler's synthetic, user-guessable spellings for generated locals / parameters / module bindings (`argN`, `__ann_args`/`__ann_ctx`/`__ann_subject`/`__ann_result`/`__ann_before_result`, `__target_arg__`, `__ctx_arg__`) with a compiler-issued hygienic token (`HygienicSymbol`, minted beside `SymbolId` in `expansion_provenance.rs` via a private-field content-derived ctor). Roles are bound by POSITION/TYPE (`HygienicRole`), never a spelling. `declare_hygienic_local` (helpers.rs) keys the generated slot in the scope name-table by the token's UNSPELLABLE (`\u{1}`-prefixed) descriptor; the comptime mini-program's forwarder params + target/ctx module bindings render the same descriptor so param-decl and reference agree.

**1. Keeping the synthetic spellings "because the comptime mini-program is isolated and cannot collide."** REFUSED — the isolation argument is false at the leak surface. The handler BODY and the user's annotation-argument EXPRESSIONS are spliced into the SAME mini-program namespace where `__target_arg__`/`__ctx_arg__` are module bindings, and the annotation before/after locals are declared in the SAME user-visible scope the annotated expression is compiled in. A user reference to the former spelling captured the compiler-provided binding (rejection-matrix row 2 leak). The negative `u10_former_target_binding_spelling_does_not_leak_into_handler_body` pins this closed: the former spelling is now an unresolved identifier.

**2. Allocating a plain monotonic-counter identity for the generated local (the schema-id collision shape).** REFUSED per CLAUDE.md (counter-allocated identity is the recurring schema-id collision root). The local identity is a content-derived `HygienicSymbol` (128 bits of SHA-256 over the role descriptor + a per-mint nonce, framed like `FrozenTypeIdentity`). The nonce disambiguates two applications of one annotation (distinct non-forgeable tokens — asserted in `hygienic_symbol_mint_is_deterministic_distinct_and_unspellable`) but never keys a symbol table on its own; the token's unspellable descriptor does.

**3. Adding a parallel `Vec<HashMap<HygienicSymbol, u16>>` scope stack alongside `self.locals` for hygienic locals.** NOT DONE — deliberately avoided. The emitted bytecode addresses generated locals by SLOT INDEX, never re-resolving them by token during compilation, so a parallel stack would be dead structure that also had to be kept in sync at all six `self.locals` save/restore/reset sites (helpers push/pop + the two function-body-boundary swaps in `functions.rs`/`functions_annotations.rs`) — a desync hazard for no consumer. Instead the hygienic slot rides the EXISTING scope machinery keyed by the unspellable descriptor: not a user-addressable spelling (the `\u{1}` prefix is grammar-rejected), so rows 1 and 2 both hold, and scope lifetime is managed exactly like any other local with zero new sync points.

**Scope note:** `ctx.original` / `ctx.target` typed capabilities (U11), the `__comptime_handler_fn__`/`__comptime_block__`/`annotation___handler` registry names (U10 handlers/wrappers), and `functions_annotations.rs`'s `__ann_*_wrapper`/`__ann_arg_` chain names are LATER slices (S2–S4) — untouched here. S1 is the generated-LOCALS leaf.

## 2026-07-14 — ADR009-E3 S2 (#19): U10 hygienic generated FUNCTION/HANDLER/WRAPPER names (rejected compromises)

Slice: replace the four synthetic, user-guessable FUNCTION-registry spellings the compiler minted for its generated wrappers/handlers — `__comptime_block__` (`comptime.rs`, `comptime { }` mini-program entry), `__comptime_handler_fn__` (`comptime.rs`, annotation-`comptime`-handler mini-program entry), `__ann_{name}_{before|after}_wrapper_{n}` (`functions_annotations.rs` specialized runtime handler, registered in the outer function table), and `{name}___ann_wrapper` (`functions_foreign.rs` foreign-annotation wrapper slot) — with compiler-issued hygienic tokens. New `HygienicRole` variants (`ComptimeBlockWrapper`, `ComptimeHandlerWrapper`, `SpecializedAnnotationHandler`, `ForeignAnnotationWrapper`) mint through the SAME `HygienicSymbol` surface S1 introduced; the unspellable (`\u{1}`-prefixed) rendering is the registry key.

**1. Routing these wrappers through `GeneratedSymbolTable::reserve_generated_decl` (the SymbolId provenance mint), per the slice's opening phrasing.** REFUSED as the wrong surface for these four sites, on grounds the grounded worktree makes decisive. (a) `__comptime_block__`/`__comptime_handler_fn__` are built by FREE functions (`execute_comptime_with_context`, the handler wrapper builder) that construct an ISOLATED mini-`Program` and never hold `&mut BytecodeCompiler` — the provenance table is unreachable there, and threading it in would be a parallel plumbing path, not the single authority. (b) All four wrappers carry `Span::DUMMY` (or a synthesized handler span) with no real APPLICATION anchor; `reserve_generated_decl` REJECTS a dummy application span (row-1 `SourceAnchor` validation) — so forcing them through it would require fabricating fake anchors, exactly the shape the provenance table exists to forbid. (c) These wrappers are never LSP-navigation targets (nobody "goes to definition" on `__comptime_block__`); they are compiler temporaries at the function level, the exact analogue of S1's generated LOCALS, which also mint `HygienicSymbol` and do NOT enter the provenance table. Chosen: mint via `HygienicSymbol` (the landed S1 hygiene surface), reserving `reserve_generated_decl`/`SymbolId` for the user-visible generated DECLARATIONS (extend methods) that genuinely need provenance. This is not a "parallel mint" — `HygienicSymbol` is the established compiler-temporary identity; `SymbolId` is the user-visible-declaration identity; the split is by whether the symbol is a navigation target.

**2. Keeping the descriptive spellings "because the wrapper name is only cosmetic / referenced by index."** REFUSED. The specialized-annotation handler is looked up by name once (`register_function` -> `find_function` -> handler INDEX); if two wrappers shared a spelling, `register_function` silently keeps the first arity-matching slot and `find_function` returns the WRONG index — a real dispatch bug. The unspellable name is disambiguated by the compiler nonce so distinct wrappers never share a slot, and the mini-program wrappers become unreferenceable from a user's comptime-block/handler body (which becomes the wrapper body). The `annotation_handler_wrapper_anchors_at_the_handler_definition` unit test now asserts NO `*_wrapper` synthetic spelling survives in the function table (rejection row 2), and finds the wrapper by its hygienic marker + handler-line anchor.

**3. Using a fresh monotonic nonce for the mini-program wrappers too (uniformity with the outer-registry sites).** NOT DONE — deliberately fixed at nonce 0. Each mini-`Program` holds exactly ONE such wrapper (one-level comptime nesting, J-CT.2), so a deterministic identity is sufficient, reproducible (good for content-addressing), and lets the wrapper decl and its tail call reuse one rendering trivially. The outer-registry sites (specialized handler, foreign wrapper) DO use the monotonic `self.hygienic_local_nonce` because multiple wrappers coexist in `program.functions` and a shared name would collide (see #2).

**Scope note:** `ctx.original`/`ctx.target` typed capabilities (U11) and the `{name}___impl`/`{name}___{annotation}` + `{name}___before`/`{name}___after` chain names (U11/S4) remain untouched here. S2 is the generated-FUNCTION/HANDLER/WRAPPER leaf.

## 2026-07-14 — ADR009-E3 S3 (#19): U11 typed `ctx.original` replaces the `__original__` name-encoded alias (rejected compromises)

Slice: a `replace body { … }` replacement reaches the PRE-ANNOTATION body through the TYPED `ctx.original` capability, deleting the `__original__{fn}` shadow spelling + the `function_aliases["__original__"]` map + all six of its consumers. The pre-annotation body is compiled into a compiler-issued HYGIENIC shadow function (new `HygienicRole::OriginalBodyShadow`); `ctx.original` is a typed `FrozenCallable` (B6) minted through the single semantic-freeze handle; `ctx.original(args)` is rewritten to a direct typed `FunctionCall` to the hygienic shadow (`original_body_rewrite`).

**1. Keeping `function_aliases` as a small "resolution helper" / "alias-lookup bridge" for `__original__` only.** REFUSED on sight per the CLAUDE.md renames-to-refuse discipline — a name-encoded alias kept "just for the replace-body path" is precisely the shape a deleted class survives under a benign rename. The map is DELETED wholesale (field + init + all six consumers: `mod.rs`, `compiler_impl_initialization.rs`, `functions.rs` alias track + cleanup, `helpers.rs` `find_function` alias branch, `expressions/function_calls.rs` `remote::call` lookup). The typed replacement covers the parity matrix (`executed_extend_authority` VM+JIT: the `ctx.original` invocation row + the two rejection rows) before the deletion, so nothing the alias carried is lost. The `remote::call` consumer only ever saw `__original__` in the map, so after deletion it resolves the reference name directly — no aliasing regression (no test exercised `remote::call(addr, __original__, …)`).

**2. Keeping the deterministic `__original__{fn}` shadow spelling "because it's an internal name, not user-facing."** REFUSED. `__original__{fn}` entered the function table by a guessable string; a user function of that spelling would collide with (or resolve to) the generated shadow. The shadow's registry name is now the unspellable `HygienicSymbol` descriptor (rejection row 2), and its nonce is a stable digest of the annotated function's name so `register_function` dedups it idempotently across recompiles (one shadow per function) instead of a fresh monotonic nonce minting orphan shadows on re-entry. Row 1 (`s3_user_original_spelling_does_not_collide_reaches_body_via_ctx_original`, VM+JIT) proves a user `let __original__x` resolves to the user binding.

**3. Recognizing `ctx.original(...)` at `compile_expr_method_call` (a field-based capability hook) instead of an eager AST rewrite.** REFUSED as unsound for the typed requirement. The MIR-derived type-inference pass runs on the swapped body BEFORE `compile_function_body`; a `MethodCall` on the unbound `ctx` receiver is untyped there, so `ctx.original(5) + 100` would fall into trait dispatch (the exact §4.5.5 Phase-3e failure the shadow's return-type copy exists to avoid). Chosen: rewrite `ctx.original(args)` → `FunctionCall(shadow, args)` at directive-application time (before MIR), so the pre-annotation call is a fully typed function call everywhere downstream. The rewrite is an EXHAUSTIVE AST fold (`original_body_rewrite`, mirroring `substitute_const_in_expr`) — a catch-all that silently skips exotic positions was refused because a missed `ctx.original` would fail as an opaque "undefined ctx" rather than being rewritten; the exhaustive match makes a new AST variant a compile error here (CLAUDE.md Exhaustive Match Rule).

**4. Making `ctx.original`'s type a copied `ConcreteType` only (skipping the FrozenCallable / freeze handle).** REFUSED — that would make `ctx.original` a string/concrete-shaped surface, not a typed capability, and would not exercise the freeze barrier (rejection row 3). The capability's callable identity is canonicalized through `comptime_freeze_overlay` (`build_original_capability`), so requesting it without the single freeze handle is the named `NO_FREEZE_HANDLE_DIAGNOSTIC` compile error (`ctx_original_capability_without_freeze_handle_is_the_named_row3_compile_error`), and a param with no resolvable type / an unfreezable return surfaces the canonicalizer's named error — no partial descriptor. The Phase-3e `ConcreteType` return-type copy onto the shadow is RETAINED (it types the rewritten call's arithmetic), orthogonal to the freeze-minted capability identity.

**Scope note:** the `ctx.target` RUNTIME before/after capability and the `{name}___impl`/`{name}___{annotation}` + `{name}___before`/`{name}___after` chain-name hygiene remain the S4 leaf. S3 is the `ctx.original` (replace-body) leaf.

## 2026-07-14 — ADR009-E3 S4 (#19): U11 typed target-OWNER resolution replaces the magic `"target"` string substitution + chain-name hygiene (rejected compromises)

Slice: a comptime annotation handler's `extend <target> { … }` directive resolves its OWNER head against the handler's POSITION-0 target parameter and a TYPED `TargetOwner` descriptor (name + `NominalShape`, B5), deleting the hardcoded literal `TypeName == "target"` match at both substitution sites (`apply_comptime_extend` inline + the discovery pre-pass inline). The before/after runtime-hook wrapper CHAIN names (`{name}___impl` / `{name}___{annotation}`) become unspellable `HygienicSymbol` descriptors (new `HygienicRole::{AnnotationHookImplBody, AnnotationHookWrapper}`).

**1. Keeping the literal `if name == "target"` match "for the documented `extend target` spelling" — just resolving `ctx.target` alongside it.** REFUSED on sight per the CLAUDE.md renames-to-refuse discipline: a magic keyword kept "just for the one documented path" is exactly how a deleted class survives. The literal match is DELETED at BOTH sites; the placeholder is resolved against the handler's POSITION-0 binding (`resolve_extend_owner_placeholder`, called at the handler-execution site where the handler + its param names are in scope), so `target` is honored only because it is the handler's first parameter name in the book examples — a handler that names its target parameter `t` resolves `extend t`, and a user type literally named `target` resolves NOMINALLY when the first parameter is spelled otherwise. Proven RED-then-GREEN: with the literal semantics restored, `literal_target_type_is_nominal_when_binding_differs` and `extend_head_binds_by_position_not_by_the_word_target` fail (owner hijacks the real `target` type / the `t` binding is ignored); they pass on the position-bound resolution.

**2. Substituting the `extend` head inside `apply_comptime_extend` (leaving the substitution in the shared pass-2 sink).** REFUSED as the wrong seam. `apply_comptime_extend` no longer knows the handler that produced the directive (only the already-resolved `target_name`), so keeping the match there would force the literal `"target"` spelling to remain. Chosen: resolve ONCE, upstream, at the two handler-execution sites (the executed discovery pre-pass AND the pass-2 type-target compile), each with the SAME handler — so both phases produce the identical resolved `extend` and reserve ONE expansion identity per generated method (a disagreement would trip the existing `GENERATED_SYMBOL_CONFLICT` surface-and-stop, never a silent double). The function-target site resolves identically (owner name = the function name, `NominalShape::Opaque`); a v1 function-target extend names an explicit type (`extend Widget`) that does not match the position-0 binding and is untouched.

**3. Retyping the runtime-hook `ctx.target` field (`§4.1.5`, `("target", FieldType::Any)`) as part of the "target" deletion.** NOT DONE, deliberately. The runtime-hook `ctx.target` is a SEPARATE, already-typed capability — the annotated function as a callable value (the same referent `ctx.original` reaches), load-bearing for `@remote`/WF-2C (`ctx.target(...)` is an ordinary typed call, its type supplied by `annotation_ctx_type_annotation`). It is NOT the deleted comptime `"target"` placeholder; the scout's inclusion of that site is over-broad. Touching it (e.g. forcing the slot's `FieldType::Any` to a function field type) risks breaking a shipped capability for zero benefit to the magic-string deletion. Left untouched and documented as the scope boundary. Likewise the `emit_annotation_handler_call` `"fn" | "target"` param-name fill (on_define/metadata legacy handlers) is a distinct spelling-bind subsystem outside the comptime `extend`-placeholder deletion; not in this slice's Files scope.
...
**2026-07-15 — ADR009-C1 rework index:** The canonical generated-only capture plan, exact semantic/cache authority, native scalar/refcounted Shared lifecycle, and descriptor-authoritative tooling are CURRENT on the branch with supervisor execution gates pending. [Full rejected-design and boundary record](design/typed-comptime/c1-capture-defections.md); [current status](design/typed-comptime/c1-generated-captures.md).

**2026-07-14 — ADR009-C1 (#12) declared-capture mode rulings (user):** Ruling 1 — `move` never lies: `move` on a module-level binding is a total `[C0906]` rejection; there is NO `Move -> Shared` arm. Ruling 2 — the fourth mode `share`: `share` is the only spelling for a shared-ownership capture (module binding / `var` / an existing `SharedCell`) -> `CaptureKind::Shared`; `share` on a plain local is `[C0908]`; `&`/`&mut` stay total `[C0902]`. Invariant bought: DECLARED WORD == EMITTED `CaptureKind`, always. CONSIDERED AND REJECTED: "accept the declared word but surface a `lowered != declared` mismatch field" — refused because a mismatch carrier IS the defection; the first C1 attempt validated-then-discarded the declared mode, and rework finding 6 was that this deviation from a posted ruling went unrecorded. [Detail: leaf §9](design/typed-comptime/c1-capture-defections.md).

**2026-07-16 — ADR009-C1 (#12) scope-growth disclosure (user-dispositioned; merged as disclosed C1-necessary collateral):** The C1 delta carries a capability-permission-authentication subsystem (`compiler/import_permissions.rs`, ~415 LOC, plus a rewrite of the blob-permission recording gate in `expressions/function_calls.rs`) and the transactional annotation-import/alias machinery (staged aliases, local-shadow tombstones, per-module graph snapshots). Why: C1's flagship rejection surface is imported-annotation generated bodies, so totality demanded the import machinery. Evidence: piecewise Standards/Spec review cycles in the `AGENTS.md` registry + focused gates (import_permissions 8/8, pipeline 5/5, binding 6/6) + full-battery differential green vs main. [Detail: leaf §10](design/typed-comptime/c1-capture-defections.md).

**2026-07-17 — ADR009-C1 (#12) CHECK-14 branch-new `Ptr(_)` wildcard made exhaustive:** The branch-new `Ptr(_)` wildcard accept-arm in the transferred-closure reconstruction path (`bytecode/closure_layout_fallback.rs`, commit `f1a47eac`) was made EXHAUSTIVE rather than cataloged — cataloging a branch-new wildcard would be gate-dodging. The six non-transferable heap kinds keep erroring, the remaining kinds keep the opaque-pointer mapping, and a new `HeapKind` now fails compile at this arm (`verify-merge` CHECK 14). [Detail: leaf §11](design/typed-comptime/c1-capture-defections.md).

## 2026-07-17 — ADR009-C2 (#13): `CheckedBody` installation validator (rejected compromises)

Slice: the single atomic install transaction + §4.2 validation battery + named rejection matrix (`[C0922]`/`[C0923]`) + existing-body edit transactions (D7, `[C0924]`/`[C0925]`) + VM+JIT install-success proofs, in `crates/shape-vm/src/compiler/checked_body/`.

**1. Carrying generated-body provenance on a new `FunctionDef::generated_origin` AST field (design (a) as surfaced for the `replace body` D6 gap).** SUPERSEDED by moving the D6 async-drop gate to the END of `compile_function_inner`, reading the EFFECTIVE definition, with the origin threaded as a PARAMETER. Two independent reasons: (i) the field rippled to **31 files / 78 struct-literal construction sites** (24 production + 7 test; no `Default`/constructor to absorb it), 3× the ~10-file change budget — surfaced-and-stopped for a supervisor ruling rather than pushed through; (ii) it bought the WRONG thing — the field rides `func_def`, but a `replace body` swap lands on `effective_def` (the clone inside `compile_function_inner`) mid-compile, so a field-on-`func_def` gate would have scanned the PRE-EDIT user body, not the replacement that ships. The effective-def move is both smaller (~4 files) and the correctness fix, and it DELETES the fragile shared `pending_generated_body_origin` field (a parameter can never be stolen by a nested monomorphization compile). Proven by the inverse-control pin pair (`c2_slice4_edit_tests`): a benign pre-edit body with a drop+await REPLACEMENT rejects `[C0922]`; a drop+await pre-edit body with a clean replacement INSTALLS — together proving the gate reads the new body and only the new body.

**2. A staging overlay that mirrors every publication surface and replays on commit (Q1 option (a) / "Option-B" pack rollback).** SUPERSEDED by fixing the ROOT (D8 ruling): a watermark + displaced-entry undo journal that brackets the real `compile_in_place` driver, plus COMPLETING the truncation over the whole function-index-keyed closure-index cluster (the five push-accumulated tables `closure_type_ids` / `closure_function_ids` / `closure_capture_names` / `function_type_ids` / `closure_capture_packs`) as one unit. A parallel staging overlay that replays publications is presumptively a second install path (the CLAUDE.md parallel-implementation attractor); a per-pack rollback that dropped only capture packs would leave the other four cluster members dangling and trip the closure-uniqueness checks on a reused compiler. The cluster is rolled back TOGETHER (batch) or retained together (query-session mode) — never one member spot-patched.

**3. Four separate spot-patches for the four displaced-key hazards (H1 name-keyed side tables, H2 `generated_symbols`, M3 `owned_mutable_locals`, M4 `hoisted_fields`).** DECLINED in favor of ONE journal mechanism. A bounded `remove(key)` per hazard on rollback deletes a SHARED key a prelude/dependency (or an earlier successful install on a reused compiler) put there below the watermark — it does not RESTORE the displaced prior. Four independent remove-by-key patches would each carry that H1/H2-class corruption bug; the journal records every keyed write's displaced prior (present → the value, absent → `None`) BEFORE the write and replays in reverse, subsuming all four hazards in one mechanism whose correctness is argued once. Pinned by the H1/H2/M3/M4 preflight pins.

**Scope note (public surface, not a defection):** an explicit public `CheckedBody<Sig, Captures>` builder type visible to comptime authors was NOT built and is NOT a C2 gap — the ADR's typed builders / semantic edit cursors (`ctx.rewrite.replace_body(quote …)`) are the E-track deliverables (E1/E2, blocked BY C2 per the §6 blocking-edges table). C2 delivers the VALIDATOR; its public surface is the shipped annotation/`extend`/`replace body` pathway now routing through it. Improvising the typed builder in C2 would pre-empt the E-track and delete-U01-U14 sequencing (§6 non-goal). RATIFIED: the public `CheckedBody<Sig,Captures>` builder + `finish()` ship with the E-track (E1/E2, where their consumers exist) per the user-ratified D1 amendment 2026-07-17, tracked on #13/E1.

**Maintainability exception (append-only log over 800 lines):** This historical defection log must grow for the C1 index rather than rewrite or compress earlier entries, because those original bytes are the reviewable chronological record of rejected compromises and stable decision references. Detailed C1 material remains in the focused linked leaf so this required append is bounded.

## 2026-07-21 — ADR009-C3 (#14) S5: C3-G8/G11 deliberate capability withdrawal — generic-target hook installs

Slice: the S5 rejection matrix ([C0926]/[C0931] at S5a; the static G8 arm, the G12 Legacy extension, and the non-function-target rejections at S5b). This entry is the C3-G11 obligation: the G8 rejection lands as a **deliberate capability withdrawal**, recorded here WITH the rejection per the slice-0 supervisor ruling.

**The withdrawn WORKING class (S0-measured, `c3-slice0-report.md` §5).** Installing hooks on a generic target is NOT uniformly broken today — four measured cases WORK through the legacy homogeneous-args weave: **g1** (type-agnostic before/after on `fn id<T>(x: T) -> T`, one concrete instantiation — hooks fire, `42` correct); **g2** (same target called at TWO types — hooks fire per call at both instantiations); **g4** (hooks READING `args[0]`/`result` at two types — correct values at int and string, no silent corruption on the read path); **g5** (args-MUTATING before `[args[0] * 2]`, int-only — prints 42). The C3-G8 rejection withdraws this class for the NEW typed-template path. The class works **by accident of the homogeneous-args representation C3 deletes**: the legacy weave passes hook args as a single homogeneous array whose element type happens to unify for these shapes; the C3 per-param typed carrier (C3-G9 pseudo-tuple) has no such accident to lean on, and a per-specialization install without specialization-origin bookkeeping would silently attach ONE checked specialization to a family of signatures.

**The two failure modes the rejection strictly improves.** (i) **g3**: a generic target with a concrete second param dies PER-MONOMORPHIZATION with the heterogeneous-args hard error, leaking the mangled internal name (`first_of::f64::semantic_0000_c9e2…`) and anchoring at the param — the wrong site, wrong name, wrong count of errors. (ii) **g6**: a mutating hook at two instantiation types dies with a strict-typing array-literal inference error anchored INSIDE the hook body with zero mention of the annotation, the target, or the offending instantiation. The G8 sentence replaces both with ONE named rejection at the `@application` site rendering the USER-SPELLED generic signature (`fn id<T>(x: T) -> T`, never a mangled mono-key — the S0 g3 rule), the #59 re-arm cite, and the concrete-target positive twin.

**The S5b static arm closes the silent half.** S2b left a residual: an UNCALLED generic target with an API-path installing annotation was a SILENT NO-OP (the pre-pass handler run fails before function registration, the `[comptime error]`-filtered swallow defers to pass-2, and pass-2 skips generic body compiles — measured S5b probe P-G8b: `compiles, run=Ok(Some(7)), registry_rows=0`). The S5b static application-site check (`reject_template_engaging_annotation_on_generic_target`) keys on the target's `type_params` plus a conservative SYNTACTIC template-engagement scan (sugar artifacts; install-family names in call-name OR value position across handler bodies + transitive helpers) — no handler-run dependence, both the sugar and API paths, uncalled targets included.

**Lift condition.** #59 (the monomorphization-origin re-arm): when specialization-origin bookkeeping lands, installs re-arm PER SPECIALIZATION ORIGIN — each monomorphized instantiation gets its own Sig-bound, per-spec-checked handler specialization, restoring the g1/g2/g4/g5 capability on the typed path with per-instantiation checking instead of the homogeneous-args accident.

**Positive twin (kept, pinned).** Signature-polymorphic templates stay definable and usable on EVERY CONCRETE target — the polymorphic body instantiates per concrete target (the r-matrix and charter pins); an uncalled CONCRETE target still installs (S5b pin).

**S5 considered-and-rejected compromises:**

**1. Handler-run-dependent G8 only ("the dynamic arms already fire for called targets; document the uncalled case as a limitation").** REFUSED — that is the "document it as out-of-scope" rationalization by another name. The uncalled-generic silent no-op was a MEASURED hole (P-G8b), and a rejection that depends on the handler happening to execute is not surface-and-stop; it is surface-if-lucky. The static arm rejects at the application site with zero execution dependence; the three dynamic sites remain as layered backstops, never the sole line.

**2. Construction-time-only [C0926] (checking ambient references when the template handle is built, skipping the specialize-time gate).** REFUSED at S5a (recorded in the gate's site-rationale comment): construction-time checking is ORDER-SENSITIVE — a module binding registered between construction and handler compile slips through, and the pre-pass fn-lookup-miss defers handler runs to pass-2 by design. Every install passes `specialize_template`, so the gate fires there — totality by construction.

**3. Call-position-only engagement scan.** REFUSED — `let f = before_hook; install(f(t))` was MEASURED silent on a generic target (P-G8d), and the same dodge spells as `let f = install; f(t)`. The scan covers VALUE-position references via the shared collector's marked arm. Within the engagement key, over-approximation is only ever the LOUD G8 rejection (the Lens-2 F4 conservative-set precedent), never a silent accept. NARROWING (measured, disclosed in the slice-5 report): engagement keys on the `install` name ONLY — the sole installer — because a five-name key (constructor names included) rejected the fix-round-1 F5 store-lifecycle baseline pin, whose construct-only handler on a polymorphic template body fn is legal load-bearing machinery; C3-G8 withdraws INSTALLS, not construction. Every spelling of `install` (call, qualified, method, value, helper-transitive) engages.

**4. Registered-table-only helper closure (reusing `collect_authorized_comptime_helpers` alone for the transitive scan).** REFUSED as sole mechanism — in a single-module unit BOTH pre-passes run before function registration (the S2b measured reach), so the registered table is empty exactly where the uncalled-generic hole lives; a registered-only scan would have left the helper-mediated shape (P-G8c) uncovered at the static site. The authoritative closure IS reused, complemented by a scan-only syntactic AST fn table (disclosed in the slice-5 report; never an execution surface).

## 2026-07-21 — ADR009-C3 (#14) S6 A-phase: probe-blocked pin rewrites (rejected compromises)

Slice: the S6 A-phase pin-rewrite wave (typed args-mutation family + the target-validation quartet + the F4/F2 wrapping pins). Two of the 13 S6-ONLY rewrites went RED on their mandatory pre-commit probes; both reds are surfaced findings, and the compromises that would have made them green were REFUSED:

**1. The arithmetic-swap rewrite-around (`before_hook_swaps_arguments`).** The typed exchange spelling (`let t = args[0]; args[0] = args[1]; args[1] = t; args`) is rejected by the S2c aggregate-kind guard — a hoisted LOCAL is outside the provable write-RHS set (the guard's own positive twin: "a literal, another `args[i]` slot, a declared capture, a typed fn's result, or arithmetic over those"), and an exchange has no temp-free spelling. The XOR-style arithmetic swap (`args[0] = args[0] + args[1]; args[1] = args[0] - args[1]; args[0] = args[0] - args[1]`) WOULD pass the guard and the fixture — REFUSED: it is an overflow-unsafe rewrite-around that masks a genuine guard-expressiveness gap (the F5 boundary BLESSES hoisted-local reads, so the read pattern and the write-RHS domain are inconsistent by design). Surfaced for supervisor disposition (candidate fix: teach the guard provable-initializer locals); the pin stays on the legacy spelling as retained coverage.

**2. Pinning the unsound type-changing-after acceptance (`after_hook_wraps_result_in_string`).** The planned F4 rejection-pin conversion (typed `stringify(prefix: string)` + `after(result) { f"{prefix}: {result}" }` on an int target, expecting the per-specialization body-vs-Sig rejection) does NOT reject at HEAD `adb746dd`: the polymorphic `after(result)` sugar path never checks the body's ACTUAL return type against the bound `(R) -> R`, the woven program runs, and the string's HEAP POINTER prints as the int result (measured `97366122325584`; a bare `"xx"` body leaks the same way — not f-string-specific). Converting the pin to `expect_run_ok` on that output — or landing the conversion with any green-making assert — REFUSED: it would pin a soundness hole (heap-pointer-reinterpretation, the exact class the strict-typing program exists to kill). The before-side has its S2c write-kind gate (finding 1 proves it firing); the after-side return gate is MISSING. Surfaced as a high-severity product gap; the F4 dated withdrawal cannot execute until the gate exists; the pin stays on the legacy spelling as retained coverage.

## 2026-07-21 — ADR009-C3 (#14) S6 soundness fixlet: both A-phase findings CLOSED (supervisor-ordered fixes)

Slice: the supervisor-ordered S6 fixlet discharging the two A-phase probe-blocked findings above (fork-independent of the pending @remote ruling). Both landed in one append-only commit; both reverted pins converted to their intended typed forms.

**Finding 2 (soundness — fixed first): the AFTER-side return-kind gate.** `pseudo_tuple::guard_after_template_return_kind` + the seam wrapper `reject_after_template_return_divergence` in `template_specialization/mod.rs::specialize_polymorphic_after` — the S2c analog for `(R) -> R`: at specialization, BEFORE the monomorphization ride (both capture routes, sugar AND API paths), every value-producing exit of the after body (implicit-return tail + every `return`, with `if`/`Conditional`/`match`/block tails decomposed per branch) must PROVE the bound result type under the same bounded evaluator the S2c guard runs. Proven-divergent, unprovable, and value-less-completion exits are all named rejections wrapped through `template_application_error` (the established two-signature application-site attribution). Measured closed: the sugar fixture and the API twin both now reject at compile (previously ran printing the string's heap pointer as the int); `after_hook_wraps_result_in_string` is the rejection pin, the bare-`"xx"` measured fixture is a MUST-REJECT control, and the type-correct twin (same shape, string target) runs green. On the analyzer-visited API path a value-less body still dies EARLIER at definition ("must return a value" — pinned as-is in weave.rs); the gate's own value-less arm backstops the never-analyzer-visited sugar path (pinned in wrapping.rs).

**Finding 1: provable-initializer locals (S2c guard growth).** The candidate fix named in the A-phase entry, executed as ordered: a local whose initializer is provable under the existing rules joins the provable write-RHS set at its binding, transitively (`resolve_provable_locals`, consumed by both guards), scoped to the ONE walker (binding/assignment events collected on the existing rewrite face; a fourth `AfterReturnScan` face reuses the same event chokepoints). Soundness posture: flat name→proof map with sticky poisoning — a name EVER whole-name-assigned, conflictingly rebound, unprovably bound, or colliding with a capture parameter never joins the set (textual walks cannot bound loop re-execution). `before_hook_swaps_arguments` is rewritten onto the typed exchange spelling; the refused arithmetic-swap rewrite-around stays refused.

**Considered and rejected in-fixlet:** (i) poison-is-final on poisoned locals (returning unprovable without fallback) — rejected because it REGRESSED the green rule6 Option-config pin: analyzer-visited API bodies legitimately prove ordinary locals through per-occurrence span-keyed inference facts, so a poisoned name falls back to exactly the pre-fixlet unknown-identifier resolution (facts, then module resolvers, never the capture env) — the provable set only ever GROWS relative to the pre-fixlet guard; (ii) a second standalone walker for the after-body collection — rejected as parallel-implementation (the module's one-walker invariant); the fourth face rides the existing traversal. NAMED BOUNDARY (deliberate, in-code): the after gate proves the TYPE of every visible value-producing exit; exhaustive control-flow reachability (e.g. `loop`-break value paths — rejected conservatively as unprovable) stays with the downstream compile battery, exactly like the before-side guard. PRE-EXISTING class disclosed, not widened: a sugar-body local whose name collides with a module binding could pre-fixlet be blessed through the module resolvers on the fallback path; the locals map strictly NARROWS this (provable locals now resolve correctly before any fallback), and the full closure of that class is module-resolver territory outside this fixlet's order.

## 2026-07-21 — ADR009-C3 (#14) S6 fixlet ROUND 2: F1 `?`-exit arm (both sides) + F2 body-level-only return collection

Slice: the supervisor-ordered round-2 fixlet on the round-1 lens findings (F1 HIGH + F2 LOW in one append-only commit; F3 — the before-side exit gate — stays the next ordered round and is NOT partially landed here beyond the F1 arm the order itself extends to the before-side scan).

**F1 (`?`-exit, both sides — MEASURED before fix).** `Expr::TryOperator` was traversed transparently by both specialization-time scans while compiling to an UNCONDITIONAL early return of the propagated Err carrier (`expressions/advanced.rs::compile_expr_try_operator`). Round-2 probes on the round-1 state: after side — `let x = fallible(0)?; result` in a sugar `after` body on `fn add(a: int, b: int) -> int` RAN, the Err carrier escaped typed as `R` (`print(add(3, 4))` → `Err("boom")`), and the int consumer `add(3, 4) + 1` printed the carrier's pointer bits `102997035238305` (the heap-pointer-reinterpretation class); before side — the same `?` in a `before(args)` body RAN and the Err carrier, consumed as the weave aggregate, silently corrupted the woven call (`1` where `8`). Fix: ONE named surface-and-stop rejection arm at the walker's TryOperator node, firing on `Face::Rewrite` (before) and `Face::AfterReturnScan` (after) in TemplateBody mode with the surface live; explicit-`match` positive twin in the sentence. NAMED BOUNDARIES (deliberate, in-code): (i) closure interiors and comptime blocks stay transparent (`?` there exits the closure frame / the compile-time mini-VM evaluation — never the specialized body); (ii) construction (Validate face) stays permissive — specialization is the total chokepoint (the S2c/after-gate site rationale; pinned as a site-choice test); (iii) on the analyzer-visited API path the shape dies EARLIER at definition ("operator '?' requires the function to return Result or Option" — pinned in weave.rs, the round-1 value-less-body layering); the arm's live line is the never-analyzer-visited sugar path (MUST-REJECT pins both sides in wrapping.rs/injection.rs + the rewrite-face unit pin).

**F2 (body-level-only return collection).** `note_return_value` fired for closure-interior and comptime-block returns (no ScanMode filter) — a closure helper's own non-`R` internal return inside a type-correct after body false-positively rejected the install (MEASURED: the probe rejected with "the template body returns `string`" for a body whose tail proves the bound int). Fix: the collection skips `ScanMode::ClosureInterior` and comptime depth > 0 — body-level returns only. Tail-position closures/comptime blocks still reach the guard as whole-expression `Value` leaves (conservative unprovable → LOUD rejection, never a silent accept). Pinned: the fixed closure-helper case specializes green (sugar suite + API weave twin) and the body-level control (statement-form `return "xx"` inside `if`, non-tail) still rejects.

**Surfaced sibling (NOT fixed — outside the round-2 order, for disposition):** a method def inside an `Extend` statement in a hook body walks its body in TemplateBody mode, so (a) its own `return`s still enter the after-side return collection and (b) a `?` inside it hits the F1 arm — both conservative-LOUD over-rejections of an exotic shape (extend blocks inside hook bodies), never silent accepts; the same one-walker mode plumbing would close both if ruled worth carrying.

## 2026-07-21 — ADR009-C3 (#14) S6 fixlet ROUND 3: F3 — the BEFORE-side exit gate

Slice: the supervisor-ordered round-3 fixlet landing F3 (the before-side family), the last of the three round-1-lens findings. One append-only commit on `adr009/c3`.

**The gap (as ordered).** Before template bodies had NO exit gate — the S2c guard covers per-slot `args[i] = expr` writes only, the sugar carries bodies verbatim with no auto-appended return, and the minted body fn is never analyzer-visited — so a `before(args)` body ending in a stray value or completing value-less specialized, wove, and reached the woven carrier boundary's TYPED read unchecked (the same reinterpretation class as the fixed after-side leak; the `?`-exit third of the family was already closed by the round-2 F1 arm on `Face::Rewrite`).

**The bound before-protocol type (determined from the weave's actual carrier consumption, quoted in the gate's code doc).** `weave.rs`'s before chain binds a non-observer handler's result to a local ANNOTATED with the carrier type and reads it back typed: `MutationCarrier::Single` → the target's one declared parameter type, rebound as the next call's argument; `MutationCarrier::Aggregate` → the inline-schema object over the carrier's field types (the target's AST-side spellings) with per-field `m.a0..m.aN-1` typed reads. The bound type at every value-producing exit is therefore exactly `PseudoTuplePlan::carrier_return_annotation` (C3-G4's typed args-mutation contract, G9-resolved).

**The gate.** `pseudo_tuple::guard_before_template_exit_kinds`, at the same specialization seam as the S2c guard (inside `resolve_pseudo_tuple`, after the write guard, both sugar and API routes — every PolymorphicArgs specialization passes there; rejections classify Hard and wrap through `template_application_error` with both signatures). The rewrite face grows a body-level `returns` collection (the F2 closure-interior/comptime filter applies verbatim; the conforming `return args` / bare-`args`-tail spellings are intercepted+rewritten BEFORE collection — conforming by construction); exits are the shared `ResultLeaf` decomposition (tail + collected returns, per-branch). Proof: `Single` = ConcreteType equality with the declared parameter type; `Aggregate` (which deliberately has NO ConcreteType — inline object annotations are outside `declared_annotation_concrete_type`) = a positionally exact `{a0..aN-1}` object literal proving every field's declared type, minted or user-spelled alike. Divergent, unprovable, and pack-less (value-less completion / bare `return`) exits are named surface-and-stop rejections; a `Void`-proving exit routes to the pack-less sentence. The value-less sentence names the OBSERVER positive twin (the zero-param void form — no pseudo-tuple plan, no carrier, weave calls it with zero args; structurally outside the gate per the S2 observer semantics).

**MEASURED layering (API path, pinned as-is).** Value-less API bodies die at DEFINITION two different ways (`args`-touched: the inference unifies `Args := Void` and the `args[0]` read violates the Void index-access constraint; args-untouched: "must return a value" — both pinned in weave.rs); stray-value exits do NOT die at definition (the analyzer does not check a generic body's return against `Args` — the measured after-side symmetry), so the gate is the live line on BOTH paths for stray values, and on the sugar path for everything.

**Considered and rejected in-round:** (i) a spelling-police gate (only the canonical `return args` / bare-`args` spellings accepted) — rejected: the order's charter is TYPE-proof ("every value-producing exit must prove the bound before-protocol type"), and spelling-policing would withdraw previously-working type-sound exits (`return args[0] * 2` on a 1-ary int target delivered the doubled arg pre-gate — now the pinned positive twin); (ii) a pre-ride seam gate walking the pre-substitution body (mirroring `reject_after_template_return_divergence`'s site) — rejected as parallel-implementation: proving exits containing `args[i]` reads before the rewrite would duplicate the rewrite face's resolution knowledge; the gate rides the ONE walk's collected events instead (the after side has no such walk to ride — its seam site is not a precedent here); (iii) provenance-marking the minted carrier expressions so only rewriter output passes — rejected: acceptance is type-based, and a user-spelled positionally exact aggregate literal proving every field is equivalent to the minted one (per-field proof covers both; the per-field arm is pinned negative on `a1`).

**Mechanical disclosures:** `note_return_value` now fires AFTER the value walk (both Return arms) so the rewrite face collects POST-rewrite spellings — observation-equivalent on the after face, which never mutates; the S2c guard's env-build extracted as `resolve_carrier_proof_env` (shared by both guards; one internal-error sentence reworded from "the aggregate-kind guard" to "the specialization guards", unreachable from user code).

**NAMED BOUNDARIES (deliberate, in-code):** exhaustive control-flow reachability (loop-tail value paths) rejects conservatively as value-less and stays with the downstream battery, exactly like the after gate; a PERMUTED user-spelled aggregate literal (right types, wrong field order) falls to the whole-expr divergent/unprovable arm — conservative-LOUD, never silent. **Surfaced sibling extension (NOT fixed — same disposition row as round 2):** the Extend-statement method-def shape now also feeds the before-side returns collection (its returns/`?` hit this gate too) — still a conservative-LOUD over-rejection of the same exotic shape, never a silent accept.

## 2026-07-21 — ADR009-C3 (#14) S6 fixlet ROUND 4: interpolation-interior exit rejection + before-gate doc fix

Slice: the supervisor-ordered round-4 fixlet closing the round-2 lens's one traced bypass (Finding 1, HIGH) plus the round-3 doc correction (Finding 2, LOW). One append-only commit on `adr009/c3`.

**The bypass (Finding 1 — MEASURED before fix).** An f-string interpolation part is re-parsed and compiled IN THE ENCLOSING FRAME at emission (`string_interpolation.rs:277-295`), but the interior is raw text to the specialization scans (the module-docs F5 boundary; `ambient_scan_fstring` early-returns on the non-ambient faces), so an exit construct inside an interior escaped the round-2 F1 arm AND both exit gates' collections. Round-4 probe on the round-3 state (HEAD `7223fcfb`): traced shape (a) — an `after` body `let x = f"{fallible(0)?}"` then `result` on `fn add(a: int, b: int) -> int` RAN, and `add(3, 4) + 1` printed the Err carrier's pointer bits `95158939846801` (the heap-pointer-reinterpretation class). Fix (the ordered shape, verbatim): on `Face::Rewrite` and `Face::AfterReturnScan` in TemplateBody mode with the surface live, the `Literal::FormattedString` leaf PARSES the interpolation parts (the same parse the ambient face runs) and a SCAN-ONLY pass over the re-parsed temp AST (`Scan::scan_fstring_interior_exits` / `scan_parsed_interior_for_exits`, a pruned `shape_runtime::visitor` search) rejects the two exit constructs — `Expr::TryOperator` and `Expr::Return` (statement form included) — with named surface-and-stop rejections per the F1 sentence conventions + the move-it-out positive twin (hoist the fallible call out, `match`-handle it, interpolate the handled local). MUST-REJECT pins: the three traced shapes ((a) after-side let-initializer in wrapping.rs; (b) before-side let-initializer + (c) per-slot-write RHS `args[0] = f"{fallible(0)?}"` in injection.rs) + rewrite-face unit pins (`?` and `return` interiors); the green twin (match-handled fallible hoisted out, `f"{v}"` on the plain local) is pinned green — verified green on the round-3 state BEFORE the fix landed (no previously-working spelling withdrawn).

**Why scan-only (the lens's explicit warning, honored).** Running the ONE walker over the re-parsed temp AST on the rewrite face would silently DISCARD intercept-Replace mutations (the raw text is what emission re-parses) — the exact quiet-wrongness class this module refuses. The temp AST is therefore searched, never walked by the faces; the F5 no-rewrite boundary stands (interior `args[i]` spellings still reject via the ambient face's F5-boundary arm, never silently resolve).

**Scan boundaries (mirroring F1/F2, in-code):** closure interiors pruned (`?`/`return` there exit the CLOSURE frame — an f-string inside a body-level closure is likewise mode-gated out, its interiors compile in the closure frame); comptime blocks pruned (mini-VM, Dec 65); NESTED f-string literals inside an interior recursed into (their interiors compile in the same enclosing frame); interior parse failures left to the emitter's own error (the ambient face's rule); construction (Validate) face stays permissive — specialization is the total chokepoint (the F1 pinned site choice). **Surfaced sibling extension (NOT fixed — same disposition row as rounds 2/3):** an f-string interior with an exit construct inside an Extend-statement method def in a hook body hits this scan too — still a conservative-LOUD over-rejection of the same exotic shape, never a silent accept.

**Finding 2 (doc-only).** The `guard_before_template_exit_kinds` WHAT-PROVES sentence claimed "a body whose every exit is a mutation-return yields zero leaves" — wrong for the tail spelling: only the `return args` spelling is intercepted before the return collection and yields zero leaves; the rewritten bare-`args` TAIL IS collected as an ordinary tail leaf and PROVES by construction (the minted carrier expression passes the per-field/`Single` arms). Corrected in place; the `Face::Rewrite::returns` field doc already scoped its claim to the returns list and needed no change.

**Considered and rejected in-round:** (i) extending the ONE walker with a fifth "interior" face instead of the visitor search — rejected: a face exists to share the walker's classifiers and rewrites, and the interior must never be rewritten (scan-only is the ordered shape; the visitor prunes exactly the F1/F2 boundaries); (ii) rejecting `?` only (not `return`) — rejected: both constructs compile to the same enclosing-frame exit and the order names both; (iii) treating interior exits as collectable returns (feeding the gates' `returns` lists for type-proof instead of rejecting) — rejected: a mid-string-build exit is not a body exit contract anyone should prove against (the string build is abandoned half-done), and the propagated `?` carrier is never provable anyway — named rejection with the hoist-out twin is strictly clearer.

## 2026-07-21 — ADR009-C3 (#14) S8a: foreign-target hook-annotation rejection — the scope-fence choice

Slice: S8a (the ordered S6 report §6.4 close-out). A typed declarative-hook annotation on a foreign fn (`extern "C"` / dynamic-language) was a MEASURED silent no-op (probe at `26780771`: extern target printed `42` only, "before fired" never appeared; foreign fns never reach `execute_comptime_handlers`, and the typed weave targets ordinary `FunctionDef`s only). Landed: the LOUD named surface-and-stop rejection at the head of `compile_foreign_function`, anchored at the `@application` span, firing on the first application-order annotation whose compiled definition carries declarative hooks (`sugar_post_handler.is_some()` — the same projection of the AnnotationDef handler list the S5b non-function-target seams use). ONE producer (`sugar_lowering::foreign_target_application_rejection`), C3-G13 uncoded message text with the #60 routing note, #68/E4 named in the sentence; must-reject pins (extern + `before(args)` reproducing the §6.4 probe shape; `fn python` + `after(result)`; stacked-first) with exact-sentence + anchored-line asserts, positive twins (annotation-free extern fn compiles; the same annotation on an ordinary fn weaves with a journaled registry row).

**The scope-fence choice (considered and NOT widened).** The S8a probe measured that COMPTIME-ONLY annotations are ALSO silent no-ops on foreign targets (`error("...")` in a `comptime post` handler on an `extern "C"` target never fired — the program compiled and ran; same class for `@json_schema`-style codegen handlers, which would silently emit nothing). Widening the rejection to cover them was REFUSED: the ordered scope is declarative-hook annotations (the forms the S4c sugar lowers), and silently folding a second annotation class into a rejection the order did not name is exactly the undisclosed-scope-move the fence rules exist to stop. The finding + disposition request (options: extend the same named rejection to ALL resolvable annotations on foreign targets; or run comptime handlers for foreign targets as an E4-adjacent workstream; or leave dark with a #68 note) is surfaced in the slice-7/8 report for the supervisor. A scope-fence CONTROL pin (`s8a_scope_fence_comptime_only_annotation_on_extern_c_fn_stays_unrejected`) freezes the rejection's boundary — it pins the SCOPE (comptime-only stays compile-green, un-rejected), not the goodness of the underlying no-op, which remains an open disposition.

**Also considered and rejected:** (i) anchoring at the foreign fn's name span instead of the annotation application span — rejected, the S5b/G12 precedents anchor at the `@application` site (the name-span fallback survives only for a DUMMY annotation span, mirroring `validate_annotation_target_usage`); (ii) rejecting unresolvable annotation names on foreign fns — rejected, outside the ordered scope and inconsistent with the G12 nested-fn precedent (unknown names keep the compiler's existing behavior); (iii) a second sentence producer local to `functions_foreign.rs` — rejected, the rejection-producer home is `sugar_lowering` beside the S5b application-tier producer (one producer per sentence).

## 2026-07-21 — ADR009-C3 (#14) S6 capstone / C3-G14 A′: the dark window as a deliberate cut, and the deletion-direction judgment calls

Slice: the S6 completion arc (`10fcf533` dark window → `cf108a70` collapse → `d2ad9c5f` pure deletion → `193937a4` absence pins), recorded here per the completeness pass ordered for slices 7/8 (the S6 report §§6–7 carry the full ledgers; this entry is the defections-registry projection).

**The A′ cut over incumbency (@remote + @indicator).** The stdlib `@remote` annotation (untyped config + `ctx.target` + the `{result:}` short-circuit — the HookDecision-protocol precursor) and the finance `@indicator` cache hooks (zero-param + on_define + ctx-cached short-circuit) both rode capabilities with NO typed C3 spelling. The compromise candidates — keep the legacy weave alive just for these two consumers, or rush a HookDecision protocol design inside C3 — were both REFUSED under the user principle that legacy earns nothing by incumbency: everything was CUT (C3-G14 A′, user-ratified 2026-07-21). @remote went dark (`std::core/remote.shape` keeps the `remote.execute`/`remote.ping` call API, no annotation block — sentinel-pinned); @indicator's cache hooks went dark with a #68 note while its lifecycle `metadata()` handler survives and now truthfully reports `cacheable: false`. E4 (#20) re-implements @remote on the properly-designed typed HookDecision protocol and closes #68.

**The 21-test `#[ignore]` acceptance-suite disposition.** The distributed e2e tests riding @remote (6 snapshot-polyglot + 1 extern-C-snapshot + 2 composition + 4 matrix + 2 dynamic-snapshot + 3 serve_cmd + the 3-row W9 import trio) were `#[ignore = "dark window: E4 …#68"]`'d, NOT deleted and NOT rewritten onto `remote.execute` — they are E4's acceptance suite, and rewriting them would have destroyed the acceptance evidence E4 needs to prove parity. The alternative (deleting them as dead) was rejected for the same reason.

**The ctx-pin RETIRE-vs-ignore choice.** The S2-F3 E4-blocked ctx pins (`before_hook_passes_ctx_info`, `ctx_target_calls_original_impl_from_after_hook`, `ct_30_annotation_ctx`) were RETIRED (deleted with #68 dark-window comments at their sites) rather than `#[ignore]`'d: their untyped fixtures cannot compile post-collapse and E4's typed HookDecision surface WILL differ — an ignored fixture could never be un-ignored verbatim, so keeping it would fake future value. DISCLOSED asymmetry: the e2e suite above is ignored, the ctx pins are retired; the `#[ignore]` alternative for symmetry remains available if the supervisor wants it (recorded in the S6 report §6.2).

**Two deletion-direction judgment calls (both went DELETE).** (i) The weave's defensive legacy-classification internal error called the deleted `find_compiled_annotations` and guarded a state the collapse made unconstructible — DELETED with its callee rather than rewritten onto a new probe (a rewritten probe would have been dead code wearing a live name — the renamed-fallback shape). (ii) The 9 orphaned legacy `HygienicRole` variants (args/ctx/subject/result/before-result locals, specialized-handler + foreign-wrapper registry labels, both S4 hook-chain identities) — DELETED with their `canonical_descriptor` arms rather than left inert (C3-G7: deletion is cleaner; roles are compile-time name-mint inputs, nothing persisted references them); the two role-exemplar unit tests were rewritten in place onto surviving roles, same names, no coverage lost.

**The W9 coverage-zero finding + its re-target.** Ignoring the W9 import trio left stdlib-annotation-import coverage at ZERO during the dark window — surfaced (not silently accepted) at S6-b with a named recommendation, executed in S8b: one new green row (`scoped_contract_named_stdlib_annotation_import_enables_bare_json_schema`) re-targets the named-import path onto `@json_schema`, restoring coverage without touching the three ignored @remote rows.

**Ledgered surfaced findings (not compromises, tracked):** the C0902 provenance drop (2 `#[ignore]`'d reference-provenance pins — the weave impl shadow drops inferred pass-mode provenance for stamped declared-capture closures; needs its own issue + shadow provenance threading) and the pre-existing `modules_visibility` red (`scoped_contract_snapshot_requires_explicit_import`, proven main-merge-origin by stash differential) — both carried in the slice-6 report §6.6 and the slice-7/8 report.

## 2026-07-21 — ADR009-C3 (#14) S7: JIT-residual honesty — async as a named-expected-fallback, and the aggregate gap split fixed/ticketed

Slice: S7 (stage 1 of the slices-7/8 workflow; commits `d508bad0` + `26780771`).

**Async = named-expected-fallback, not faked native.** The async-hook-target cell (`c3-async-hook-target.shape`, cell 11) pins the CAPABILITY STATE as measured: VM proves BOTH hooks execute on the `async fn` target (600000, value-distinguishing per skipped hook with every refuter derived in the fixture header); `--mode jit` deopts whole-program with EXACTLY ONE loud line naming the pre-existing VM-only `Await` opcode (verbatim in the fixture header), VM==JIT by fall-through, loud-flip semantics (zero lines = async lowering got kinded = the cell FAILS and flips to the zero-fallback form). REJECTED alternatives: skipping the async cell (the S0 fence names it mandatory); asserting only stdout equality without the fallback-count pin (a whole-program fallback also runs the interpreter — output equality alone proves nothing, the vacuous-green shape); rewriting the fixture to avoid `await` in main (would fake nativity by construction). The fixture's `while` driver spelling is a DISCLOSED measured constraint (for-over-range inside an async fn hits the pre-existing `op_iter_done` SURFACE, hook-independent, reproduced by a no-hook control) — recorded in the header, not hidden.

**The aggregate proof-gap family: (a) fixed at the kind tracker, (b) re-pinned + ticketed — never un-suppressed, never demoted.** Gap (a) landed as the CLAUDE.md-sanctioned fix direction (extend the compiler's kind tracker): a bounded TOP-LEVEL `TypeAnnotation::Object` arm in `classify_type_annotation_metadata` stamping `Ptr(TypedObject)`/`Plain` — no anonymous-object `ConcreteType` minted (the B4/B5 fence respected), no new `Convert*` opcode, no classifier lie. The load-bearing proof was the execution differential: cell 5's pinned W36 identity STOPPED firing the moment the arm landed — the loud-flip mechanism working as designed. Gap (b) (MirToIR inline-Object field layout) is real MIR-depth work: per the charter it was NOT attempted in-slice; cell 5 was RE-PINNED to the measured gap-(b) identity (still VM==JIT==400600, exit 0 both, exactly one loud line) and issue #70 filed carrying BOTH measurements verbatim + the flip-to-zero-fallback close condition. REJECTED: un-suppressing anything on the legacy weave (MOOT post-S6 — the weave does not exist; the typed path is the only path and is MIR-attached); compile-failure demotion (a natively-referenced demoted body refuses the whole program at finalize — S0-measured); massaging the fixture to dodge the inline-Object spelling (the user-declared `type` spelling is already natively proven by cells 2/3 — dodging would erase the attribution boundary the pin exists to hold).

**ctx-consuming cells recorded MOOT, not silently dropped.** The S2d parenthetical once listed ctx-consuming hook cells for S7; post-S6/C3-G14 the typed surface carries no ctx — building those cells would half-build E4's charter (#20/#68). Recorded in the module header with the routing, not just omitted.

## 2026-07-21 — ADR009-C3 (#14) S8c+S8d: hover display safety via the one origin producer; book scope bound to annotations.mdx

Slice: S8c (commit `4a8a14b2`) + S8d (shape-web branch `adr009-c3-annotations`, commit `211fcc3`) + this completeness pass.

**Hover SOH suppression through the ONE producer (S8c).** The LSP hover surface renders hook-template identity and signatures from the `hook_install_query()` display-safe projection. REJECTED: rendering the raw minted symbols (the sugar's hygienic `\u{1}`-prefixed body-fn AND type-param mints — MEASURED leaking into the raw declared-Sig rendering at the first machine-pin run, observed RED first); a hand-written parallel LSP table of annotation/template names (the C1 slice-4 shared-query precedent forbids it — every rendered string comes from query-row projections, and the origin phrase is extracted VERBATIM into the shared `template_body_origin` so the S5a [C0926] gate and the hover can never fork wordings); parsing the application view out of the specialized symbol's mono-key segments (S0 g3 rule: never render a mangled mono-key — the view is captured at apply time from the existing C3-G10 attribution renderer with user spellings). Machine pins at BOTH tiers (compiler projection + full hover markdown) with mechanically-exercised planted needles.
