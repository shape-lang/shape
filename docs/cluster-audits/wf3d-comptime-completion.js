...
DESIGN (binding): docs/design/comptime-excellence.md (ratified 2026-07-05; all 12 OQs adopted). Read section 4.1 (introspection contract v1), 4.5 (directive safety + generation, esp 4.5.1 "pass 1.5" whole-program pre-pass and 4.5.7 extend-expr), 4.9 (showcases: 4.9.1 to_json METHOD emission), 7 (acceptance probes P1-P14). Also fix-plan section 0 rules.
RECON FINDINGS (merged main 05612c77, evidence-grounded — trust these as the starting map, re-verify as you go):
  GAP 1 [FLAGSHIP, correctness]: 'extend (expr)'-emitted functions register AFTER user function BODIES are compiled, so they are invisible to fn main() and to any user function — ONLY top-level script statements can call them. Repro: a program with 'fn main() { print(User_json_schema()) }' errors "Undefined function 'User_json_schema'" (both vm+jit, script + project mode), while the same call at top-level works. The section-4.5.1 "pass 1.5" invariant is only HALF-applied (top-level sees generated items; earlier-compiled function bodies do not). The two-pass compiler is "register functions, then compile"; comptime-generated items must land in the function/method table during the pre-pass BEFORE pass-2 compiles user bodies. This breaks the design's flagship showcase form.
  GAP 2 [correctness]: 'type_info(T).fields' absent -> "Property 'fields' does not exist on type 'object'" (want the section-4.1.1 fields Array of FieldDescriptor, OR a CLEAN SURFACE message if the V3-S5 Array-of-TypedObject carrier is genuinely absent — never a generic property error, never the '__type_info_marshal_pending__' sentinel at comptime_builtins.rs:495-532 which is stale dead code to delete). Also bare-identifier 'type_info(User)' fails the OUTER typecheck ("User is not compatible with string"); only 'type_info(\"User\")' works — the rewrite_type_info_ident_args rewrite fires in the comptime mini-VM but not for the outer typecheck of a script-level comptime block. Also 'ParamDescriptor.const' reads false even for a 'const a: int' param (grammar shape.pest:465) — the const flag isn't threaded to the descriptor.
  GAP 3 [ergonomics]: a '--diagnostics json' CLI surface does not exist ("unexpected argument '--diagnostics'"; no DiagnosticsFormat in shape-cli). shape-diagnostics types are already Serialize — this is flag + serializer wiring to emit LSDS JSON (severity, location, comptime_trace). P1's designed observation mechanism + any LSP/MCP consumer needs it.
  GAP 4 [ergonomics, flagship]: method emission not delivered — '@to_json' (stdlib-src/serde/serialize.shape) ships a FREE function 'User_to_json(v)', not the 'u.to_json()' type-extension METHOD section-4.9.1 specified. 'extend (expr)' + apply_comptime_extend_items should emit a method on the type.
HARD RULES:
...
- CLAUDE.md Forbidden Patterns bind. The comptime marshal already deleted its Bool-default — do NOT reintroduce any Bool-default/kind-from-bits/ValueWord/tag-decode under any name. Descriptor rows stay TypedObjectStorage behind HeapValue (ADR-005 section 1). type_info fields, if built, use the same named-concrete-schema mechanism section-4.3 established (never positional/field-set schema ids). No runtime coercion; directives that change signatures re-enter the checker. If a correct fix needs forbidden machinery, STOP and return blocked.
- Diagnostics stay LSDS-routed (ADR-006 section 9). The jargon firewall must keep passing — no internal jargon (ckpt/ADR-/V3-S5/REFUSED/section-sign/phase-2c/W-digit/WF-) may leak into any user-facing string you add.
...
2. Confirm gap 2 sites (type_info fields projection + rewrite_type_info_ident_args outer-typecheck gap + ParamDescriptor.const threading from the AST FunctionParameter const flag). Is the V3-S5 Array-of-TypedObject carrier available for fields, or must fields SURFACE cleanly?
...
TASK — FLAGSHIP (gap 1): make comptime 'extend (expr)'-generated functions/methods visible to user function bodies and fn main(). Complete the section-4.5.1 pass-1.5 invariant: generated items register in the function/method table BEFORE pass-2 compiles user bodies. Preserve the working top-level path, annotation chaining, and directive ordering. Build release; PROVE the flagship showcase form works: a program with 'fn main() { print(User_json_schema()) }' (and a user fn calling a generated fn) prints the correct JSON under BOTH --mode vm and --mode jit, script AND project mode. Commit WIP (git add -A && git commit --no-verify -m 'WF-3D gap1 generated-fn visibility wip').
...
TASK — gap 4 (method emission, flagship ergonomics): make '@to_json' emit a type-extension METHOD so 'u.to_json()' works (per section-4.9.1), not a free 'User_to_json(v)'. Update stdlib-src/serde/serialize.shape + apply_comptime_extend_items as needed to emit a method on the type. Build release; prove a program 'let u = User{...}; print(u.to_json())' prints correct JSON vm+jit (this depends on gap 1 visibility). Keep the derive showcase (section-4.9.1) working. Commit WIP (git commit --no-verify -m 'WF-3D gap4 method emission wip').
...
3. ${DX} just test (pinned pre-existing OK: 4 shape-jit jit_closure_capture_* + pb3 flaky + 2 object-spread ignored; anything new blocks).
