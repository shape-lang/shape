...
| 31 | Reference cycles leak unboundedly | 🏗 **DESIGN RATIFIED + Phases 0/1 MERGED** (`f59e1a4e`): Bacon–Rajan trial-deletion on Arc RC (memory-only, Drop=RAII; non-moving; MT-rendezvous req in v1; snapshot v6→v7 — `docs/design/real-gc-cycle-collection.md` §0). Ph0/1 (`f59e1a4e`) = metadata on v2 HeapHeader (bits 4-6) + shared `for_each_heap_child` edge primitive per carrier. **Ph2 `054a60dd`** = RC barriers + candidate buffer. **Ph3a MERGED `9226c751`** = CollectCycles Bacon-Rajan trial deletion (memory-only, valgrind-verified ×2, no premature-free; header carriers collected, header-less LEAK-safe-deferred) · **Opus-indep**. **§3.5-part1 MERGED `bd82fa6f`** = the REAL 4-node closure-in-array #31 cycle now traced (all 4 edges in single-source `for_each_heap_child`: CALLABLE TypedArray elem, `Arc<HeapValue::ClosureRaw>` value, `TypedClosureHeader` block, Shared-capture `SharedCell` back-edge) and its two DOMINANT v2-header carriers (array buffer A + closure block C) freed memory-only (freed==2). Also fixed a real pre-existing UB (CALLABLE TypedArray freed with 8B layout vs 16B/elem). valgrind 0-err, verify-merge 15/15, 20/20 gc tests · **Opus-indep**. All behind default-off `gc` feature. **Remaining: §3.5-part2 collect the 2 std-Arc residuals B (`Arc<HeapValue::ClosureRaw>`) + D (`Arc<SharedCell>`) — ~112B/iter still leaks, so #31 not yet fully bounded (needs OwnedClosureBlock neuter-hook + SharedCell kind-neuter, design-first) → 3c JIT `inline_typed_field_set` barrier → 3b FULL cross-worker STW rendezvous (owner-ruled) → Ph4 teardown sweep (#82) → Ph5 snapshot v7 + gc-on** | WF-3C / D3 |
