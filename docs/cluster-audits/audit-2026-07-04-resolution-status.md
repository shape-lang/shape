...
| 31 | Reference cycles leak unboundedly | 🏗 **DESIGN RATIFIED + Phases 0/1 MERGED** (`f59e1a4e`): Bacon–Rajan trial-deletion on Arc RC (memory-only, Drop=RAII; non-moving; MT-rendezvous req in v1; snapshot v6→v7 — `docs/design/real-gc-cycle-collection.md` §0). Ph0/1 = GC metadata on v2 HeapHeader (bits 4-6) + single shared `for_each_heap_child` edge primitive per carrier (TypedObject/TypedArray/Closure Drop+visitor share one enumeration; container/interior deferred §3.5), all behind default-off `gc` feature, byte-identical Drop, 7 parity tests · **Opus-indep (code-inspected single-source)**. **Remaining: Ph2 barriers → Ph3 CollectCycles+MT rendezvous → Ph4 teardown sweep (#82) → Ph5 snapshot v7** | WF-3C / D3 |
