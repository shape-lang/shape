...
- **N9 cleanup hotspot** at `type_schema/mod.rs:255-290`: `nb_to_slot` re-decodes a `ValueWord` to a `ValueSlot` via `value.is_heap()` / `value.as_heap_ref()` / `value.raw_bits()` — a tag-decode hop forbidden by CLAUDE.md "Forbidden code". Already flagged for next-session pickup. Phase 1.B should clean this up.
