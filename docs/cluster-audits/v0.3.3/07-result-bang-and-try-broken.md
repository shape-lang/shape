...
(f) **Pre-existing wire_conversion bug surfaced AND FIXED by JOINT-FIX-1a.** At `crates/shape-runtime/src/wire_conversion.rs:198` the `Ptr(HeapKind::TypedObject)` arm cast `bits as *const HeapValue` — but v2-raw TypedObjectStorage (the AnyError carrier per ADR-006 §2.3 + Wave 2 D4 ckpt-2 v2-raw `_new` allocator) is NOT an `Arc<HeapValue>`. Type confusion → SIGSEGV at script-end host projection. Repro at HEAD `31e4afcf` (independent of 1a's `op_error_context` rewrite): `type P { x: int, y: int }; P { x: 1, y: 2 }` (raw TypedObject as script terminal value) — pre-fix 10/10 SEGV. The WS-3 F2b fix at L177-187 / L188-197 added Result/Option special-case projection BEFORE the bad cast, but the analogous TypedObject arm was missed. Folded into JOINT-FIX-1a because 1a's WRAP redesign of `!!` produces AnyError TypedObject inside Result for `Err(...) !! ... ?` at top-level, exposing the bug as a new SEGV inventory item — the canonical pattern from the WS-3 F2b precedent (`if hk == HeapKind::Result { … }; if hk == HeapKind::Option { … }` at L177-197) was extended to `if hk == HeapKind::TypedObject { … }` with schema-driven `WireValue::Object(map)` projection mirroring the `HeapValue::TypedObject(storage)` arm at L270-301. Additional companion fix: `heap_mask` bit guard before recursive `slot_to_wire` on heap-kind fields (`String` / `StringV2` / `DecimalV2` / `Ptr(_)`) — for `build_any_error`-constructed AnyError, fields without values are `ValueSlot::none()` (bits=0) but `field_kinds[i] = NativeKind::String`; reading the null pointer in the recursive `slot_to_wire(0, NativeKind::String, ctx)` arm at L98-104 dereferenced unconditionally before this guard. Post-fix probes: `type P { x: int, y: int }; P { x: 1, y: 2 }` ec=0 5/5; `Err("oc") !! "wc"?` at top-level ec=0 5/5 (projects as `WireValue::Result { ok: false, value: WireValue::Object(AnyError fields) }`). Top-level `?`-on-Err semantics (throw-vs-push) is still owned by JOINT-FIX-1b — the 6 `expect_run_err_contains` tests fail because eval returns Ok(<Err carrier>) instead of erroring; that's a `?` semantics question, not a wire-conversion question. Tracked as resolved here; no follow-up issue spawned.
