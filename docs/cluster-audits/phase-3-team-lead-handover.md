...
**SUPERSEDED 2026-05-17 by v0.2.0 reframe:** user architectural call 2026-05-17 reframed the v1 close ceremony as v0.2.0 (v1 requires user-facing stability + polish still outstanding). The v1 close-summary audit landed (see commit `cdbd1eb7` for the audit deliverable preserved in git history; merge at `264b8629`) and was reframed as `docs/v0.2.0-release-notes.md` (user-facing release notes). v1 tag authorization withdrawn; v0.2.0 tag pending Phase D merge-to-main + Phase E user authorization. Content below preserved as archaeology of the pre-reframe v1-close-seam rotation.
...
**Canonical HEAD:** `bulldozer-strictly-typed @ 50e5c024` — V3-S5 architectural sunset COMPLETE at 9523d57a + V3-S6 retroactive 5-checkpoint multi-session chain MERGED via take-both ceremony. V3-S6 chain ancestors integrated: V3-S6a 43ac9b7a (VM-side resolver Phase B + closure-return-typed generic monomorphization + Vec<> conduit + parametric Array method classifier ~435 LoC) + V3-S6b 48e05f3f (PATH α side-table infrastructure on BytecodeProgram/Program/LinkedProgram + linker threading + population at try_monomorphize_method_call ~370 LoC) + V3-S6c 2544f89f (PATH α-prime JIT routing block at `crates/shape-jit/src/mir_compiler/terminators.rs:176` bypassing jit_call_method trampoline + handle_int_map ckpt3_surface SIGSEGV class +256/-88 LoC) + V3-S6d 2f011e93 (complementary stamping restoration at `crates/shape-vm/src/compiler/helpers.rs` +58/-31 LoC) + V3-S6e d4d5454c (Gap B+C combined fix: local_typed_array_element_types on MirFunction + `crates/shape-vm/src/mir/lowering/stmt.rs::lower_var_decl` producer + `crates/shape-vm/src/compiler/helpers.rs::infer_top_level_concrete_types_from_mir_with_resolvers` consumer ~244 LoC). Total V3-S6 chain ~1500 LoC integrated.
