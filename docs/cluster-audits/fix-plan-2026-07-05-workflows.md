...
- **WF-2B snapshot/resume (green/yellow, committed `wave2/snapshot-resume` 6 commits):** **WF-2F precondition MET** — snapshot()+`--resume` restores state through the TypedObject-field path; **SIGINT interrupt-save exits 130 and resume continues from exact VM state** (no work redone); short-hash resolve; jargon-free barrier messages; all VM==JIT. Residuals:
  - **`snapshot()` → `Result<Snapshot,SnapshotError>` (Q22) shipped, but 7 `ACC__resumability__*` examples still `match` the bare enum → fail-to-compile in BOTH modes → route to WF-4.** The correct `match snapshot() { Ok(Snapshot::Hash(id))=>.., Err(e)=>.. }` form works live; sources at `docs/cluster-audits/v0.3.3-book-acceptance/programs/resumability/` + shape-web `advanced/resumability.mdx` + `stdlib/core/snapshot.mdx` need the Result API + corpus regen.
  - **Heap-element-array serialization gap (Defect 1) → follow-up lane:** arrays of String/Decimal/TypedObject don't round-trip in snapshot (scalar arrays do); correctly refused (ADR-006 §2.7.5.1, no Bool-default), not a silent failure.
  - **Deferred per ratified design:** Q20 (format N-1 read → post-v0.4), Q21 (state::diff/patch → own v0.4 lane), `recompile_and_resume` edited-source relocation (clean-refuse; plain `--resume` runs the snapshot's authoritative code).

- **⚠ GATE INSIGHT (from WF-2B) — `diff-vmjit` MATCH ≠ correctness.** The differential harness scores *fail-identically-in-both-modes* as MATCH, so a feature broken the same way in vm+jit passes it silently (the 7 stale resumability examples are the proof: both modes fail-to-compile → scored MATCH, `unexpected=0`). **The VM/JIT differential gate measures agreement, NOT correctness.** WF-4's book truth-gate (actual pass/fail over the full ~738-fence universe) is the *necessary* correctness gate — `diff-vmjit` cannot substitute for it. See memory `project_book_gate_denominator_trap`.
