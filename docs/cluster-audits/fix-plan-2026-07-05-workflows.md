...
- **WF-1A-followup jit-fallback (verified, finisher committing):** root cause was a `SchemaId`-counter (`TypeSchemaRegistry.next_id`) pollution, not codegen — the `[jit-fallback]` path now runs the already-built inspection bytecode via `execute_compiled` instead of recompiling. Retires the 4-id `jit-object-merge-field-add` class.
