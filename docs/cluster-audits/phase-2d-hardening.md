...
**Status (Phase 3 Wave-1 close 2026-05-12, `W17-jit-legacy-ordinal-disambiguation`):** RESOLVED via option (1) — JIT-internal `HK_*` constants moved to a JIT-private contiguous block starting at `JIT_LEGACY_HK_BASE = 256`, ABOVE the `HeapKind as u16` range. Canonical kinds (10 entries — String / TypedObject / Closure / Decimal / BigInt / DataTable / HashMap / Future / TaskGroup / FilterExpr) stay aliased to `HeapKind::X as u16` (Tier 1); every other `HK_*` (39 entries) sits at `JIT_LEGACY_HK_BASE + N` (Tier 2). `scripts/verify-merge.sh` CHECK 12 (new) enforces the two-tier invariant via an awk grep guard. **Audit amendment** during resolution: the original collision table listed 15 collisions in ords 1..23 plus a stale `HK_FUTURE_..` entry referencing constants (17/18) that don't exist in the file. After Phase 2d Wave 14/15/17 added HeapKind ords 23..33, **10 additional `HK_*` entries collide** beyond the original table: HK_TIMESPAN=24↔Channel, HK_TIMEFRAME=25↔PriorityQueue, HK_TIME_REFERENCE=26↔Range, HK_DATETIME_EXPR=27↔Result, HK_DATA_DATETIME_REF=28↔Option, HK_TYPE_ANNOTATION=29↔TraitObject, HK_TYPE_ANNOTATED_VALUE=30↔Mutex, HK_PRINT_RESULT=31↔Atomic, HK_SIMULATION_CALL=32↔Lazy, HK_FUNCTION_REF=33↔ModuleFn. Plus HK_DATA_REFERENCE=34 (one above the current HeapKind tail; bumped pre-emptively) and the entire array-family 48-71 (bumped to preserve the "JIT-private out-of-range" invariant against future HeapKind growth). All 39 JIT-private entries are now contiguous at JIT_LEGACY_HK_BASE..JIT_LEGACY_HK_BASE+38 (256..294). Producer / consumer sites continue to reference constants by symbolic name; the value change is transparent to them. verify-merge.sh: 12/12 PASS. AGENTS.md row appended. — original finding text retained below for historical record. —
