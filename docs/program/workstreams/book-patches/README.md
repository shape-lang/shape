...
| `187-per-function-fallback.patch` | PERF-DEOPT-GRANULARITY (#187) | `advanced/jit-compilation.mdx` — a new Per-function-fallback section (the constructs that now cost only their own function, that calling an interpreted function from native code costs only the trampoline hop, the two residuals that still cost the program with the producer/consumer reason for each, and toplevel as the remaining whole-program case), plus two corrections the measurement forced: the `--mode jit` fallback bullet no longer says a compile failure costs the whole program unconditionally, and the Tiered Compilation section now opens with the fact that its machinery is implemented but reaches nothing at runtime — the tier manager's only writer was deleted by 3e26c02a, so `--mode jit` is ahead-of-time whole-program compilation with no call-count promotion. The Performance Characteristics promotion timeline is relabelled as design. The one new fence is `runnable` and was executed under both `--mode vm` and `--mode jit` with identical output (`Ok(5)` / `12.0`) (generated against `2fe2d2a`) |
