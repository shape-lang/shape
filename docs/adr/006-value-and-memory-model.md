...
**2.7.32.4 Decision — disposal is the last share, synchronously, exactly once.** There is no explicit close, so there is no second path that could dispose twice; `Arc` supplies the exactly-once guarantee and the four-table lockstep supplies the correct share count. Per ADR-019 §3 the disposer is synchronous and infallible in v1 — a disposer that could fail or suspend is a later design under ADR-010 §6's finalization contract, not a v1 surface. In ADR-010 §6 terms the foreign dispose is *Finalization* and the `Arc` retirement is the mandatory *Carrier Release*; they coincide here because v1 finalization cannot fail.
