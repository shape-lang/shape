...
| HashMap | NEW arm (Wave N hashmap-value-v-arm follow-up, cluster-2 closure-wave-C, 2026-05-16) | per-V monomorphization in `HashMapData<V>` + recursive `HashMapKindedRef` values pointer (V = HashMapKindedRef itself; `TypedArray<HashMapKindedRef>` values pointer; HashMapValueElem impl walks-with-ptr::read on release, delegates to HashMapKindedRef::clone on share, lets scope-end run auto-derived Drop on release_owned). Unblocks `HashMap.groupBy` (was surface-and-stop'd at `hashmap_methods.rs:1735`). |
...
**2.7.30.8 Sequencing — gate-2 premise RETRACTED (AMENDED 2026-05-29, run-verified).** The original draft made widening `c6-binop-ref` a standalone "land FIRST" prerequisite, on the claim that a reference-typed-operand binop (`make()+1` where `make()->&int`) is a live `EXIT=139` segfault at HEAD. **That premise is run-verified FALSE** (supervisor + agent, 2026-05-29): reference-typed parameters and return types **do not parse at HEAD** (`fn f(r: &int)` and `-> &int` are parse errors — confirmed for both `&int` and `&i64`), so `make()->&int` is inexpressible; the only ref-producing forms — syntactic `&x` / `&p.x` / `&a[i]` as a binop operand, and `let r = &x; r op y` — are ALREADY caught (the existing syntactic c6 reject at `binary_ops.rs:906-928`, and strict-typing inference, respectively). There is no `EXIT=139` to reproduce at HEAD. The c6-widen guard's target (reference-typed operands) is **created by the flip** — specifically the §2.7.30.9 grammar change — so the guard CO-LANDS with the flip, NOT before it.

**2.7.30.9 Narrow-floor build is a single co-landing change (ADDED 2026-05-29).** After ADR apply, the narrow floor lands as ONE coordinated change (no standalone prerequisite):

1. **Grammar:** make `&T` parse in parameter and return-type position (it does not today — the gating fact behind §2.7.30.8's retraction). This makes the `ReturnSlot` flip expressible AND creates the ref-typed-operand surface the c6 guard needs.
2. **Borrow-flip:** escape→RC promotion for `ReturnSlot` + `ModuleBinding` via the §2.7.30.2 `PromotedCell` carrier.
3. **Widened c6 guard:** co-lands HERE (not before) — reject reference-typed operands in binops (now expressible), plus a test proving it fires on `make()+1` where `make()->&int`.
4. **Carrier + deferred-Drop + identity-handle replay-only snapshot** per §2.7.30.2 / .4 / .5 (shared `restore_identity_map`).

**Discipline:** the negative type-safety suite must show the widened guard *reject* the ref-typed-operand-in-binop shape AFTER the grammar change makes it expressible (red→green for that specific shape). VM==JIT lockstep. **Investigate (don't assert):** once the strict flip lands, re-check whether the audit's `f(&a)+&a` `EXIT=139` auto-closed (the ReliableOnly-suppression hypothesis) — if so, `c6-binop-ref` as a separate cluster evaporates.

---

#### 2.7.31 Content-derived schema identity (WF-3A M1, in-process) — RATIFIED 2026-07-06

**Status:** RATIFIED (user 2026-07-06). Scope: **M1 only** — the in-process root-fix. Design corpus: `docs/design/schema-identity-structural.md`. Cross-note to ADR-005 §1 (single-discriminator). M2 (wire content-id carrier + snapshot content-id + cross-node blob determinism) is a separate lane and is NOT authorized by this amendment. Code touchpoints carry `// ADR-006 §2.7.31` / `WF-3A` markers.

**2.7.31.1 Context.** `SchemaId = u32` was a monotonic counter, minted by **two independent counters** (per-registry `TypeTracker.allocate_id`, and the process-ambient `allocate_current_id` → `NEXT_SCHEMA_ID`-family) that both fed the **same** `by_id` map. Independent counters seeded at 1 over one shared keyspace collide silently (`by_id.insert` overwrite), so identity depended on global registration history: an unrelated `Snapshot`-enum registration shifted every downstream id and flipped the object-spread test red at the Wave-2 merge. Identity was temporal/ordinal, never a function of content. Four point-patches accreted to suppress the symptom (`ensure_next_id_above`, the `merge()` id-collision reallocation loop, the `reserved` inference-guard, the WF-2E `resolve_typed_object_schema` runtime-arity heuristic) plus two name-based dedup caches — the recurring band-aid family diagnosed in the design doc §5-6.

**2.7.31.2 Decision — two layers: canonical content id + derived intern handle.** Identity is a function of schema **structure**, minted at exactly one place:

- **`SchemaContentId([u8;32])`** (`type_schema/schema.rs`) — the canonical identity, a SHA-256 over the schema's structure. Single source of truth.
...
- Fields hash in **DECLARATION order** (layout-significant — Shape's TypedStruct has C-compatible fixed offsets), plus enum variants in declaration order. (Prior `compute_content_hash` sorted by name — corrected to declaration order for layout identity.)

**2.7.31.4 Decision — counter path + point-patches DELETED (root cutover).** Keeping any old-id-resolution or arity-heuristic fallback beside the new path is the forbidden `ensure_next_id_above`-reborn pattern. Deleted, not gated: both counters' identity role (`allocate_current_id` / `NEXT_SCHEMA_ID`-family; `allocate_id` demoted to intern-index allocation only); `ensure_next_id_above` + `ensure_next_schema_id_above` + all 5 callers; the `merge()` id-collision reallocation loop; the `reserved` flag; the WF-2E `resolve_typed_object_schema` runtime-arity heuristic (collapsed to a single `get_by_id`); the `__inline_obj_N` by-(name,type) and `__merged_L_R` by-name dedup caches (subsumed by content-interning). `merge()` still returns an `id_remap` (old→final handle) — this is legitimate **cross-registry handle translation** (registry B's local handles become registry A's on merge; applied to bytecode operands at `program_impl.rs`), NOT a collision disambiguator: there is no counter-bump or reallocation loop.

**2.7.31.5 Consequence — blob hashes change once (a determinism WIN).** `FunctionBlob` content hashes embed schema-id operands, so they change under the new mint (one-time content-addressed cache invalidation on rollout). This is EXPECTED and BETTER: handles become deterministic across registration order, a precondition for cross-node blob dedup (fully realized in M2). Blob-hash operand→content-id substitution is M2, not done here.

**2.7.31.6 Forbidden (per CLAUDE.md §Forbidden).** NO blind-counter allocation of schema identity, and NO order-dependent disambiguator (`ensure_next_id_above`, arity heuristic, id-collision reallocation loop) re-added under any name — re-introducing one is a defection on a par with the W-series `ValueWord` renames. A 256-bit SHA-256 collision is treated as cryptographically unreachable (same assumption as `FunctionBlob.content_hash`); on a genuine hash collision, surface-and-stop — NO probabilistic/rehash fallback (that reintroduces order-dependence). The `u32` handle must stay a derived intern index of the one `SchemaContentId`, never a parallel discriminator. NO `ValueWord`/tag-decode/Bool-default. M2 must not be blocked by an M1 shim (the wire `Some(1)` placeholder + raw-u32 snapshot are left as-is, unshimmed).

---

#### 2.7.32 The opaque foreign-reference carrier — `HeapKind::ForeignRef` (Q26; ADR-019 §3 / POLY-FOREIGN-REF #200, 2026-07-28)

**Status:** amendment. Authority: ADR-019 §3 (ratified design), R25, the 2026-07-05 design ratification that green-lit the carrier. Composes with ADR-010 (teardown), ADR-014 §1 (`Ffi`), ADR-015 §8 (refusal discipline). Code touchpoints carry `// ADR-006 §2.7.32` markers.

**2.7.32.1 Context.** Shape could not hold a foreign object at all. Every value crossing the `fn python` / `fn typescript` boundary was copied through MessagePack, so an object with identity — an open database connection, a loaded model, a module — had no representation on the Shape side. ADR-019 §3 makes it one: an opaque, refcounted handle whose lifetime Shape manages with ordinary drop discipline.

**2.7.32.2 Decision — a pure-discriminator `HeapKind` with a typed `Arc` payload.** `HeapKind::ForeignRef = 36`. Slot bits are `Arc::into_raw(Arc<ForeignRefData>) as u64`, labelled `NativeKind::Ptr(HeapKind::ForeignRef)`. This is the §2.7.9 `FilterExpr` shape: **no `HeapValue` arm**, `as_heap_value()` undefined on these bits, retain/release exclusively through `Arc::{increment,decrement}_strong_count::<ForeignRefData>` in the kind-dispatch tables. The absence of a `HeapValue` arm is not an omission to be tidied later: there is nothing a Shape-side pattern match could observe (the representation lives in another runtime), and adding one would open a `HeapValue`-shaped clone path around the share accounting that disposal depends on. `crates/shape-vm/src/executor/tests/no_dynamic.rs::foreign_ref_stays_a_pure_discriminator_kind` pins this, because Rust cannot express "this variant must not exist".

**2.7.32.3 Decision — the payload binds handle, origin, and disposal authority.** `ForeignRefData { handle: u64, origin: ForeignRefOrigin, disposer: Arc<dyn ForeignRefDisposer> }` (`crates/shape-value/src/foreign_ref.rs`).

- **Handle.** The extension's own identifier, opaque to Shape: never interpreted, never compared across languages, never serialized.
- **Origin** (`language`, `foreign_type`, `produced_by`). Carried on the value rather than reconstructed from context, because the sites that must refuse it — snapshot serialization, JSON projection, remote artifact construction — run with no VM or extension in scope, and ADR-019 §3 requires those refusals to name the value and its origin.
- **Disposer.** `shape-value` sits below the plugin layer and must not learn to reach a `PluginLanguageRuntime`, yet drop runs *here*, in the kind-dispatch tables. So the layer that can reach the owning instance injects the authority at construction and `Drop` calls it. This is also what makes thread-affine disposal expressible without `shape-value` knowing that thread-affinity exists.

**2.7.32.4 Decision — disposal is the last share, exactly once, non-suspending.** There is no explicit close, so there is no second path that could dispose twice; `Arc` supplies the exactly-once guarantee and the four-table lockstep supplies the correct share count. Per ADR-019 §3 the disposer is synchronous and infallible in v1 — it never awaits, never retries, never declines, and a disposer that could fail or suspend is a later design under ADR-010 §6's finalization contract. In ADR-010 §6 terms the foreign dispose is *Finalization* and the `Arc` retirement is the mandatory *Carrier Release*; they coincide here because v1 finalization cannot fail.

Where the owning instance is bound to another thread — a dedicated async worker owning a V8 isolate — the release necessarily runs on that thread, because it is the only one permitted to enter the runtime. The drop point hands the request to that thread and does not wait for it: waiting would block a Shape scope exit for the length of whatever unrelated foreign call the worker is inside, turning an ordinary drop into an unbounded pause, which is a worse contract than a deferral bounded by the worker's queue. The deferral owes delivery, and gets it: the request is queued on the worker's own command channel, and `std::sync::mpsc` delivers everything already queued before a receiver reports disconnection, so a worker shutting down still drains it. Disposal is therefore ordered against that instance's other work rather than racing it.

**2.7.32.5 Lockstep.** All four tables (§2.7.7 stack track, §2.7.6/Q8 `KindedSlot`, §2.7.8/Q10 `SharedCell`, `TypedObjectStorage` field release) plus the JIT's fifth and sixth tables (`mir_compiler/ownership.rs` retain/release arms, `ffi/v2/collection_arc.rs` bodies) carry an explicit arm. The JIT arm is not speculative bookkeeping: a `HeapKind` with no explicit arm falls through to the legacy `UnifiedValue<T>` HeapHeader carrier whose refcount is at offset +4, while a `ForeignRefData` share lives in a Rust `Arc` control block at offset -16 — the exact mismatch behind three documented segfault families. A JIT-compiled function never produces a foreign reference (functions containing `CallForeign` are interpreter-only), but one can flow through JIT code as an ordinary value.

**2.7.32.6 Forbidden.** No `HeapValue::ForeignRef` arm under any name. No `as_heap_value()` on ForeignRef-labelled bits. No serialization of the handle into a snapshot, a wire value, or a portable artifact — refusal with provenance is the only honest behaviour (ADR-019 §3; ADR-015 §8). No silent skip of a foreign reference during snapshot capture. No second disposal path beside `Drop`, and no fallible or suspending disposer in v1. No inference of disposal routing from the language id — the owning instance is recorded on the carrier, because deciding it from a name would be a terminal-name switch selecting a capability.
