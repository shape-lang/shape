...
Shape's `@` sigil is reserved for **annotations** — *data attached to a function describing a verified property*. `@ai` means "this function's body is LLM-generated structured output, validated by the JSON Schema derived from the return type". `@description("...")` attaches documentation. `@law` (RFC-002) attaches an executable property test. `@intent` (RFC-007) attaches natural-language intent. Annotations participate in `@annotation name { @before { ... } @after { ... } @comptime { ... } }` — they can carry runtime hooks (`crates/shape-ast/src/ast/functions.rs:226` `AnnotationDef`, grammar at `crates/shape-ast/src/shape.pest:345-358`).
