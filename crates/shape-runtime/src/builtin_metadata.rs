...
/// Unspellable inference-only marker for a compiler-resolved type-syntax
/// argument. It is never a runtime Shape value or a public type name.
pub const COMPTIME_TYPE_SYNTAX_MARKER: &str = "\u{1}comptime:type-syntax";
...
static CORE_BUILTINS: &[BuiltinMetadata] = &[
    ...
    // (`crate::comptime_reflection`), the same module whose
    // `FrozenTypeCategory::ALL` the compiler queries and LSP completion
    // consumes; the `type_category` description's category enumeration is
    ...
    BuiltinMetadata {
        ...
        return_type: "string",
        example: Some(
            "comptime { extend (f\"fn name() -> string \\{ {string_lit(computed)} \\}\") }",
        ),
    },
];
...
mod tests {
    ...
    /// ADR-009 A1 S4: the `type_category` row's category enumeration must be
    /// generated from the shared runtime-owned catalog
    /// (`comptime_reflection::FrozenTypeCategory::ALL`) — the same catalog the
    /// compiler queries and LSP completion consumes. A hand-written variant
    /// list would drift the moment a category is added.
    #[test]
    fn type_category_row_enumerates_categories_from_shared_catalog() {
        let row = collect_builtin_metadata()
            .into_iter()
            .find(|meta| meta.name == "type_category")
            .expect("type_category builtin metadata should be registered");

        let generated = crate::comptime_reflection::FrozenTypeCategory::ALL
            .into_iter()
        ...
        assert!(
            ...
            "type_category row must embed the category catalog generated from \
             FrozenTypeCategory::ALL (expected to contain {generated:?});\ndescription: {}",
            row.description
        );
    }

    /// ADR-009 A1 S4: the registered `type_ref`/`type_category` rows must BE
    /// the shared-catalog descriptors — exactly once each, byte-identical.
    /// Reintroducing a hand-written row (or a duplicate that shadows the
    /// catalog one) fails this test.
    #[test]
    fn typed_reflection_rows_are_the_shared_catalog_descriptors() {
        let rows: Vec<_> = collect_builtin_metadata()
            .into_iter()
            .filter(|meta| meta.name == "type_ref" || meta.name == "type_category")
            .collect();
        assert_eq!(
            rows.len(),
            2,
            "expected exactly one type_ref row and one type_category row"
        );

        let type_ref = rows.iter().find(|m| m.name == "type_ref").unwrap();
        let type_category = rows.iter().find(|m| m.name == "type_category").unwrap();
        assert_eq!(**type_ref, crate::comptime_reflection::TYPE_REF_BUILTIN_ROW);
        assert_eq!(
            **type_category,
            crate::comptime_reflection::TYPE_CATEGORY_BUILTIN_ROW
        );
