...
// ---------------------------------------------------------------------------
// GC cycle-collection metadata (Phase 0, real-gc-cycle-collection.md §3.1/§7).
//
// Bacon–Rajan trial-deletion needs a 2-bit tri-color (Black/Gray/White/Purple)
// plus a 1-bit `buffered` flag per header carrier. Per design §3.1 these live in
// `HeapHeader.flags` bits 3–4 (color) and bit 5 (buffered); bits 0–2
// (MARKED/PINNED/READONLY) and `_pad` (offset 7, element-type-stamped by
// TypedArray) are untouched, so the header layout / DATA_OFFSET / offset tests
// stay byte-identical.
//
...
//
// NOTE (surfaced for Phase 1+): the *runtime* header carriers use the distinct
// `crate::v2::heap_header::HeapHeader`, which already claims **bit 3 (0x08) =
// FLAG_CLOSURE_CAPTURES_DROPPED** on closure blocks. The color field below
// (bits 3–4) therefore overlaps that flag on the `Closure` carrier. This is
// inert in Phase 0 (gc_meta is never invoked on live objects; no collector
// exists), but Phase 1/2 must reconcile the closure color placement (relocate
// the color field, migrate the closure flag, or special-case the Closure kind)
// before any barrier body reads/writes color on a closure header.
/// GC color field mask within `HeapHeader.flags` (bits 3–4, 2-bit color).
#[cfg(feature = "gc")]
pub const GC_COLOR_MASK: u8 = 0b0001_1000;
/// Bit position of the low color bit within `HeapHeader.flags`.
#[cfg(feature = "gc")]
pub const GC_COLOR_SHIFT: u8 = 3;
/// GC `buffered` flag within `HeapHeader.flags` (bit 5).
#[cfg(feature = "gc")]
pub const GC_FLAG_BUFFERED: u8 = 0b0010_0000;

/// Bacon–Rajan trial-deletion color (design §3.1). Two-bit field stored in
/// `HeapHeader.flags` bits 3–4 for header carriers, or in the side table for
