//! NaN-boxing bit layout constants and inline helpers.
//!
//! Extracted from `value_word.rs` in Phase R6.3. This module holds the raw bit
//! manipulation primitives used to pack type tags + payloads into an 8-byte
//! `u64` value word via IEEE 754 quiet NaN space.
//!
//! ## NaN-boxing scheme
//!
//! All tagged values use sign bit = 1 with a quiet NaN exponent, giving us 51
//! bits for tag + payload. Normal f64 values (including NaN, which is
//! canonicalised to a positive quiet NaN) are stored directly and never
//! collide with our tagged range.
//!
//! ```text
