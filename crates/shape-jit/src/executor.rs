...
use shape_ast::ast::{
    Assignment, BlockItem, ComprehensionClause, EnumConstructorPayload, Expr, ForInit, Item,
    ObjectEntry, QueryClause, Statement, VariableDecl,
};
...
impl ProgramExecutor for JITExecutor {
    fn execute_program(
        ...
        // W10 fuzzy-comparison divergence (2026-06-26): the bytecode VM
        // compiles `a ~= b within tol` into tolerance-aware numeric bytecode
        // (`abs(a-b) <= tol` for `~=`), but the MIR consumed by the JIT
        // currently lowers `Expr::FuzzyComparison` to a plain
        // `Rvalue::BinaryOp(Eq|Gt|Lt, lhs, rhs)` and drops the tolerance.
        // Native JIT comparison lowering cannot recover the lost tolerance
        // from operand kinds or values without reintroducing runtime probing.
        // Route the whole program through the bytecode interpreter before
        // MIR codegen so `--mode jit` observes the same semantics as VM.
        if program_contains_fuzzy_comparison(program) {
            tracing::info!(
                target: "shape_jit::fallback",
                function = "main",
                reason = "fuzzy comparison is tolerance-aware in bytecode but \
                          tolerance is not represented in JIT MIR",
                "jit-fallback: fuzzy comparison, running under interpreter",
            );
            return self.bytecode_executor.execute_program(engine, program);
        }
...
fn program_contains_fuzzy_comparison(program: &Program) -> bool {
    program.items.iter().any(item_contains_fuzzy_comparison)
}

fn item_contains_fuzzy_comparison(item: &Item) -> bool {
    match item {
        Item::Function(func, _) => func.body.iter().any(statement_contains_fuzzy_comparison),
        Item::VariableDecl(decl, _) => variable_decl_contains_fuzzy_comparison(decl),
        Item::Assignment(assignment, _) => assignment_contains_fuzzy_comparison(assignment),
        Item::Expression(expr, _) => expr_contains_fuzzy_comparison(expr),
        Item::Statement(stmt, _) => statement_contains_fuzzy_comparison(stmt),
        Item::Comptime(stmts, _) => stmts.iter().any(statement_contains_fuzzy_comparison),
        _ => false,
    }
}

fn variable_decl_contains_fuzzy_comparison(decl: &VariableDecl) -> bool {
    decl.value
        .as_ref()
        .is_some_and(expr_contains_fuzzy_comparison)
}

fn assignment_contains_fuzzy_comparison(assignment: &Assignment) -> bool {
    expr_contains_fuzzy_comparison(&assignment.value)
}

fn statement_contains_fuzzy_comparison(stmt: &Statement) -> bool {
    match stmt {
        Statement::Return(expr, _) => expr.as_ref().is_some_and(expr_contains_fuzzy_comparison),
        Statement::VariableDecl(decl, _) => variable_decl_contains_fuzzy_comparison(decl),
        Statement::Assignment(assignment, _) => assignment_contains_fuzzy_comparison(assignment),
        Statement::Expression(expr, _) => expr_contains_fuzzy_comparison(expr),
        Statement::For(for_loop, _) => {
            let init_has_fuzzy = match &for_loop.init {
                ForInit::ForIn { iter, .. } => expr_contains_fuzzy_comparison(iter),
                ForInit::ForC {
                    init,
                    condition,
                    update,
                } => {
                    statement_contains_fuzzy_comparison(init)
                        || expr_contains_fuzzy_comparison(condition)
                        || expr_contains_fuzzy_comparison(update)
                }
            };
            init_has_fuzzy
                || for_loop
                    .body
                    .iter()
                    .any(statement_contains_fuzzy_comparison)
        }
        Statement::While(while_loop, _) => {
            expr_contains_fuzzy_comparison(&while_loop.condition)
                || while_loop
                    .body
                    .iter()
                    .any(statement_contains_fuzzy_comparison)
        ...
        Statement::If(if_stmt, _) => {
            expr_contains_fuzzy_comparison(&if_stmt.condition)
                || if_stmt
                    .then_body
                    .iter()
                    .any(statement_contains_fuzzy_comparison)
                || if_stmt
                    .else_body
                    .as_ref()
                    .is_some_and(|body| body.iter().any(statement_contains_fuzzy_comparison))
        }
        ...
        | Statement::ReplaceModuleExpr { expression, .. } => {
            expr_contains_fuzzy_comparison(expression)
        }
        Statement::ReplaceBody { body, .. } => body.iter().any(statement_contains_fuzzy_comparison),
        Statement::Break(_)
...
fn expr_contains_fuzzy_comparison(expr: &Expr) -> bool {
    match expr {
        Expr::FuzzyComparison { .. } => true,
        Expr::DataRelativeAccess { reference, .. } => expr_contains_fuzzy_comparison(reference),
        Expr::PropertyAccess { object, .. } => expr_contains_fuzzy_comparison(object),
        Expr::IndexAccess {
            object,
            index,
            end_index,
            ...
            expr_contains_fuzzy_comparison(object)
                || expr_contains_fuzzy_comparison(index)
                || end_index
                    .as_ref()
                    .is_some_and(|expr| expr_contains_fuzzy_comparison(expr))
        }
        Expr::BinaryOp { left, right, .. } => {
            expr_contains_fuzzy_comparison(left) || expr_contains_fuzzy_comparison(right)
        }
        Expr::UnaryOp { operand, .. } => expr_contains_fuzzy_comparison(operand),
        Expr::FunctionCall {
            args, named_args, ..
        }
        | Expr::QualifiedFunctionCall {
            ...
            args.iter().any(expr_contains_fuzzy_comparison)
                || named_args
                    .iter()
                    .any(|(_, expr)| expr_contains_fuzzy_comparison(expr))
        }
        Expr::EnumConstructor { payload, .. } => match payload {
            EnumConstructorPayload::Unit => false,
            EnumConstructorPayload::Tuple(exprs) => {
                exprs.iter().any(expr_contains_fuzzy_comparison)
            }
            EnumConstructorPayload::Struct(fields) => fields
                .iter()
                .any(|(_, expr)| expr_contains_fuzzy_comparison(expr)),
        },
        Expr::Conditional {
            ...
            expr_contains_fuzzy_comparison(condition)
                || expr_contains_fuzzy_comparison(then_expr)
                || else_expr
                    .as_ref()
                    .is_some_and(|expr| expr_contains_fuzzy_comparison(expr))
        }
        Expr::Object(entries, _) => entries.iter().any(|entry| match entry {
            ObjectEntry::Field { value, .. } | ObjectEntry::Spread(value) => {
                expr_contains_fuzzy_comparison(value)
            }
        }),
        Expr::Array(exprs, _) => exprs.iter().any(expr_contains_fuzzy_comparison),
        Expr::ListComprehension(comp, _) => {
            expr_contains_fuzzy_comparison(&comp.element)
                || comp.clauses.iter().any(|clause| match clause {
                    ComprehensionClause {
                        iterable, filter, ..
                    } => {
                        expr_contains_fuzzy_comparison(iterable)
                            || filter
                                .as_ref()
                                .is_some_and(|expr| expr_contains_fuzzy_comparison(expr))
                    }
        ...
        Expr::Block(block, _) => block.items.iter().any(|item| match item {
            BlockItem::VariableDecl(decl) => variable_decl_contains_fuzzy_comparison(decl),
            BlockItem::Assignment(assignment) => assignment_contains_fuzzy_comparison(assignment),
            BlockItem::Statement(stmt) => statement_contains_fuzzy_comparison(stmt),
            BlockItem::Expression(expr) => expr_contains_fuzzy_comparison(expr),
        }),
        Expr::TypeAssertion {
            expr,
            ...
            expr_contains_fuzzy_comparison(expr)
                || meta_param_overrides
                    .as_ref()
                    .is_some_and(|overrides| overrides.values().any(expr_contains_fuzzy_comparison))
        }
        Expr::InstanceOf { expr, .. } => expr_contains_fuzzy_comparison(expr),
        Expr::FunctionExpr { body, .. } => body.iter().any(statement_contains_fuzzy_comparison),
        Expr::Spread(expr, _) => expr_contains_fuzzy_comparison(expr),
        Expr::If(if_expr, _) => {
            expr_contains_fuzzy_comparison(&if_expr.condition)
                || expr_contains_fuzzy_comparison(&if_expr.then_branch)
                || if_expr
                    .else_branch
                    .as_ref()
                    .is_some_and(|expr| expr_contains_fuzzy_comparison(expr))
        ...
        Expr::While(while_expr, _) => {
            expr_contains_fuzzy_comparison(&while_expr.condition)
                || expr_contains_fuzzy_comparison(&while_expr.body)
        }
        Expr::For(for_expr, _) => {
            expr_contains_fuzzy_comparison(&for_expr.iterable)
                || expr_contains_fuzzy_comparison(&for_expr.body)
        }
        Expr::Loop(loop_expr, _) => expr_contains_fuzzy_comparison(&loop_expr.body),
        Expr::Let(let_expr, _) => {
            let_expr
                .value
                .as_ref()
                .is_some_and(|expr| expr_contains_fuzzy_comparison(expr))
                || expr_contains_fuzzy_comparison(&let_expr.body)
        }
        Expr::Assign(assign_expr, _) => {
            expr_contains_fuzzy_comparison(&assign_expr.target)
                || expr_contains_fuzzy_comparison(&assign_expr.value)
        }
        Expr::Break(expr, _) | Expr::Return(expr, _) => expr
            .as_ref()
            .is_some_and(|expr| expr_contains_fuzzy_comparison(expr)),
        Expr::MethodCall {
            receiver,
            ...
            expr_contains_fuzzy_comparison(receiver)
                || args.iter().any(expr_contains_fuzzy_comparison)
                || named_args
                    .iter()
                    .any(|(_, expr)| expr_contains_fuzzy_comparison(expr))
        ...
        Expr::Match(match_expr, _) => {
            expr_contains_fuzzy_comparison(&match_expr.scrutinee)
                || match_expr.arms.iter().any(|arm| {
                    arm.guard
                        .as_ref()
                        .is_some_and(|expr| expr_contains_fuzzy_comparison(expr))
                        || expr_contains_fuzzy_comparison(&arm.body)
                })
        }
        ...
        | Expr::UsingImpl { expr, .. }
        | Expr::Await(expr, _)
        | Expr::AsyncScope(expr, _)
        | Expr::Reference { expr, .. } => expr_contains_fuzzy_comparison(expr),
        Expr::SimulationCall { params, .. } => params
            .iter()
            .any(|(_, expr)| expr_contains_fuzzy_comparison(expr)),
        Expr::FromQuery(query, _) => {
            expr_contains_fuzzy_comparison(&query.source)
                || query
                    .clauses
                    .iter()
                    .any(query_clause_contains_fuzzy_comparison)
                || expr_contains_fuzzy_comparison(&query.select)
        }
        Expr::StructLiteral { fields, .. } => fields
            .iter()
            .any(|(_, expr)| expr_contains_fuzzy_comparison(expr)),
        Expr::Join(join, _) => join
            .branches
            .iter()
            .any(|branch| expr_contains_fuzzy_comparison(&branch.expr)),
        Expr::Annotated { target, .. } => expr_contains_fuzzy_comparison(target),
        Expr::AsyncLet(async_let, _) => expr_contains_fuzzy_comparison(&async_let.expr),
        Expr::Comptime(stmts, _) => stmts.iter().any(statement_contains_fuzzy_comparison),
        Expr::ComptimeFor(comptime_for, _) => {
            expr_contains_fuzzy_comparison(&comptime_for.iterable)
                || comptime_for
                    .body
                    .iter()
                    .any(statement_contains_fuzzy_comparison)
        }
        Expr::TableRows(rows, _) => rows.iter().flatten().any(expr_contains_fuzzy_comparison),
        Expr::Literal(_, _)
        | Expr::Identifier(_, _)
        | Expr::DataRef(_, _)
        ...
        | Expr::Continue(_)
        | Expr::Unit(_)
        | Expr::WindowExpr(_, _) => false,
    }
}

fn query_clause_contains_fuzzy_comparison(clause: &QueryClause) -> bool {
    match clause {
        QueryClause::Where(expr) => expr_contains_fuzzy_comparison(expr),
        QueryClause::OrderBy(specs) => specs
            .iter()
            .any(|spec| expr_contains_fuzzy_comparison(&spec.key)),
        QueryClause::GroupBy { element, key, .. } => {
            expr_contains_fuzzy_comparison(element) || expr_contains_fuzzy_comparison(key)
        }
        QueryClause::Join {
            source,
            left_key,
            ...
            expr_contains_fuzzy_comparison(source)
                || expr_contains_fuzzy_comparison(left_key)
                || expr_contains_fuzzy_comparison(right_key)
        }
        QueryClause::Let { value, .. } => expr_contains_fuzzy_comparison(value),
    }
}
