...
use super::super::ffi::object::closure as closure_ffi;
...
pub fn register_arc_symbols(builder: &mut JITBuilder) {
    ...
    // W15.2-LANG-4 jit-filter-predicate close (2026-05-18). ADR-006
    // §2.7.11/Q12 `Arc<HeapValue::ClosureRaw>` carrier retain/release.
    // Refcount discipline at slots whose `NativeKind` is
    // `Ptr(HeapKind::Closure)` dispatches HERE instead of the legacy
    // `jit_arc_retain` / `jit_arc_release` (which would scribble on
    // the inner `HeapValue` payload at offset +4) — see
    // `mir_compiler/ownership.rs::retain_func_for_place` /
    // `release_func_for_place` for the dispatch arms.
    builder.symbol(
        "jit_arc_closure_retain",
        closure_ffi::jit_arc_closure_retain as *const u8,
    );
    builder.symbol(
        "jit_arc_closure_release",
        closure_ffi::jit_arc_closure_release as *const u8,
    );
}
...
pub fn declare_arc_functions(module: &mut JITModule, ffi_funcs: &mut HashMap<String, FuncId>) {
    ...
    // W15.2-LANG-4 jit-filter-predicate close (2026-05-18). Both
    // entries are `extern "C" fn(bits: i64)` — a single
    // `Arc::into_raw(Arc<HeapValue>) as u64` argument, no return
    // value. Mirror of the `arc_string_*` declarations above.
    // jit_arc_closure_retain(bits: i64) -> void
    {
        let mut sig = module.make_signature();
        sig.params.push(AbiParam::new(types::I64));
        if let Ok(func_id) =
            module.declare_function("jit_arc_closure_retain", Linkage::Import, &sig)
        {
            ffi_funcs.insert("jit_arc_closure_retain".to_string(), func_id);
        }
    }
    // jit_arc_closure_release(bits: i64) -> void
    {
        let mut sig = module.make_signature();
        sig.params.push(AbiParam::new(types::I64));
        if let Ok(func_id) =
            module.declare_function("jit_arc_closure_release", Linkage::Import, &sig)
        {
            ffi_funcs.insert("jit_arc_closure_release".to_string(), func_id);
        }
    }
}
