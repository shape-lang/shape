...
impl VirtualMachine {
    ...
    pub(crate) fn exec_v2_sized_int(&mut self, instruction: &Instruction) -> Result<(), VMError> {
        match instruction.opcode {
            OpCode::AddI32 => {
                let b = self.pop_tagged_i64()? as i32;
                let a = self.pop_tagged_i64()? as i32;
                self.push_tagged_i64(a.wrapping_add(b) as i64)
            }
            OpCode::SubI32 => {
                let b = self.pop_tagged_i64()? as i32;
                let a = self.pop_tagged_i64()? as i32;
                self.push_tagged_i64(a.wrapping_sub(b) as i64)
            }
            OpCode::MulI32 => {
                let b = self.pop_tagged_i64()? as i32;
                let a = self.pop_tagged_i64()? as i32;
                self.push_tagged_i64(a.wrapping_mul(b) as i64)
            }
            OpCode::DivI32 => {
                let b = self.pop_tagged_i64()? as i32;
                let a = self.pop_tagged_i64()? as i32;
                ...
                self.push_tagged_i64(a.wrapping_div(b) as i64)
            }
            OpCode::ModI32 => {
                let b = self.pop_tagged_i64()? as i32;
                let a = self.pop_tagged_i64()? as i32;
                ...
                self.push_tagged_i64(a.wrapping_rem(b) as i64)
            }
            OpCode::EqI32 => {
                let b = self.pop_tagged_i64()? as i32;
                let a = self.pop_tagged_i64()? as i32;
