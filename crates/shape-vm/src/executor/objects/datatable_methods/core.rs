...
pub(crate) fn handle_columns(
    ...
    let names: Vec<ValueWord> = dt
        ...
        .collect();
    Ok(ValueWord::from_array(Arc::new(names)))
}
...
pub(crate) fn handle_column(
    ...
    Ok(ValueWord::from_column_ref(schema_id, dt.clone(), col_id))
}
...
pub(crate) fn handle_slice(
    ...
    Ok(wrap_result_table_nb(&args[0], sliced))
}
...
pub(crate) fn handle_execute(
    ...
    Ok(args.into_iter().next().unwrap_or_else(ValueWord::none))
}
...
pub(crate) fn handle_to_mat(
    ...
    Ok(ValueWord::from_matrix(std::sync::Arc::new(mat)))
}
...
pub(crate) fn handle_first(
    ...
    if dt.is_empty() {
        ...
        Ok(wrap_result_table_nb(&args[0], dt.slice(0, 1)))
    }
}
...
pub(crate) fn handle_last(
    ...
    if dt.is_empty() {
        ...
        let n = dt.row_count();
        Ok(wrap_result_table_nb(&args[0], dt.slice(n - 1, 1)))
    }
}
...
pub(crate) fn handle_select(
    ...
    if let Some(func_nb) = args.get(1) {
        if super::common::is_callable_nb(func_nb) {
            ...
            if row_count == 0 {
                return Ok(super::common::wrap_result_table_nb(
                    &args[0],
...
pub(crate) fn handle_columns_ref(
    ...
    Ok(ValueWord::from_array(Arc::new(cols)))
}
