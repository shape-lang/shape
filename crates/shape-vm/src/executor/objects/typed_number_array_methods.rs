...
//! `objects/mod.rs` currently handles `Vec<number>` receivers via the legacy
//! `HeapKind::FloatArray` → `FLOAT_ARRAY_METHODS` cascade (for v1
//! `Arc<AlignedTypedBuffer>` receivers) and via
//! `dispatch_v2_typed_array_method` (for native v2 `TypedArray<f64>`
//! receivers).
//!
...
//! that calls on a native-typed `TypedArray<f64>` dispatch straight here,
//! eliminating the runtime HeapKind match.
//!
//! Until then, the entries in [`TYPED_NUMBER_ARRAY_METHODS`] are unreachable
