//! Integration tests for std::time module and Instant methods.
//!
//! Tests exercise:
//! - time.now() / time.stopwatch() returning Instant values
//! - Instant.elapsed(), elapsed_ms(), elapsed_us(), elapsed_ns()
//! - Instant.duration_since(other)
//! - time.millis() returning epoch millis
//! - time.benchmark() returning timing object
//! - time.sleep_sync() validation

use shape_runtime::stdlib_time::create_time_module;
use shape_value::ValueWord;

/// Create a dummy `ModuleContext` for test-only module function calls.
...
#[test]
fn test_instant_elapsed_increases() {
    let module = create_time_module();
    let now_fn = module.get_export("now").unwrap();
    let instant = now_fn(&[], &test_ctx()).unwrap();

    // Do some trivial work to ensure elapsed > 0
...
#[test]
fn test_time_millis_returns_epoch() {
    let module = create_time_module();
...
#[test]
fn test_time_benchmark_returns_timing_object() {
    let module = create_time_module();
    let bench_fn = module.get_export("benchmark").unwrap();
    // Pass a dummy value and 10 iterations
    let result = bench_fn(
        &[ValueWord::from_f64(0.0), ValueWord::from_f64(10.0)],
        &test_ctx(),
    )
    .unwrap();
...
#[test]
fn test_time_sleep_sync_zero_succeeds() {
    let module = create_time_module();
...
#[test]
fn test_time_sleep_sync_negative_errors() {
    let module = create_time_module();
    let sleep_fn = module.get_export("sleep_sync").unwrap();
...
#[test]
fn test_time_sleep_sync_no_args_errors() {
    let module = create_time_module();
    let sleep_fn = module.get_export("sleep_sync").unwrap();
...
#[test]
fn test_time_module_has_all_exports() {
    let module = create_time_module();
