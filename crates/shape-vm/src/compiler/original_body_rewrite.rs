...
fn rewrite_in_expr(expr: &Expr, bound: &HashSet<String>, shadow: &str) -> Expr {
    match expr {
        ...
        Expr::FunctionExpr {
            ...
            Expr::FunctionExpr {
                // ADR-009 C1 (slice 2): the `ctx.original` rewrite runs OVER a
                // `replace body` expansion — the replacement body's closures
                // are generated nodes and must keep their stamp through the
...
/// ADR-009 C1 (slice 2) — the `replace body` leg of node-borne provenance.
///
/// A `replace body` replacement is GENERATED code that compiles under the USER's
/// function name, so the Wave-46 gate's old name predicate never fired on it.
/// The gate now reads `Expr::FunctionExpr::generated_origin`, and the stamp is
/// applied to the replacement body — which then passes through THIS rewrite. The
/// `Expr::FunctionExpr` arm above names the field explicitly (no `..`), so
/// dropping it is a compile error; this test pins the runtime half.
#[cfg(test)]
mod generated_origin_survives_rewrite {
    use super::*;
    ...
    fn stamped_body(src: &str) -> Vec<Statement> {
        ...
        let mut body = program
            ...
            .expect("function");
        // K1b: the stamp comes from the ONE mint (expansion_provenance.rs).
        let origin =
            ...
            match v {
                serde_json::Value::Object(map) => {
                    if let Some(f) = map.get("FunctionExpr") {
                        if let Some(path) = f.pointer("/generated_origin/node_path") {
                            out.push(
                                path.as_array()
                                    .unwrap()
                                    .iter()
    ...
    fn stamp_survives_the_ctx_original_rewrite_including_nested_closures() {
        // The rewrite REBUILDS every node it walks, so a closure containing a
        // `ctx.original()` call exercises the rebuild path, not a clone.
        let body = stamped_body(
            "fn f() -> int { let outer = || { let inner = || ctx.original() + 1; inner() }; outer() }",
        );
        let before = closure_paths(&body);
        assert_eq!(before.len(), 2, "fixture must have an outer + nested closure");
