...
pub struct BytecodeCompiler {
    ...
    /// Names of free functions that a whole-program pre-pass already
    /// materialized from computed `extend (expr)` comptime directives
    /// (comptime-excellence §4.5.1). The pre-pass runs the annotation's
    /// comptime handler before the analyzer, parses the generated source,
    /// and inserts the resulting free functions as ordinary program items so
    /// they are visible to `fn main()` and to every user function body — not
    /// only to top-level script statements. When pass-2 later re-runs the
    /// same handler while compiling the annotated type, `apply_comptime_extend_items`
    /// consults this set and skips re-registering / re-compiling those
    /// functions (they are already normal items), avoiding a double
    /// definition. Type-extension methods generated by the same handler are
    /// NOT recorded here — those are still applied in pass-2.
    pub(crate) materialized_comptime_fns: std::collections::HashSet<String>,
