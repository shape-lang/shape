        ...
        for target in [CaptureTarget::Local(1), CaptureTarget::ModuleBinding(1)] {
            for ownership in [
                ...
                for witness_shared_local in [false, true] {
                    let mut reference = facts(Some(target), Some(ownership), witness_shared_local);
...
fn c0905_unresolvable_target_is_rejected_not_defaulted() {
    for mode in [CaptureMode::Move, CaptureMode::Share] {
        let message = reject(
            mode,
            &facts(None, Some(BindingOwnershipClass::OwnedImmutable), false),
        );
...
fn c0904_move_cannot_unshare_a_var_or_a_sibling_shared_local() {
    ...
    assert!(
        sibling_shared.starts_with("[C0904]"),
        "got {sibling_shared}"
    );
}
                                    ...
                                    Ok(plan) => {
                                        ...
                                        match mode {
                                            CaptureMode::Move => assert!(matches!(
                                                plan.kind(),
                                                CaptureKind::Immutable | CaptureKind::OwnedMutable
                                            )),
                                            CaptureMode::Share => {
